...
```mermaid
  graph TB
    ...
    %% エッジラベルは左寄せ・不透明の白背景
    %% 評価者 → アプリ
    Review --->|"<div style='display:inline-block;text-align:left;background-color:#ffffff;'><b>評価者 → アプリ</b><br/>1. アプリに登録<br/>2. 目標（パッケージ）を決める<br/>3. 評価軸を決める<br/>4. 評価軸とパッケージの紐付け<br/>14. 評価結果をアップロード"| Platform
...
5. **（供給者）出品と評価軸ポイント指定**
   ...
   - 需要量が供給量を上回る場合はオークションを行うが、その際に需要者が使用できる評価軸ポイントを供給者が、出品の設定時に指定する
     - それにより、ポイントの使用用途を作ることで、供給者が応援したい評価軸のエコシステム発展に貢献しているので、その評価軸ポイントを報酬として得ることができる
6. **（需要者・アプリ）出品データを参照する（アプリが需要者に出品情報を表示）**
   - 需要者が、供給者の出品内容を確認する
7. **（需要者）入札・落札**
   - 基本的には、無料で供給者から商品やサービスを得られる
   - 需要量が供給量を上回る場合は、供給者が指定した評価軸ポイントを入札として使用してオークションを行う
8. **（需要者・アプリ）入札で使用した評価軸ポイントを消費する**
   - アプリ側で、オークションに使用した評価軸ポイントが消費される
9. **（供給者・アプリ）落札者情報の伝達**
   - アプリが供給者に落札した需要者の情報を伝える
...
1. **（評価者）評価結果を無料主義アプリに登録**
   - 評価者が評価結果をアプリに登録・アップロードする
2. **（アプリ→供給者）評価軸ポイントを付与する**
...
**基本ルール**<br>
`1.`のあと、`2.`〜`4.`を踏まえたうえで、`5.`〜`15.`のサイクルを繰り返します。
...
6. C さんが、無料主義アプリに出品された「価値の提供 Z のパン」のオークションに、評価軸 X の評価軸ポイントを入札として使用し、落札します。<br>
   ...
   - フェーズ：**（需要者）入札・落札**（入札に使用した評価軸ポイントの消費を含む）
...
- フェーズ：**（アプリ→供給者）評価軸ポイントを付与する**（評価結果を踏まえてアプリが処理する）
