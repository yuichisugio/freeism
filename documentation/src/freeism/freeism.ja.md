...
`1.`のあと、`2.`〜`4.`を踏まえたうえで、`5.`〜`8.`を繰り返します。
...
```mermaid
  graph TB
    ...
    %% エッジラベルは左寄せ・不透明の白背景
    %% 評価者 → アプリ
    Review --->|"<div style='display:inline-block;text-align:left;background-color:#ffffff;'><b>評価者 → アプリ</b><br/>1. アプリに登録<br/>2 目標（パッケージ）を決める<br/>3 評価軸を決める<br/>4 評価軸とパッケージの紐付け<br/>8 評価結果に基づく評価軸ポイント付与の手続き</div>"| Platform

    %% 需要者 → アプリ
    Demand --->|"<div style='display:inline-block;text-align:left;background-color:#ffffff;'><b>需要者 → アプリ</b><br/>1 アプリに登録<br/>6 入札・落札（評価軸ポイント）<br/>受領の連絡</div>"| Platform

    %% 供給者 → アプリ
    Supply --->|"<div style='display:inline-block;text-align:left;background-color:#ffffff;'><b>供給者 → アプリ</b><br/>1 アプリに登録<br/>5 出品と評価軸ポイント指定（価値の提供）</div>"| Platform

    %% 評価者 → 供給者
    Review --->|"<div style='display:inline-block;text-align:left;background-color:#ffffff;'><b>評価者 → 供給者</b><br/>7 貢献度の評価</div>"| Supply

    %% 供給者 → 需要者
    Supply --->|"<div style='display:inline-block;text-align:left;background-color:#ffffff;'><b>供給者 → 需要者</b><br/>5・6 価値の提供・商品やサービスの受け渡し</div>"| Demand

    %% アプリ → 需要者
    Platform -->|"<div style='display:inline-block;text-align:left;background-color:#ffffff;'><b>アプリ → 需要者</b><br/>6 入札で使用した評価軸ポイントの消費</div>"| Demand

    %% アプリ → 供給者
    Platform --->|"<div style='display:inline-block;text-align:left;background-color:#ffffff;'><b>アプリ → 供給者</b><br/>落札者情報の伝達<br/>8 評価軸ポイントの付与</div>"| Supply

    %% アプリ → 評価者
    Platform ---->|"<div style='display:inline-block;text-align:left;background-color:#ffffff;'><b>アプリ → 評価者</b><br/>供給者の情報提供（評価のため）</div>"| Review
```

1. **（供給者・評価者・需要者）無料主義アプリに登録**
   - 付与されたポイント管理のために登録が必要
   - 補足：「無料主義アプリ」は、無料主義のあらゆる機能を管理するアプリ
2. **（評価者）目標を決める**
   - この目標を「パッケージ」と呼ぶ
   - 例）自分が住む街の発展
3. **（評価者）評価する軸を決める**
   - 評価する軸（価値観・視点）は、なんでも良いし、自由に作り出して良い
   - 評価する軸のことを「評価軸」と呼び、その評価軸が発行するポイントを「評価軸ポイント」と呼ぶ
   - 例）犯罪発生率、電車の止まる本数
4. **（評価者）評価軸とパッケージを紐付ける**
   - パッケージ（目標）の達成に貢献しているか評価する指標として「評価軸」を紐付ける
5. **（供給者）価値の提供**
   - 価値を提供する内容であれば、なんでも良い
   - 例）ゴミ拾い、町内会の活動、町内会の資金を管理する SaaS の開発
   - 需要量が供給量を上回る場合はオークションを行うが、その際に需要者が使用できる評価軸ポイントを供給者が指定する
     - それにより、供給者が貢献したい評価軸のエコシステム発展に貢献して報酬を得ることができる
6. **（需要者）供給者から商品やサービスを得る**
   - 基本的には、無料で提供してもらう
   - 需要量が供給量を上回る場合は、供給者が指定した評価軸ポイントを入札として使用してオークションを行う
7. **（評価者）評価する軸を基に、供給者を評価する**
   - 評価軸が「犯罪発生率」の場合は、その率の減少への貢献度によってポイントを付与する
8. **（評価者）評価結果をもとに、供給者に評価軸ポイントを付与する**
...
**前提**<br>
以下の例は、「オークション形式」かつ「入札に使用したポイントは消費する形式」かつ「供給者が、どの評価軸ポイントで購入できるか決める方式」の条件で進めます。<br>
※意味が理解できていなくても読み進めてください。

**基本ルール**<br>
`1.`のあと、`2.`〜`4.`を踏まえたうえで、`5.`〜`8.`のサイクルを繰り返します。

1. 無料主義アプリのアカウント作成・SNS アカウント連携などを行い、登録します
   - フェーズ：**（供給者・評価者・需要者）無料主義アプリに登録**
   - 付与されたポイント管理のために登録が必要です
2. A さんが、目標として「町（M）の発展」を決め、この目標をパッケージとして作成します
   - フェーズ：**（評価者）目標を決める**
3. A さんが、評価軸（X）として、あらゆる作業を評価することを決めます
   - フェーズ：**（評価者）評価する軸を決める**
   - この評価軸（X）が発行するポイントを、評価軸ポイントと呼びます
4. A さんが、評価軸（X）をパッケージ「町（M）の発展」と紐付けます
   - フェーズ：**（評価者）評価軸とパッケージを紐付ける**
   - パッケージ（目標）の達成に貢献しているか評価する指標として、評価軸（X）を紐付けます
5. B さんが、その町（M）で数量限定のパンを提供します（価値の提供を Z とします）。需要量が供給量を上回る場合に、需要者が使用できる評価軸ポイントとして評価軸 X を指定したうえで、無料主義アプリに出品します
   - フェーズ：**（供給者）価値の提供**
   - 場合分け：**無料主義アプリに出品する場合**
   - 注意：「供給者が、どの評価軸ポイントで購入できるか決める方式」を採用する場合、出品時に使用できる評価軸ポイントを明示する必要があります
   - ポイント：**無料主義アプリに商品やサービスを出品することは必須ではありません。**
     - しかし、出品することで、その評価軸のエコシステムの発展に貢献するため、通常より重み付けして評価してくれる評価軸も出てくるでしょう。<br>
     - 評価者が、出品による報酬を上乗せして評価軸のエコシステムとして育てるインセンティブは、評価・管理することによる報酬の額を増やし、ポイントの使い道を増やすためです
     - **「評価者を評価する仕組み」** と **「評価軸エコシステム発展の貢献を評価する仕組み」** は、価値の提供を評価することで、上記の仕組み・インセンティブ構造を実現できます
6. C さんが、無料主義アプリに出品された「価値の提供 Z のパン」のオークションに、評価軸 X の評価軸ポイントを入札として使用し、落札します。<br>
   落札に使用した評価軸 X の評価軸ポイントは、消費されます
   - フェーズ：**（需要者）供給者から商品やサービスを得る**
   - 注意：基本的には無料で提供してもらいます。本例のように需要量が供給量を上回る場合は、供給者が指定した評価軸ポイントを入札に使うオークションの流れになります。前提として「入札に使用したポイントは消費する形式」とします
7. D さんが、その町の清掃のスケジュール調整を行います（価値の提供を Y とします）
   - フェーズ：**（供給者）価値の提供**
   - 場合分け：**無料主義アプリに出品しない場合**
     - 提供内容は誰もが得られるサービスであるため、出品は不要で、オークションもありません
8. その町に住む方々が、D さんの価値の提供 Y の成果物を受け取ります
   - フェーズ：**（需要者）供給者から商品やサービスを得る**
   - 注意：全員が得られる内容なので、無条件で無料で得られます
   - ポイント：出品がないため、無料主義アプリを経由しません
9. E さんが、貢献の種類によって、価値の提供 Z の供給者 B さん、価値の提供 Y の供給者 D さんを評価します
   - B さんは出品したため、通常の貢献による評価に加えて重み付けする、といった整理ができます
   - フェーズ：**（評価者）評価する軸を基に、供給者を評価する**
   - **（貢献度合いの計算方法）**
     - 貢献度合いの計算方法は、参加者内で議論して決めたり、目標を定量化した指標と商品供給の相関性や因果関係から貢献度を決めるなど、様々な貢献度合いの計算方法で決めると良いです
10. 評価結果をもとに、評価軸（X）の評価軸ポイントが供給者に付与されます

- フェーズ：**（評価者）評価結果をもとに、供給者に評価軸ポイントを付与する**
- ポイント：「他の人から得たポイント」が報酬になるのではなく、「第三者による評価」が報酬になります
- ポイントの付与方法：貢献度の決定後、無料主義アプリに代表者が情報を入力して、ポイント付与を行います
...
5. **マイナス評価による罰金・罰則**
   - 無料主義では、評価軸の方向性と逆行する場合は、マイナス評価になりポイントが減ることになる
6. 占める
...
- 1 ポイントも無料主義プラットフォームに預けないで得られる商品やサービスのこと

プロジェクト

- 現在の株式会社の代わりの仕組みのこと
- それぞれの位置関係
  - 無料主義では、「無料主義アプリ」「評価軸」「プロジェクト」に細分化されていく
  - 無料主義プラットフォームが国を束ねる仕組みの役割、貢献度のルールが国の役割、プロジェクトが企業の役割
...
- 貢献度のルールによって違うだろうけど、いったんは落札額 ✖︎ 入札参加者数に比例して、預ける必要がある期間が延びるようにする
