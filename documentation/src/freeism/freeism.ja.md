...
- [無料主義 v3 beta](#無料主義-v3-beta)
  ...
  - [想定課題と解決策](#想定課題と解決策)
    - [データ公開したくない問題](#データ公開したくない問題)
    - [商材ごとにSNSアカウントが無い場合](#商材ごとにsnsアカウントが無い場合)
    - [どの粒度の目標・評価軸にすれば良い？](#どの粒度の目標評価軸にすれば良い)
    - [交換比率の必要性](#交換比率の必要性)
    - [国・街のポイント付与方法について](#国街のポイント付与方法について)
    - [「即決価格」の必要性](#即決価格の必要性)
    - [初期ユーザーの使用インセンティブ](#初期ユーザーの使用インセンティブ)
    - [レバレッジ](#レバレッジ)
    - [固定報酬を付与する方法](#固定報酬を付与する方法)
    - [保有ポイントを公開する？](#保有ポイントを公開する)
    - [分配する仕組みは避けたい](#分配する仕組みは避けたい)
    - [毎月のポイント発行上限を設定する？](#毎月のポイント発行上限を設定する)
    - [ポイントの分配がない仕組みの場合に、効率化するインセンティブがない](#ポイントの分配がない仕組みの場合に効率化するインセンティブがない)
    - [評価軸の仕組みについて](#評価軸の仕組みについて)
    - [参考・参照・依存関係の調査方法](#参考参照依存関係の調査方法)
    - [出品なしのタスク報告は必要か](#出品なしのタスク報告は必要か)
    - [保険の仕組みを実現する方法](#保険の仕組みを実現する方法)
    - [共同所有自己申告税COST](#共同所有自己申告税cost)
    - [無料主義アナキスト](#無料主義アナキスト)
    - [普通になったときに感謝・評価しなくなる問題](#普通になったときに感謝評価しなくなる問題)
    - [譲渡可能なポイントの必要性](#譲渡可能なポイントの必要性)
    - [作業内容を全てデータベースに保存することは現実的か](#作業内容を全てデータベースに保存することは現実的か)
    - [ポイントを預ける必要性](#ポイントを預ける必要性)
    - [複数種類の評価軸ポイントを組み合わせて入札する場合の処理](#複数種類の評価軸ポイントを組み合わせて入札する場合の処理)
    - [商品の提供を強制的にする仕組み](#商品の提供を強制的にする仕組み)
    - [正しい評価をする仕組み](#正しい評価をする仕組み)
    - [プリンシパル・エージェント問題を解決したい](#プリンシパルエージェント問題を解決したい)
    - [恣意的な数値を減らしたい](#恣意的な数値を減らしたい)
    - [指標のハック（グッドハートの法則）を減らしたい](#指標のハックグッドハートの法則を減らしたい)
    - [お金で解決することは反対意見が出やすい](#お金で解決することは反対意見が出やすい)
    - [資本主義に勝つ方法](#資本主義に勝つ方法)
    - [資本主義より報酬を多くする方法](#資本主義より報酬を多くする方法)
    - [資本主義では買えない商材を購入できるようにする方法](#資本主義では買えない商材を購入できるようにする方法)
    - [資本主義では報酬を得られない作業で報酬を貰える](#資本主義では報酬を得られない作業で報酬を貰える)
  - [今後考える必要があること](#今後考える必要があること)
    - [ポイント付与するためのIDが散らばる](#ポイント付与するためのidが散らばる)
    - [分析結果の公開を避けたい場合の対応](#分析結果の公開を避けたい場合の対応)
    - [「非干渉の仕組み」により、裸の王様になるのでは？](#非干渉の仕組みにより裸の王様になるのでは)
    - [「報酬なし」から「報酬あり」に転換すると、実行者がいなくなる事象](#報酬なしから報酬ありに転換すると実行者がいなくなる事象)
    - [正しい意思決定を促進する仕組み](#正しい意思決定を促進する仕組み)
    - [プラットフォーム・アプリ連携としての論点（未確定）](#プラットフォームアプリ連携としての論点未確定)
...
- データ公開したくない問題
- 正しい事実に気づけない問題
- 同じ場所内の多様性がなくなり、イノベーションが起こらない問題
- 成果報酬の懸念点
- 汎用的な法律の懸念点
- 幸福度の測定の問題点
...
3. **成果報酬の懸念点**
   1. 無料主義では基本的には成果報酬になってしまう
   2. しかし成果報酬になった場合は、生活を安定させるために、もし報酬が少なかったときのための消費を抑えてしまうかもしれないという問題がある
      1. 将来の予測が立てられず、消費が抑制されてしまう問題
   3. 自分の仕事の成果が正しく反映されていないと感じていれば、従業員の方が仕事に意義を見出すことが少なくなってしまう問題がある
   4. 結果が出るのに、時間がかかる仕事がある場合は、そのような仕事は不人気になるかもしれない
   5. 効率賃金が払えない問題がある
      1. 無料主義では、労働市場のその人の市場価値よりも高く、モチベーションも高められる効率賃金を払うことをできなくなる問題がある
   6. 無料主義の成果報酬の下限を設けなければ、賃金は少ないから働く人が減ってしまうのではないか
      1. これだけしか貢献のポイントが得られないならしなくても良いや、と思ってしまい仕事をしない人が出てくる
   7. 社会集団において成果や目標が指数化されたり測定されると、すぐにハッキングされ、成果や目標が適切でなくなる
...
5. **幸福度の測定の問題点**
   1. 幸福度のみを測定したら、毎日得られているから得られる当然だと感じているけどとても重要な商品はサービスを提供する人が正当な対価が得られない問題が出てくる
      1. 失ったときの幸福度への悪影響度合いを、幸福度の貢献度合いを評価するための要素としても使うことで、得ることに慣れてしまって、提供したとしても幸福度に貢献していないと判定されることがなくなるようにするか
         1. 幸福度を測る指標の要素に入れたい
   2. 水は得られるのは当然だと思い、幸福度の寄与率は下がるけどなくなれば悲惨な社会になる
      1. だから、その人の幸福度を維持するために必要不可欠かどうかを測って、その指標が高ければ幸福度を高めるのに寄与していると判断する
      2. 幸福度の算出のために、幸福度を測る指標の中に、「必要不可欠性」を入れる

6. **評価軸に貢献しないけど、人間が欲しがるサービスの対処法**
   1. 評価軸の 1 つが「幸福度の上昇」の場合は、提供しないほうが幸福度が高まるために提供しないような商品やサービスもある。その場合、ユーザーが欲しがるけど幸福度が下がる Instagram のようなサービスは、Instagram 運営者はほとんど貢献度はマイナス評価で常に赤字かもしれない
      1. それでもサービスを続けてもらうためには、自分がそのようなサービスを評価する評価軸の仕組みに移動するか、マイナス評価だけど支えるために自分が代理購入して開発者の生活を支える
   2. 人間が望んだものを手に入れられるからといって幸福度が高くなるとは限らず、人間が全く欲しいとは思わないものでも、幸福度が高くなるものはいくらでも存在するから、そういった面にも配慮できる指標、評価軸も作る
      1. タバコやお酒はみんなが欲しがるけど体に悪い
      2. サラダが 8 割の生活は健康に良いけど、みんなは欲しがらない
   3. 長期的に幸福度を高める行動だとわかっているのに実行しない人がいるみたいに、幸福度を高める行動がその人の取りたい行動だとは限らず、幸福度の向上につながる行動ばかりを促されていても少しずつ不満が溜まっていくかもしれない
7. **解雇できない・仕事を与えられない問題**
   1. 解雇されることにより幸福度が下がるので解雇できない問題
   2. 仕事を与えたら、その仕事が辛くて幸福度が下がって、上司の貢献度が下がる問題
      1. 部下の幸福度が下がるよりも、仕事でサービス提供によって報酬が増えるほうが上回るので、仕事を与え続けるかもしれない
      2. マイナスを大きく重み付けする評価軸の仕組みも出てくると思う
8. **SNS を見ると比較癖で不幸になるので、インフルエンサーたちは評価軸ポイントが与えられなくなるかも**
9. 不幸にならない人にしか見せられない仕組みで対応するしかない。その仕組みがない SNS を利用すれば KOL はマイナス評価で報酬が得られない
   1. 1 回目は不幸になるかどうかをお試しで全員に見せて、不幸になったとしても評価軸ポイントを下げずに、次からは不幸になれなかった人のみに表示するか
   2. 無理やりにでも他の人たちに表示させているインフルエンサーに対しては評価軸ポイントを下げていく
10. **幸福度向上のジレンマ**
11. 人間は幸福になろうと目指すことで逆に不幸になる性質があるから、評価軸で幸福度の向上をかけたら、みんなが幸福になろうとすることで、逆に不幸になってしまうのではないか
12. **生産性や科学技術の進歩の向上が止まるかも**
13. 幸福度の向上を評価軸に入れたら、経済成長が必ずしも幸福度の向上をもたらすわけでは無いから、技術革新の発生や生産性の向上が無くなってしまうかもしれない
14. **評価軸の決定時の懸念点**
15. 評価軸を直接民主主義で決める場合は、間違った指標で測定することに決定してしまえば、間違ったインセンティブ設計になってしまう
    1. すぐに他の評価軸の仕組みに移動できるようにしたり、馬鹿な意思決定を選ぶことがなくなるような仕組みが必要
16. 貧困の測り方、幸福度の向上の測り方、その他のいろいろな評価軸を測るときの指標の計算方法はいくらでもある
17. **測定が難しい人の幸福度をどのようにして測ればよい？**
18. 赤ちゃん、子供、死人などの幸福度の測定はどうすれば良いか
19. それによって、暴力をしたり、教育的に良い行いをした場合の対処の方法を提供したい
20. **評価軸に貢献しないけど欲求的に求めてしまうサービス**
    1. 幸福度を高めないけど、ゲームや YouTube 動画を見たいと思うときはある。でも提供者は評価されない場合はどんな社会になるか
21. **パターナリズム**
    1. 世界中には、それが科学的に正しいことが証明され、他の人がその商品やサービスを使ってもらうためにお金も与えるのに、全くその商品やサービスを受け取ろうとしない人たちが多数存在する
       1. パターナリズムになるかもしれないけど、その人たちに半強制的に受けてもらうことで、逆にその人の長期的に見たときの幸福度を高められる仕組みを、そのような人たちも、科学的に正しい生活ができるように支援する
       2. でも、本当にそれで良いのか
22. **技術や労働者のリソースの問題**
    1. 受益者負担がなしでサービスが提供できるようになれば、より高齢者に対してサービスを提供することが評価軸ポイントを獲得するうえで重要になり、それによって科学技術の促進に人材のリソースが割かれないことで、科学技術が進歩しない可能性が出てきてしまう
       1. だから、高齢者には少し厳しくて、高齢者に対する治療はほとんど報酬がない評価軸の仕組みも出てくるのかもしれない
          1. 過度な延命治療を行わない、など
23. **広告収益ビジネスモデルができない懸念点**
    1. 広告という無料で受けられる仕組みで受益者負担ではなくなる仕組みが資本主義にあるけど、無料主義では、そのビジネスモデルを使えるか
       1. 広告を見る代わりに、一定額の代理購入をしてくれる仕組みにしても良いけど、広告が表示されることによって幸福度の向上がなければマイナス評価になり、その広告を出している人の評価軸ポイントが減るから、結局広告を表示させない可能性が出てくる
          1. その場合に、広告という受益者負担ではなくなることができるビジネスモデルを使えないから、逆にサービスを提供できる人数が減ってしまうのではないか
24. **無料だけど良いことに報酬を与えるデメリット**
    1. 無料だけど良いことに報酬を与えるけど、思ったより報酬が少ないから行わない問題が発生する
       1. 今までは善意がインセンティブになったけど、報酬がインセンティブになれば、仕事内容に見合った報酬だとみなされないようになる問題
    2. Wikipedia の記事を編集したり加筆することが評価軸ポイントを稼げる行動になったとしても、評価軸ポイントを稼げるようになってしまったことで、思ったよりも報酬が少ないから逆に無報酬の現在よりも編集してくれる人の質や量が下がってしまう問題がある
    3. そうなったとしても別に良いのではないか
25. **人数制限がその瞬間に難しい場合はどうすれば良い？**
    1. 高速道路の料金所で、ポイントを預けた人から優先的に提供するのは無理ではないか
       1. オークションで人数制限せず、自由に供給側が販売方法を決められるようにしても良い
       2. 社会厚生を損なう販売方法を行った分だけ供給側をマイナス評価にすれば良い。でもサービス提供の報酬がマイナス評価分を上回るからビジネスは続けられる。それを無料主義バージョンに変えて、本来はオークションなどの合理的な販売方法によって提供すれば得られた評価軸への貢献度合いよりもどれだけ乖離しているかを算出し、その乖離している度合いだけマイナス評価を与えるか
26. **倒産することがないので、効率性を高めるインセンティブが少なくなる**
    1. 倒産はしないけど、会社運営をすることで社会のリソースを奪っていることで小さくマイナス評価を受けているから、それを上回る評価軸ポイントを得ていなければ廃業したほうが良いとなる
    2. 現在も、給料はどれだけ低くても良いから倒産して欲しくないと考える労働者がたくさん存在することによって生産性が下がっているから、生産性を高めながら倒産することによる問題を解決できる仕組みを作る
27. **グッドハートの法則**
    1. 「グッドハートの法則とは『成果を測るための尺度が目標になってしまうと、成果測定自体が役に立たなくなる』という現象のこと」
    2. [https://gigazine.net/news/20230101-overfitting-machine-learning-goodharts-law/](https://gigazine.net/news/20230101-overfitting-machine-learning-goodharts-law/)
28. **定量的な評価の公平性の問題点**
    1. 定量評価が反発を受けることを念頭に置いたうえで、無料主義の貢献度を算出する必要がある
       1. 人間は定量的な評価されるのに敏感でストレスを感じる
       2. さらに、公平性も必要
29. **評価軸の仕組みが細分化しすぎないか、収束しすぎないか**
    1. 評価軸の仕組みが多くなりすぎたら非効率になるし、評価軸の仕組みが結局数種類しか登場できなかったら、それぞれの思想を選べないからよくない
       1. ちょうど良い数に収束するような仕組みを作りたい
30. **「嫌なら移動すれば良い」という考えでは改善されないかも**
    1. 発言をして改善するように求めずに、嫌なら移動するだけの社会になれば、建設的な批判を受けることができないから、全く改善されないのではないか
31. **経済的インセンティブのみの問題点**
    1. 豊かになり、評価軸ポイントを稼がなくても、あらゆる商品やサービスが得られるようになったら、評価軸ポイントが稼げるインセンティブによって、愚かな行動やオープンソースにするメリットを作っていたのに、経済的なインセンティブが不要になってしまったら、無料主義の前提が崩れる可能性がある
       1. 褒美で釣るのではなく、罰で避けたいと思わせてインセンティブを作るしかない
32. **少数の富裕層の顧客**
    1. 今までは少数の人が多くのお金を払ってくれるから成立していたビジネスが成立しなくなる
33. **相手が所属している評価軸の仕組みを知ろうとする懸念**
    1. 相手の所属している評価軸の仕組みを知っておかないと、その評価軸の仕組み上提供できない商品もあるので、その抜け道を利用して提供する相手の所属している評価軸の仕組みを知ろうとして、それにより差別が生まれるかもしれない
       1. それを防ぐような無料主義アプリの仕組みを作る
    2. 解決方法
       1. 相手のデータを分析して好みを分析すれば、それと相関している評価軸の仕組みは直ぐに見つけられちゃうのではないか
       2. 提供する相手は、たとえ評価軸の仕組みが見つけられたとしても、枠を提示すれば優先的に強制的に与える必要があるから、販売する相手を変えられるようなことは無い
       3. 相手の所属している評価軸の仕組みを知られる前提で、「所属している評価軸の仕組み関連の差別を禁止」と言う評価軸を無料主義アプリに設定してもらうしかないか
34. **共産主義と同じになってしまう可能性がある懸念点**
    1. 無料主義では、全員に提供するインセンティブが働くので、全てが無料に近づいていき、共産主義で全員の報酬が一律同額になる状態と同じになり、労働が馬鹿らしくなる人が増えてしまう問題点があるかもしれない
35. **クレーマーが増える問題**
    1. 今までは対価を払えなかった人たちもサービスを受けられるようになることで、民度の低い消費者からクレームが来る
       1. クレームを言うことで、他の客の迷惑になったり、店員の幸福度も下がるし、クレーマーは評価軸の仕組みを追い出されるのでクレームは増えないかもしれない
36. **指摘するだけでマイナス評価になる**
    1. 怒らず指摘をして優しく指導をするだけで一時的に落ち込んでしまい、教師や上司がマイナス評価になる問題
       1. 長期的には指導をすることにより、指導した人が報酬の一部を受け取れることにより、大きくプラスになるから問題ないかもしれない
37. **バイアス増強装置**
    1. 思想が別々の人は別々の評価軸の仕組みに属することが多くなり、自動的なバイアス増強装置・プロパガンダ装置みたいな仕組みになってしまう可能性がある
    2. 他の評価軸の仕組みに干渉せずに、その人たちの幸福度が高いなら、それでも良いかもしれない
       1. 陰謀論にはまっていない人に悪影響を与えないで人生を生きるなら、陰謀論にはまっている人は幸福度が高いなら、それでも良いと思う
38. **解雇できない問題が出てくるかも**
    1. 解雇することによって解雇された人の幸福度が下がるので、解雇できない企業が出てくるか
       1. 解雇しないと効率化できず、会社に残っている人が悪影響を被るから、解雇を受け入れない人がマイナス評価になることで解決できるのではないか
39. **暴力でマイナス評価を受けたら、より暴力を振う問題**
    1. 親の暴力や両親が喧嘩をしているなどの原因で、子供の幸福度が低くなり、親がマイナス評価になって親に対して罰が与えられるけど、それによって機嫌が悪くなった親がより子供に対して攻撃的になるかもなので、結局子供を救うことができない
       1. 親が教育に悪い行動をして罰を受けても、行動を改めることがない問題を改善したい
       2. 行動をあらためてもらうために罰を与えても、より過激な行動をしてしまう問題の解決策を作りたい
40. **値段という信用の基準が無くなる**
    1. 値段が高いと信用できる印になっていたけど、無料主義では、全ての値段が無料になることで、どれが価値があるかわかりにくい
       1. 今でも値段関係なくレビューで見るのだから変わらない
41. **幸福のジレンマを防ぐ仕組みを作りたい**
    1. 評価軸である幸福度の向上を達成させようとして、幸福を得ようと頑張ったのにどんどん減っていくジレンマに陥りそうだから、それを防ぐ仕組みを作りたい
       1. 幸福になろうとするほど不幸になっていく現象がある
42. **無名の会社がより大変になる？**
    1. 今まではお金さえもらえれば良いと思って参加する人がいてくれたけど、無料主義では成果報酬なので無名の会社（プロジェクト）に対しては、参加者が集まりにくくなる。今も同じ感じだけど、無料主義ではよりその傾向が強くなるか。そんな流れで、無名の会社が大変になって、より大きな会社が有利になっては意味がない
43. **情報を教えるとマイナス評価を受ける**
    1. 本当はその人のためになるのに、知らされた人が使うべきではない言葉なので修正してもらったほうが良かったと感じることによって、情報を知らせた人がマイナス評価を受ける問題
       1. それを防ぐ仕組みを作りたい
       2. 被害を救ったと仮定して評価できるような仕組みが必要か
       3. もし知らせなかった場合に、知らせた情報によって発生する評価軸へのマイナス評価の分だけ報酬を与える
44. **無許可で利用可能の評価軸の仕組みに二次利用で悪用されて、ブランドの価値が下がってしまう問題**
    1. 「加工する社会」になることにより、ブランド毀損の問題が無くなるから大丈夫かもしれない
       1. ※無料主義では、付加価値をつけるメリットが無いので、付加価値をつけるためのブランドは無くなる
45. **オーダーメイド・イレギュラー競争になる懸念**
    1. 懸念点として、顧客の幸福度を高められるようにする商品は、オーダーメイドでイレギュラーを行うことがクライアントの幸福度を高めてしまうから、イレギュラーをする競争を行う問題が生まれてしまうかもしれない
       1. でも、オーダーメイドするより、より多くの人に提供する競争になるから、オーダーメイド競争にならないのではないか
          1. いかに自動化して、オーダーメイドができるか競争になる。結局資本主義と同じだから問題ないのではないか
       2. でも、消費者がイレギュラー要求することで、通常オペレーションの商品の供給が下がることで、他の消費者の供給に悪影響を与える結果になることで、イレギュラー要求の消費者はマイナス評価になるので、イレギュラー要求を行うインセンティブが下がるのではないか
          1. そのためには、「消費者のイレギュラー要求によって他の消費者の供給量が下がってしまう」という供給側の問題をオープンにする必要がある
46. **ピアツーピアな取引ではない懸念**
    - 貨幣はピアツーピアで取引できるが、無料主義では無料主義アプリを通して行う必要がある。
    - PayPayなどでもアプリを通じて行う必要があるから問題は無さそう
47. **嫌いなグループにノウハウ共有するとマイナス評価になり、ノウハウ共有のインセンティブがなくなる問題**
    1. 嫌いな相手にノウハウ共有することは気分を害することだから、自分（A さん）が所属する評価軸の仕組みの他メンバー（B さん）が、嫌いな相手にノウハウ共有すると（A さんの）気分が悪くなり（A さんの）人生の満足度を下げることとなるから、B さんの報酬が下がって、結果的にノウハウを嫌いな相手に共有するインセンティブが下がってしまうのではないか
       1. 共有して得られる報酬よりも、満足度を下げる悪影響の方が大きくなってしまい、報酬がマイナスになってしまう可能性を防ぐ仕組みが必要
48. **汎用的な法律の懸念点**
    1. マイナス評価になるのは後から判明すること。事前に分からないから萎縮して何も行えなくなってしまう
       1. そのため、行った時点で、その行為が違法レベルの一定基準以上がマイナスの場合は、違法にする
       2. つまり、行った時点で、マイナス評価が出ているかどうかで判断する
    2. まだ、その事例が登場していない場合は、現在の類似の事例と後からの評価を掛け算して計算するか
       1. 後から評価を受ける代わりに、競合がいないから抜け駆けできるというメリットも用意するか
    3. 上記の場合は、前例主義で、過去にマイナス評価と判断されたら、それ以降の、その行為を行ったら常に違法になってしまう
       1. 過去はマイナスの影響だけど、現在は良い貢献をする場合の行為が、過去のマイナス評価により違法となってしまう問題を解決する仕組みを作りたい
       2. 合法にする仕組みを作る。定期的にマイナス評価かどうか確認して、マイナス評価では無くなった時点で結果を更新したら良い

49. 相互運用性を高めず、独占する問題
    1. それぞれが共有しあって商品やサービスを作って限界まで需要を満たすことができたら、今までは協力しあっていたところを、今度は自社のサービスとして全てを開発して囲い込むインセンティブが生まれてしまうこと
    2. 今まで他社に提供してもらって機能していたサービスを自社開発して囲い込むように開発することで、無駄な開発が生まれてしまう。それを防ぐために、「代替性の仕組み」によって、すでにあるような同じようなサービスを開発したら同じ業界の人たち全体が評価軸ポイントが得られないようにする
    3. でも、ノウハウを公開したほうが、参入障壁を高めて独占するよりも稼げるようにすることで、次々と新しい機能や技術を開発するインセンティブを高めている。ノウハウを公開しなければ一社で独占するよりも報酬が低くなるのでノウハウを公開するし、でもノウハウを公開した状態で現状維持では、他社が自社のサービスに参入してきて「代替性の仕組み」で獲得できる報酬が少なくなるから、常に新しい技術を開発して、共有する必要が出てくる。その仕組みで次々と新しい技術を作るインセンティブにしている

50. 全体のために動いても最適化される？
    1. 他の人を考えず、自分の利益のためだけに動くから経済や市場が最適化されるのであって、全体の利益のために、自分が動いたら、自分が利益を得られるようにしてしまえば、市場や経済が最適化されないのではないか
    2. でも自分の利益を最大化するためには、全体のために動かないといけないから、全体のために動いたら自分の利益のために動くのと同じ効果があるのではないか

51. **供給量は価格ではなく貢献度で評価される**
52. 資本主義では価格を見て供給側が供給量を考えていたのを、無料主義では供給側はどれだけ評価軸に貢献できるかどうかを見て供給量を考える社会になる
    1. 貢献度合いを予測市場で予測して、その予測に従って供給するようになるのかもしれない
53. 全く貢献度合いを予測できずに、供給管理がめちゃくちゃになり悲惨な社会になるか
54. **オークションにするだけで嫌悪感を感じる人がいる**
    1. 予約困難店の席のオークションサイトやアニメのグッズをオークションにするだけで嫌われることが多いけど仕組みとして強制的にオークションにするので、そんな嫌われるような問題がなくなるようにする
55. **ゼロ価格効果で消費を喚起させることによるデメリット**
    - 無料主義で完全な無料になることで、現在の医療の分野では、不適切な治療や無駄な治療が行われることが多くなる問題が起きているから、それが多発してしまうのではないか
    - 引用
...
56. **無料主義は裸の王様が多くなるかも**
    1. 「干渉しない社会」によって、批判が表示されにくいため

57. **時代によって幸福度が下がる基準が変わる懸念**
    1. 現在の国の暴力装置の仕組みでは、行った行為の内容によって暴力装置の与え方や内容が変わるけど、無料主義では、評価軸への悪影響を与えた度合いによって、暴力、掃除の後言うことや内容が変わるから、幸福度の向上が評価軸なら、時代が進めば進むほど、昔はそんな悪影響として捉えなかった行為によって幸福度が下がらないとしても、現在ではささいなことについて幸福度が増減してしまう場合に、たった少しの何の残虐性もない行為だけども、幸福度が下がることに対して思い、暴力装置や罰が与えられるようになってしまう
    2. 無料主義では、未来は些細なことで幸福度が変わるから、窃盗をするだけで死刑になるかもしれない
    3. 幸福度は、一定期間が経つと、幸せでも、幸福に慣れてしまい、ちょっとしたことで幸福度が増減する

58. **介護**
    1. 本当は介護する能力があるのに、介護しないと評価軸に悪影響を与えていると認識されるのではないか。そうならないようにする仕組みを作る必要がある。高齢者を優遇しない評価軸の仕組みに移動すれば、そのような判断が下されることがなくなるようにする、など

59. **コモンズ**
    - 無料主義で、どうやってコモンズを支援するか

60. **「常時預ける仕組み」の懸念**
    1. 「常時預ける仕組み」では、常時預けるのを避けるために指名販売になるのではないか
    2. 対処法として指名販売になっても、推定価格を算出して、その推定価格で判断する
    3. もし推定価格を下げたい場合は、その財産に関する価値を下げる不利な情報を出すインセンティブが高まるから、透明性の向上も作れるのではないか

61. **評価軸への貢献による報酬としての評価軸ポイント発行で交換比率で不利になる問題**
    1. 評価軸に貢献したら報酬が払われて、評価軸ポイントが発行されるなら、評価軸に貢献するほど互換性を持たせるときに不利になるのではないか。それを防ぐ方法を考えたい

62. **Harberger's tax**
    1. Harberger's tax は税率を決める部分で間違った税率を設定して効率性を失うかもしれないから NG かもしれない
...
- 懸念点
  - 商材ごとにSNSアカウントがあるわけでは無い。その場合にポイントを付与する方法

- 解決策
  - 貢献度は、その商材関連のSNSや何かしらのID指定でポイント付与する
  - SNSアカウントがない場合は、それを製造している企業のSNSアカウントIDでポイント付与する
...
- 懸念点
  1. 無料主義アプリで、国・街のIDを統合する方法

- 結論
  - その国・街が運営するSNSアカウント名 or ユーザーID
- 説明
  - 国・街でもXやFacebookのアカウントがある
  - グループのID指定方法は、個人と同じでSNSや何かしらの連携方法でOK
  - 公式の評価軸と紐づける方法は、OAuth外部アカウント連携後の初回アカウント設定 or 設定画面から連携
...
- 懸念点
  - 無料主義の基本は事後評価による成果報酬
  - そのため、事前にどれだけ報酬を受け取れるか不明で働く意欲が湧きづらく安心感を得られない問題がある。
  - なので、事前に報酬を保証する仕組みが欲しい
...
- 解決策
  ...
  - 「特定のタスクに週OO時間だけ従事すれば固定報酬を付与する」という契約を評価軸コミュニティ内で結んでもらう
  - 無料主義アプリ内の処理は何も変わらず、固定報酬の分だけCSVアップロードすればOK
...
- 目的
  1. OSSの「Contributor」一覧に名前を掲載されることで貢献の証明になり就職で有利になるのと同じように、保有ポイント額を示すことで信頼でき能力があることを示すような無料主義アプリの使用方法を作っていきたい。
  2. また、保有ポイントを示すことで、他者との関わりに使えるようにしたい

- **より細かい開示単位について**
  - 「評価軸ごとに保有を公開する／隠す」に加え、**履歴単位の非表示**（例：ポイント交換履歴・譲渡履歴・貢献度アップロード履歴）や**連携済み外部アカウントごとの表示制御**を設ける余地がある。自分の評価履歴や行動様式まで含めて他人に見られるのを避け、必要なときだけ協力関係にある相手だけに見せるといった運用を可能にしたい。
- **プロフィール全体を秘匿する選択**
  - 評価軸別の保有表示だけではなく、アカウント単位で**一覧検索にも出ず、固有URLへの直叩きでも存在を示さない**（例：未取得と同種の応答で秘匿する）ような**プロフィール全体の非公開**がありうる。利用動機として、相手側の公開情報から方針・思想が読み取れる文脈では、自分の側の情報を与えずに協力行為を辞退したい、といったケースへの配慮が挙げられる（思想が異なる参加者同士が同一アプリにいる状態での摩擦軽減）。
...
- 解決策
  ...
  - ポイントを一定期間預ける仕組みの場合は、ポイント額をどれだけ消費しても減らず、一度使用しても戻ってくる為、より短時間で大きなポイント額を得ないと、他者の方がポイント額が大きくなりオークションに負ける。
...
- 問題点
  - 無料主義のグループ（資本主義で言う会社）への貢献をしているか判断する評価軸（＝「グループの評価軸」）は作成したほうが良いのか？
- 結論
  1. 無料主義のグループ（資本主義で言う会社）への貢献をしているか判断する評価軸（＝「グループの評価軸」）を作成しても、しなくても良い。
     - だが、「グループの評価軸」は作成した方が管理しやすいし、評価の漏れが少なくなるメリットがある。
  2. 評価軸としては、「『経済圏の評価軸』のみ方式」を選んだほうが分配する必要が無くなる。
     - でも、グループのメンバーの貢献度を経済圏から直接評価してもらえない場合は、**「『グループの評価軸』と『経済圏の評価軸』を分ける方式」を採用する**
     - 「React発展」と「OSS全体の発展」があり、Reactへ貢献した場合は、評価軸の基本は直接的な貢献しか評価されない
     - なので、Reactへの貢献は直接的には「React発展」で評価され、Reactが「OSS全体の発展」に貢献する流れになる
- 説明
  - グループが他の評価軸Bに貢献した場合に、その評価軸Bのポイントを、グループに属するメンバーに配布する際、どれだけの比率で配布するかを決める指標として、「グループの評価軸」の貢献度で配布する比率を決められる。
...
- 評価軸の形として、以下の2つに分けられる
  - **「『グループの評価軸』と『経済圏の評価軸』を分ける方式」** と **「『経済圏の評価軸』のみ方式」**
  1. **『グループの評価軸』と『経済圏の評価軸』を分ける方式**
     - 一般的な個人の例
       - 一般的な個人は、会社に属し、その会社が経済圏に属する
       - ある個人(A)の属する会社(B)が経済圏(C)に貢献すれば、CからBに貢献した分だけポイントが与えられ、BからAに貢献した分の報酬が与えられる
       - CからAに報酬が与えられる時の使用する指標は、「会社への貢献度」×「会社の経済圏への貢献度」
     - デメリット
       - 経済圏のポイントを会社が受け取ったら、会社内の人で「経済圏の評価軸ポイント」を分配する必要が出てくる。
       - ポイントを分配する必要があると、関わる人をできるだけ少なくするインセンティブが働き、それで参加者が減って技術革新が遅くなる可能性がある。
  2. **『経済圏の評価軸』のみ方式**
     - メリット
       - 「**『グループの評価軸』と『経済圏の評価軸』を分ける方式**」のデメリットを解消できる。つまり、会社に一旦経済圏ポイントが渡らず、会社内で分配する必要が無い
     - **「会社と経済圏のグループを分ける方法」が必要とされたのは、分けることで貢献度を測る方法を書く業務や業界ごとに最適化された評価方法を適応できそうだから。**
...
- 説明
  - 無料主義を用いて、無政府・国家が不要な社会を実現できる仕組み
  - 政府を今よりも簡単ですぐに移行可能で自由に選べる仕組みも作りたい
...
- 例
  ...
  3. 治安維持
     - 問題点
       - 逮捕件数をKPIにすると冤罪が増え、発生件数の減少をKPIにすると隠すことが増える
     - 解決策
       - ？
  4. バグのないシステム
     - 問題点
...
- 懸念の理由
  1.  ポイントを譲渡可能にすることで、無料で提供する仕組みが崩れる
      ...
      - 逆に、「誰かの費用が供給者の報酬になる仕組み」以外に「評価軸からの評価が報酬になる仕組み」もあるので、その仕組みを使って抜け駆けして無料で提供する人が増えるので問題ない可能性もある？
        - また、抜け駆けできるFediverse的な設計を推進する仕組みもあるので抜け駆けして無料提供がしやすいので譲渡可能で問題なさそう
  ...
  1. 無料主義の仕様では、できる限り無料主義を成立させるための条件を事前に増やしたくない。
     - 評価軸ごとに色々な意思決定や競争をすることで、より良い仕組みを模索できるようにしたいので、事前には選択肢を少なくする設計にはしない
       - もし、貨幣的な役割を防がないと成立しない設計なら、個人の利益としては貨幣的に使用した場合にメリットがあるので、絶対に通貨的な役割を持たせるために悪事を働く人が出てくる
         - なので、初めから自由に選べる仕様にして、評価軸毎の戦略で競争してもらう
...
- ポイントを譲渡可能にするメリット・通貨的な役割を持たせるメリット
  1. ポイントの互換性を持たせることが簡単になる
     - ①ポイントを為替のように他と交換できるようになる
     - 譲渡不可にした場合でも、②一つの作業に複数の評価軸が評価することで、複数のポイントを得られるようにして、一種類の評価軸ポイントしか得られない問題を回避している。
     - 譲渡可能にすることで、↑の方法以外にも、一つのタスクで複数の評価軸ポイントを得られる手段を増やすことができる
     - ①は交換で、②は２倍になる方法なので、両方とも必要
       - 評価軸A、Bがあって、交換比率が1:1の場合に、
         - ①は、AとBに交換するだけ
         - ②は、AとBのそれぞれで評価してもらえば、両方とも得られる
...
- 結論
  ...
  2. 一旦は、無料主義アプリv2では、すべての作業をデータベースで保存しない
...
- 指標ハックの例
  - 「OSS業界の発展」で、コミットした行数で評価する場合に、冗長なコードにするハックが出る

- 解決策
  - OSSのコミット評価で、行数を入れる場合のハック防止策
    1. 行数スコアは「逓減」させる（増やすほど旨味が減る）
       - `Base = 10 × log(1 + nLOC)`（または `√nLOC`）にして、100→200行の増加より、10→110行の増加の方が伸びない形にします。
       - これだけで「無意味に+1000行」が“割に合わない”方向に寄ります。
    2. 他の指標と組み合わせる
    3. 「事後精算（30日留保）」でハックの期待値を潰す
       - スコアを 2段階払いにします。
         - マージ時：Scoreの 50%だけ即時付与
         - 30日後：残り50%を、下記を満たせば支払い
           - リバート無し
           - 重大バグ（P0/P1）として紐づくIssueが出ていない
           - “変更の安定性”が一定以上
         - スループットだけでなく安定性も同時に見る
       - 評価してすぐは評価結果の50%のみポイント付与して、その一定期間後も、コードが残っている場合は、残りの50%も付与する
       - そうすることで、のちにリファクタリングされるような「冗長にして通す → 後で壊れる/直される」が、きっちり損になります。
       - 直後にやり直し（Churnで二重取り）
         - **Churn Penalty**：マージ後30/90日以内に同じ領域が再変更されるほど減点
    ...
    4. 「数える行」を固定して、無意味な増量を無効化
       ...
       - ここでの狙い：冗長化の王道「コメント増やし」「整形だけ大量」「コピペで水増し」をスコア上ほぼ無価値にする。
    5. 得点の本体：品質係数Q（レビュー・テスト・保守性・影響）
       - 最終スコアをこうします：**Score = Base × Q − Penalty**
       - Qは 0〜1.5 程度で、例として：
       - **レビュー係数（R）**：独立レビューの強さ
         - 2人以上承認で 1.1、1人で 1.0、自己マージ/実質ノーレビューは 0.5 など
       ...
       - **保守性係数（M）**：複雑性・重複・静的解析
         ...
         - 重複検出（コピー増殖）をペナルティ
       - **影響係数（I）**：Issue/PRの“意味”を担保
         - 例：必ずIssue紐付け、優先度ラベル、メンテナが「価値あり」判定で最低1.0、そうでなければ0.7
         - （ここを入れないと“動かない改善”が増えがち）
       - ここまでの狙い：「行数を増やす」だけではQが上がらず、むしろ下がる状況を作る。
    6. PR分割で稼ぐ（小分けスパム）を防ぐ
       - **Issue単位で週次上限**（同一Issueは合算してBaseを計算）
       - 「依存PRチェーン」は最後にまとめて評価（またはBaseを連結計算）
  - アンケート回答によるハックの対策
    - 正直に回答するインセンティブを作るゲーム理論のPeer-predictionの研究を参考にする。
...
2. **良い行動をしてポイントを得られるため、商材の販売による批判は少なくなり、購入できる**
   ...
   - お金は不正や不公平感のあるお金の稼ぎ方をしている人がいるから印象が悪い。
     ...
     - なので、お金で購入できる状態にすることが倫理的にNGの場合があった。
...
- 問題点
  - 商材ごとにSNSアカウントがあるわけでは無い。
  - その場合では、その商材関連のSNSや何かしらのID指定でポイント付与する
  - それ関連の商材や様々なID指定になるため、アカウントで外部連携を様々なサービスで行う必要がある問題点がある
  - 絶対に連携していないことにより、得られないポイントが出てくることになる

- 解決策
  - IDを固定にするためにできることは何だろう？
...
### 「非干渉の仕組み」により、裸の王様になるのでは？

- 問題点
  - 「非干渉の仕組み」により、裸の王様になるのでは？
  - 周りに指摘する人がいなくなるため、ひたすらに過剰な思想になっていく懸念がある

### 「報酬なし」から「報酬あり」に転換すると、実行者がいなくなる事象

- 問題点
  - 「内発的動機づけ」を「外発的動機づけ」に変えてしまう問題
  - 「ボランティア・報酬なし」だからこそ自己肯定感UPなどを目的に行っていたことが、報酬をもらえると行わないようになる事象がある。
- 例
  1. ボランティアで無償で働き、貢献している感覚を得られるから無償労働する
  2. 1990年代のスイスの原子力の放射製廃棄物処理場で、保証金を払う場合は、受け入れ賛成数が減った
...
- 問題点
  - 現在の組織・個人が正しく合理的で最適な意思決定ができているとは考えられない
    - 平社員、部長、社長、株主、政治家、顧客、その他のあらゆる立場で 「間違った情報源」と「足りない情報量」と「自分を有利に働かせたい考え」によって全体最適から離れた意思決定をする
  - 全体の利益になる意思決定を行う支援ができる仕組みを作りたい
- 解決策
  - ？

### プラットフォーム・アプリ連携としての論点（未確定）

- **オークションの「載せどころ」**
  - 取引機能をハブ・アプリ側に長く集約し続けるか、外部サービスとのAPI連携へ寄せるかは製品世代ごとに再検討の余地がある。無料主義のアルゴリズム（シングルプライス等）と異なり、アプリ単体の構成はプラットフォーム戦略であり本質経済モデルとは切り離して判断しうる。

- **既存マーケットプレイスと評価軸ポイント**
  - 法定通貨中心の大手EC／オークション等で評価軸ポイントを消費し、残余をアプリ側で検証する、といった外部決済との接続を目指す場合、**読み取り専用の公開情報**とは別に、**残高変動や落札結果を自動で反映する書き込み系API**の設計や権限制御が必要になる（実装フェーズでの検討事項）。

- **「市場を作る」タイプと賞金タスク**
  - オークションではなく、時間経過により需要側が示す事前固定報酬が上昇する**市場**をアプリ側にまで実装しない場合でも、評価軸コミュニティ内でのルール運用やスプレッドシート等で類似インセンティブを再現できる。GitHub Issues 作成と連動して自動で「賞金タスク」を立ち上げる等のワークフローは、インフラを経済モデル側にだけ置いた例として検討の余地がある。

- **参照・教育用のアプリレイヤとの切り分け**
  - 経済モデルをコードで説くための製品では、インフラ課題（リアルタイム配信、通知センター、画像ストレージ等）を増やさない割り切りと、機能を本質経済アルゴリズムに寄せる方針が取りうる。これは理論本体の代替ではなく、普及・理解のために厚みを調整しているに過ぎない。

- **退会・アカウント削除と評価軸の運営責任**
  - アカウント削除時には当該者に紐づくデータの扱い（包括削除と匿名化残存の境界）や、評価軸の唯一管理者であった場合には**権限譲渡または評価軸のアーカイブ**を退会要件にする、など、コミュニティが宙に浮かない運用ガイドがプラットフォーム側に求められる。
