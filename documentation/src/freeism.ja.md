...
- 必要な理由
  1. 互換性を持たせる方法を色々無料主義ドキュメントv2で考えたけど、それぞれデメリットがあって、結局はすべての行動を、それぞれの評価軸を個別で分析したほうが良い結論になった
     - その場合に、同じ観点で同じ分析をする車輪の再開発が起こらないようにするために、無料主義ドキュメントv2の仕様であるグループ（色々な評価の視点を組み合わせてポイント発行）ではなく、一つの観点で評価して、それぞれのパッケージが組み合わせて使用する形に変更する
