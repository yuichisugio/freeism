...
9. **ゼロ価格効果の悪影響**
   - 説明
     ...
     - 現在の医療分野では、不適切な治療や無駄な治療が増え、トラブルは起きやすいです
   - 引用
...
- 懸念点
  ...
  - 支え手への正当な対価は返りにくく、設計上の歪みが生じやすいです

- 結論
  1. 失ったときの幸福度への悪影響度合いを、幸福度の貢献度として評価する
     - それにより、得ることに慣れて幸福度に貢献していないと判定されることがなくなるようにしたい
     - 水は手に入るのが当たり前だと考えがちで、幸福度への寄与率は下がりやすいものの、途切れれば社会は大きく傷む
...
- 説明
  - 「即決価格」とは、オークション等で出品者が設定する買い切り価格です
  - 買い手はその額以上を提示すれば、競り合わずにすぐ落札できます
  - 急いで手に入れたい場面で利用します
...
- 結論
  ...
  2. 出品がある場合は、出品するタスクを行っている＆登録しているのではなく、ただ出品登録しているだけ
     - 実施タスクを個別に無料主義アプリへ登録する義務は設けません
     - そもそも、実施済みタスク全体を自力で見つけ出し、アプリへ網羅的に載せ続けるだけの運用規模になりにくいです
...
- 必要条件
  1. 「貨幣的に利用する無料主義ポイント」と「目標貢献に基づき得られる無料主義ポイント」は同一種類とし、同じ商品へ充当できるようにします
...
- 懸念点
  - 無料主義ではすべての行為から貢献度を算出します。一方ですべての行為をデータベースへ保存することが現実的かは検討の余地があります
...
- 説明
  - 商品を得るためのポイントの扱い方は、設計の自由度は高めに保てます
...
- 扱い方の種類
  ...
  4. 供給者など他者に譲渡する
     ...
     - 「消費」とは、保有していたポイントが無料主義の経済圏から失われることです
...
- 「`オークション`のみ」と「`即決価格`のみ」を主軸とする評価軸が並立すると、両立関係の解釈があいまいになりやすいです
  - そのときは出品側および評価軸側の設定へ整理を依頼し、購入導線の主線などを読み書きできる状態にします（たとえば `オークション` と「即決のみ」の比率がほぼ半々に迫るときはどちらを主導線にするかまで必ず明示する）
...
- 結論
  - 評価者が正しい評価をしないと、供給者は他の評価軸に移ってしまう
    - その結果、自分の保有ポイントの価値が下がりやすくなるので、適切な評価につなげるインセンティブを設計できます
...
- 不満が出る原因
  1. 不正や不公平感を伴う稼ぎ方への不信により、貨幣自体への印象も悪化しやすいです
     - 貨幣以外では優先権を獲得しづらい設計に対して不公平認識が生まれやすく、その結果ひとしく不満を誘発しやすいです
  2. または、競争結果が能力との相関として捉えられる場面での嫉み
...
3. **優先的に得られる資金がある人への嫉みの行為がマイナス評価になり、批判が減る**
   - 「能力がある人ほど優先されやすい」状況への嫉みが、優先権などを批判し市場として成立させにくくする場面があります
   - その批判は経済活動を縮小させやすく、評価軸でもマイナス評価となりポイントが目減りします
   - その結果、同種の批判は起きにくくなり、優先権を商材として提供しやすくなります
...
- 懸念点
  - 商材ごとに SNS アカウントがそろっているわけではありません
  - そのときは、商材に紐づく SNS や任意の ID 指定によってポイント付与することが想定されます
  - 商材単位での ID の指定や外部連携が増えるほど、ひとつのアカウントで多数サービスとの外部連携を担う必要性が高まるという懸念があります
  - 連携していないことにより、得られないポイントが絶対に出てくる
  - ID を一定に保つうえで取りうる手段は何ですか
...
- 懸念点
  - 現在の組織・個人が、つねに合理的かつ最適な意思決定だけをするとは限りません
...
- 説明
  - 「企業」と「評価軸」は別物
  - 「企業」は活動単位、「評価軸」は評価単位です
  - 1 社を複数の評価軸が見る場合もあれば、1 つの評価軸が複数企業を対象にする場合もあります
...
1. 評価軸が貢献者アップロード等でポイントを付与するとき、貢献データ側の識別子は**各サービス上で当該人物を表すプロフィールURL**を原則とする
   - その理由
     1. 「OAuth のユーザーID」や「アカウント名の生文字列」の照合は、表記ゆれ・独自ドメインなどで一致し損ねやすいため
        - 「ユーザーID」または「ユーザー名」の一致のみで確認する際、OAuth を除いた連携は `bi-directional link` に依存します
        - 独自ドメインだとユーザー名が URL に含まれないことがあり、表記ゆれによる突合漏れも起きやすくなります
        - そこでできる限り突合漏れを避けるため、識別子は URL でそろえる方針にします
   - 補足
     - もしそのサービスのアカウントではない場合も、無料主義アカウントや連携できる

2. 無料主義アプリは、「貢献データの URL」とユーザーがプロフィールへ書き込んだ連携 URL とを同一の正規化ルールに合わせ、整形後の値どうしを比較する
...
2. URL の相互リンク
   ...
   - 実装者の視点：サービス単位での例外対応が少なく済み、拡張の余地も広いです
3. それ以外
...
1. 自分のアカウント証明
   ...
   - 証明ができるとたとえば次のような橋渡しもしやすくなります
     - Zenn 記事執筆の貢献で Zenn のアカウント名を指定したポイント付与がある
     - 一方、アプリ側は Google ログインのみで無料主義のアカウントを作った状態でも、対応する Zenn アカウント分の報酬を受け取れる
...
1. **OSS で提供したい**
   - 追随が早い領域なら、プロプライエタリで囲うより OSS で社会に広く波及させたほうがよいです
2. **戦略を伏せたまま同業と差を競うのは嫌**
   - メモ
     - 他社・他者と協力して作り上げたい
   - その理由
     1. 自分が負けると何も得られないのが嫌
...
- **ボタン一つで立国できる**
  - 国の立ち上げは無料主義アプリで宣言するだけで成立させます。コストを抑えて立国でき、外部の許可なく無料主義の国を設けられます
  - 許可が不要なので既得権益も生じにくく、すぐに立ち上げられるため既得権益化の動機も働きにくくなります
  - 犯罪手続きやネット上のルールは、適用地域に合わせて評価軸の仕組みへ差し替えます。別の仕組みにすり替えて犯罪を免れようとしても、現在地の評価軸が適用されるため、他者に害を与える都合の悪い行為は自動的に難しくなります
...
- **全てが何もできない状態でも自動的に救われる評価軸の仕組み。**
  - 資金が乏しく職も得にくく、支援の手続きすら取れない人を自動的に救済する
  - 例えば、
    - 自動的に見つけて家を与えてくれる
...
- **セーフティーネットを用意する**
  - 75 億人以上の全人類が、社会的評価や経済条件がどれほど厳しくても、尊厳のある生活（2022 年の日本で年間 360 万円程度の消費水準を目安）を営めるようになるまで、必要な商品やサービスが全人類に届くよう重み付けを強めます
  - 事業者が経済合理性に沿って最下層を支えられる状態を目指します
  - フードスタンプや生活保護などの支援制度を不要にしたい
    - 制度を単純化し、フードスタンプや生活保護で取りこぼす人を減らします
  - ステイタス性の強い限定品ばかりが目立ちます
  - 生活必需でも情報が届かず、真の需要は把握しづらい状況になります
    - 知識として届ける仕組みを用意します（例：尊厳ある生活に必要な品目の学習へ評価軸ポイントを付与する、保健知識の実践へ貢献度を付与する）
    - 幸福度向上と相関が高い品目のなかから、尊厳ある生活に必要な品をシステムが選びます
  - 経済面のセーフティーネットに加え、精神面の支援も無料主義に組み込みます
  - 経済面では生活保護やベーシックインカムに相当する底支えを、精神面では幸福度が閾値を下回った場合に無償で精神医療につなぐ導線を想定します
    - 幸福度が閾値を下回った場合に精神科医の貢献度が不当に上がらないよう、トリガーと報酬の設計を分けます
    - 経済・精神に続き、社会的評価の再建など、尊厳を守る別レイヤーのセーフティーネットも検討します。発想を広げると新しい防衛線が見えてきます
    - 「恋人・友人・親のセーフティーネット」として、交流が乏しい人へ AI の対話相手を、困難な養育環境の子どもへ AI の保護的ロールを割り当てる案があります。将来は人より汎用性の高い AI が主要な対話相手になるでしょう
...
- **ベーシックインサムの実装**
  ...
  - 多くの人はセーフティーネットがないと、目先の消費や単純作業に流れがちです。セーフティーネットがあると長期視点の投資や教育を選びやすくなり、生産性と全体利益が高まります。そのためユニバーサルベーシックインカムや特定の評価軸ポイントの付与を進めたいです
...
- ブロック経済をしているグループの評価軸は、無料主義アプリ側の評価対象から外す仕様にしたいです
...
- 説明
  - 評価に納得できなければ、別の評価軸へ切り替え、より多くのポイントで優先取得できるように変えるだけ
...
- 無料主義アプリの中に、この世に存在するあらゆるデータを集めたデータベースを作り、その場所に集められたデータを全て匿名化処理がされた状態で、許可なく誰でもデータを使うことができるようにしたい
  ...
  - 貢献度の算出ではサービス利用状況データが品質向上に効くので、無料主義では取得許可と報酬を結びつけます
  - 許可した人には貢献度ポイントを付与し、許可しない人には評価軸ポイントを付与しない設計にします
...
- 現在では、砂漠や島などに人を集めて街づくりを目指しても、インフラ構築コストが巨大で実現しづらいです <br> インフラ前提を薄め、建設負担を下げる技術やサービスで街づくりを容易にします <br> 関連技術を公開し、建国のハードルも下げます
  - 個別の事例
  - スターリンクでネット環境は作れる
  - 太陽光パネルのみで生活して電気を確保
...
- 具体的な方法
  - 他の人間と関わらない社会へ近づけます
    - 汎用的 AI を友達や恋人として育てます
    - 親より多様なデータから感情を読み取り、最良の助言と科学的な教育を届ける存在が、汎用的 AI の 3D CG アバターになります
    - 人は他者との関わりを苦痛に感じ、次第に AI を選ぶ社会へ移ります
    - 最終的には、それぞれの人が考える理想的な仮想世界で幸福度を最大化するため、良い出来事と悪い出来事の比率を最適化します <br> いまは想像しにくい技術で仮想世界に暮らします
    - 全体に有利でも自分には不利なルールに縛られず、仮想世界では自分に都合の良いルールだけに従って暮らせる社会を目指します
  - 他の人間と関わらない施策に加え、物理的にルールが要らない状態も作ります
    - ルールが必要になる行為そのものが生じない仕組みや、物理的に実行できない仕組みを整えます
- AI がすべてを自分の都合よく加工できるようになれば、表現規制などデジタル上の規制は不要になります <br> 他者と関わらないため、対人関係で生じる法律も近い将来いらなくなります
...
- 合理的に考えた結果、ルールは必要がないと考えられる社会が理想の社会
  ...
  - 何かをしたい人がいるけど、それを押さえつける必要があるからルールを使う
    - ルールの必要性が薄ければ、抑圧を排しつつ全体の利益へ資する社会が実現します
  - 合理的ではないルール不要論で、ルールがなくなった社会では、ただの悲惨な国になるだけ

- **無料主義の評価軸の仕組みによって、麻薬を取り締まる必要がなくなる**

- 麻薬は明示的に禁止せず、評価軸への貢献度で扱います
- 麻薬は負の外部性が大きいため、悪影響があると判断されれば制裁の対象になります
- 薬物そのものかどうかは問わず、他者への負の外部性だけが判明していれば罰則を科します
- 入手ルートの捜査は不要になります
  - 誰が誰へどう影響したかを追えば足り、個々の行為の詳細までは調べなくてよくなります <br> 評価軸への影響度だけを見れば済む社会を目指します
  - 行為内容を逐一見ず、データ上で気分の落ち込みと相関する行動や関係者がいれば、その相関が強い人物を調べられます

- **無料主義の仕組みによって、色々な業務を不要にしたい**

- 営業、人事、マーケティング
- いろいろな仕事を削減し、優秀な人材を科学技術の進歩に寄与する分野へ集めたいです

- **営業プロセスを不要へ近づけたい**

- 各プロジェクト向けに、効率化につながるクラウドや各種サービスを自動レコメンドします
- 推奨どおり運用すれば、社会全体の効率が上がります
- 採用しなければ、改善余地を残したままとみなして評価を下げます
- **「余力を残さず共有させる仕組み」と同型の「余力を残さず改善させる仕組み」を用意します**
- 同じ文脈でも電話や会議の機会を減らしたいです
  - 大企業の不要会議は 1 億ドル規模の損失に相当し、労働者の約 3 分の 1 は会議不参加でも問題なかったとの分析があります
...
- 方法
  ...
  - 「無料主義アプリの仕組み」では、戦争が起きれば無アの評価軸でマイナス評価となり、不利になります
...
- 無料主義アプリ非経由では意思決定者を定められず、各種サービスの権限も使えません <br> 権限は無料主義の手続きで選び出し、RWA 登録へ載せます <br> 武力侵攻でも権限を奪えず操作できないので、侵攻の便益が希薄になります

- **警察・労働基準監督などの機関に頼らず権利と法律を守る社会**

- 行政介入を前提にせず、権利と法律が保全される仕組みを目指します <br> 評価軸へマイナス影響を与える行為には制裁を科しつつ、冤罪を訴える側はデータ開示で争えます <br> それでも不足すれば、無料主義アプリ上の第三者調査へ移行します

- **犯罪者の扱い**

- 犯罪者は地域から追放し、他者が干渉しなくてよい状態に置きます<br>それでも犯罪者は無料主義の恩恵を受け続けます<br>物理的罰則は極力設けません
  - これまで暴力装置の話をしてきました<br>犯罪者と距離を置きたい人は、どの水準の犯罪者と離れたいかを選べます
  - どの評価軸コミュニティとも距離を置きたいかも同様に選べます<br>閾値を超えた犯罪者は設定者との干渉だけを断ちます<br>それ以外は通常どおり生活し、恩恵も受けます
  - 刑務所・死刑・長期留置といった物理罰に代え、最大の制裁は次の状態に限定します<br>一定期間、互いに関連付けた拒否設定が重なる相手とは関われません
    - 拒否条件は個人設定のほか、評価軸組織や無料主義アプリが代表設定するケースも用意します
      - 干渉不能そのものが最大の罰になります
...
- 相互運用性の高さ、オープンソース度、事業者の前向きさ（相互運用性と OSS 化への姿勢）、思想の極端さなどを測れる指標を作りたいです

#### プロジェクトの仕組みで実現したいこと

- **有限責任の仕組みを作りたい**

- 運営者以外のプロジェクトへ資金を貸した場合、借り先プロジェクトの不祥事で貸し手の評価軸ポイントが減らない仕組みを検討します

- **プロジェクトに参加**

- 評価軸の仕組みが異なる人同士でも、同じプロジェクトで協働できるようにしたいです
  - プロジェクト本体と無関係な法域が異なる人同士でも、同じルール群のもとで仕事をすれば十分です

#### 評価軸関連で実現したいこと

- **栄養価の高い食事を取って報酬を得る**

- 先進国・途上国を問わず、栄養価の高い食事は生産性と所得を押し上げます<br>それでも実際には取り組みません<br>予防のほうが有効なのに事後治療へ流れ、知識不足から先進国においても注射があれば治った気分になります
- そこで生産性向上を評価軸へ組み込み、栄養の高い食事だけで対価が生じるよう仕組みます<br>経済インセンティブだけでは動かない行動もあるため、金銭以外の報奨も併設します
...
- 下請けへ案件を回すだけでは実質貢献がなく、評価軸ポイントもほとんど得られません<br>その結果、多重下請けだけの連鎖は成立しにくいです
  - 参考関係を計測し、成果への影響度で配分すれば、単純な多重下請けは評価対象外にできます
...
- 無料主義では当選率を高めるため、需要を上回る過剰供給が生じます<br>この手の供給過剰を防ぐ仕組みを作りたいです
- 下記の手口で過剰供給を抑えます
  - 供給を増やしすぎると評価軸へ貢献しにくくなります
  - 過剰仕入れを狙っても、供給側は貢献度の高い利用者から優先配分するため、買い占めが進みにくくなります
  - 大量仕入れ後の消化工数を、いまもっとも評価軸ポイントを稼げる作業へ振り向けないと、相対的な獲得が遅れ、優先度も下がります
  - ポイントのロックが続くほど、そのぶん倉庫コストが重くなります
  - 勝手に廃棄すれば負の費用を課します
  - 倉庫圧迫と処理工数が、視認しやすいコストになります
  - 相対的な貢献保有量で不利にならないよう、無駄な買い込みと時間浪費を抑える設計です

- **サービス利用者が評価軸ポイントを稼げる**

- 利用者データは事業者の評価軸スコアを支えます <br> 当事者にも対価を返すべきです
- 「参考検知」でデータ活用が評価軸へ寄与するとき、提供者へ報酬の一部を配分します
- サービス利用で事業者がデータを得、匿名化した集合データを無料主義アプリへ公開すると追加の評価軸ポイントも発生します

- **貢献度合いで報酬を与える**

- 現在の資本主義では、ニューヨークのヘッジファンドマネージャーは同じ地域の 80,000 人の教師の給料を合計した以上の報酬を得ている
  - 無料主義で、そのような価値を過剰に評価されて高い報酬を得ているような仕事を次々となくすようにしていきたい
- 極端に高すぎる報酬は、多くの人がその職業に就こうとする傾向を生みます <br> しかしその職に就ける人数は限られるため、多くの人が無駄な時間を費やす問題が起きます
...
- 評価軸で公平性を高めることができる評価軸の仕組みを使う
  - 公平性を高める評価軸を設定し、公平性自体が対価の源泉になれば、効率化も同時に報酬化できます <br> 公平性と効率性を同じループで底上げする形に組み替えます
...
- 教育は、所属する貢献度評価軸や個人評価軸である「幸福度の向上」に寄与しなければ、教育者側のポイントは上がりません
- 数学専門の教師が、生徒の将来幸福度まで担う負荷が生じます
- 宿題ゼロなど短期的な幸福度より、長期的な評価軸ポイント獲得へ繋げるため一時的に厳しくする教師も出ます
- 短期的には幸福度が下がる局面もありますが、将来の貢献へ繋げた知識提供として教師は評価軸ポイントを得て、長期ではプラスへ振れます
...
- 全員が同じ目標へ向かえば、失敗個人の責任追及は薄まります <br> 追及より達成に集中できます
- 共通目標を評価軸へ落とし込み、全員がその軸へ貢献するなら、バラバラの基準が生む利害対立や揚げ足取りは起きにくくなります
...
- 評価軸コミュニティ間と行政主体間の競争で、申請主義をゼロへ近づけます

- **いじめの罰**

- 無料主義では、子どもが良い行いをすると対価が付き、いじめなど悪行があれば生涯にわたる評価軸ポイントへ強いマイナスが載る設計にします

- **それぞれの仕事の成果報酬事例**

- 教師や医師は免許取得後、基本を成果報酬へ寄せます <br> 免許は初回成果が観測しづらいためのフィルターです <br> 教師は生徒の評価軸への貢献で測ります <br>\* 医療・運転など他分野の免許も同型ルールへ揃えます
- 供給制限を続けるだけの活動は評価軸ポイントを下げ、既得権益化を防ぎます

- **使うかも指標**

- 幸福度、財・サービス獲得に要する労働時間の加重平均、無料主義当選率（応募者のうち獲得できた割合）、評価軸ポイント合計などを束ねたダッシュボードにします
  - 幸福度は GDP での常時加算ではなく、日次・週次・月次・四半期・半期・年次といったウィンドウごとに個人の合計または中央値を採るイメージです
...
- 下記の仕組みで評価する
  - 「必要不可欠性の仕組み」で、問題発生時にどれだけ求められる役割かを測ります
  - 予防は正の外部性として評価軸へ載せ、内部化の枠で点数化します <br> 予防治療が進めば他者へのリソース負担も下がり、正の効果が積み上がります
- 災害、噴火、予防医療

- **対立を解消**

- 経済の均衡を支える仕組みを揃え、同じ方向へ進められるようにしたいです
- 1 つの事例が需要と供給
  - 無料主義では価格を下げるほど対価が厚くなる設計にし、値上げインセンティブを弱めることで緊張を和らげます
...
- 人は幸福度最大化を意識的に追いかけておらず、知識があっても誤った戦略へ寄りがちです
  - 幸福度向上を評価軸へ入れれば、正しいルートを学び、評価フィードバックで矯正しやすくなります <br> 意識的な最大化へ誘導する仕掛けも載せます
  - 幸福度を上げようとして比較競争に沈み下がる逆効果には注意します
- 人は獲得幸福を過大に見積もり、現実との差で不幸感が膨らみます

- **個人の評価軸の達成する仕組み**

- 各人の評価軸に沿う財・サービスを自動で並べ替え、預け入れる評価軸ポイント量までシステムが提案する仕組みを目指します

- **無料主義で、汎用的な法律で特定個人の恣意性を排除に挑戦したい**

- 恣意的な個人裁量を排しつつ更新できるルールが組めれば、民主主義へ依存しなくて済みます
  - 排除しきれない局面では民主主義のほうがマシ、という逃げ道は残します
- 汎用法律イメージは次のとおりです <br> 評価軸へ悪影響を与える行為を犯罪扱いし、評価軸への貢献可否を中核ルールに据えます <br> 貢献度や好影響・悪影響の大きさでルールが動き、単一の骨格で多様な事象へ当てはめます

- **幸福度を高めるために不要な消費喚起を規制**

- 気づかないうちに幸福度が保たれやすい財へ、マーケティングで知りたくさせる喚起が乗る場合、事前に幸福度予測と獲得後実績のギャップを測ります <br> 乖離が大きいほど、余計な喚起として評価軸ポイントを削ります
- SNS 掲載などで購買意欲が煽られ、本不要な財まで買わせた場合も同様に喚起側を抑えます <br> 喚起規制や露出制御など具体策はケースごとに設計します
- 幸福度を高めるために不要な消費喚起まで行わせない仕組みを作る

- **購入する商品は「個人の評価軸」に照らし自動選択してほしい**

- 目標を「個人の評価軸」へ登録すれば、全世界の財・サービス候補から軸を最大化する組合せを探索します <br> 投入予算の限界までの効果を推定し、提示すべき評価軸ポイントと獲得ルートを自動生成します
  - 自己決定感を保つため、分野ごとに自動取得のオンオフを選べます <br> オンの分野だけシステムが入札と獲得まで面倒を見ます
  - 限定財の価格設計と提示額交渉もシステムが担い、参加者全員の手間を減らします
  - 見落としていた候補も全体カタログから抽出し、合理的な購買へ寄せます
- 個別提示では消費者情報が狭い一方、全商品マスタから個人評価軸達成へ貢献する候補を自動探索すれば、気づけなかった代替肢も表面化します

- **幸福度を評価軸にしたときの注意点**
  - 幸福度に直結しない語彙でも、意味が近ければ広く幸福度プロキシとして扱います
  - うつ病発症率や人生満足度といった指標は幸福度と同列で扱い、相関研究を横断集約します
...
- 幸福度そのものより、人生満足度のような長期指標で縛ったほうが、継続利用財の評価が破綻しにくいです
- 瞬間幸福度だけを見ると、使い続ける財の価値が過小評価されます <br> 慣れで快感が薄れる財こそ、人生満足度を定量追跡する必要があります
...
- 評価軸が幸福度の向上の場合は、報酬＝「幸福の指標で感情が 0 から幸福になった+の値の全員の合計」ー「幸福の指標で感情が 0 から幸福になった − の値の全員の合計」
  ...
  - 一部の評価軸では、都合の良い属性同士だけ得をするため、マイナス当事者を追放する仕組みが生まれ得ます
  - 他コミュニティの排他性へ波及し、軽いペナルティとして重み調整が入る局面もあるでしょう
  - 被害者を単に排除すると、その評価軸自体が低評価になるため長続きしません
  - 不幸側へ重みを寄せ過ぎると、すでに幸福な層から反発が出ます

#### 評価軸の仕組み関連

- **評価軸の仕組みのレコメンド**

- 評価軸コミュニティは組み合わせが爆発するため、個人へ自動レコメンドします
  - 居住地や価値観のタグを積み上げると数万通りになります <br> 死刑・安楽死・デザイナーベイビー・女性権利の線引き・表現規制・生活保護・セーフティネット・税制・インセンティブ設計・高齢者負担・教育補助など、論点ごとにオーダーメイドの枠を敷けます
  - 論点をトグル化し、賛否と度合いを保存します <br> 単一アカウントで全サービスへログインし、進化版ウォレットへも同期します <br> 価値観タグに応じて UI・推薦・設計を自動で切り替えます

- **「タグ付けの仕組み」でユーザーがルール設定**

- 大多数向けの基準ルールは事前にデフォルト化し、例外の映像は種別タグを付ければ削除の対象にしません
- デフォルトから外れるのにタグを付けない場合は、AI が検知して従来どおりの措置対象にできます
- 削除しない方針に寄せるなら、AI が自動タグ付けし、閲覧側でショック露出を防ぎます
- タグ付き投稿はプラットフォーマーが消せない前提にし、倫理観の差はタグとフィルタで吸収します

- **評価軸の仕組みを用途ごとに使い分ける**

- 評価軸コミュニティは複数へ同時参加でき、分野ごとに異なる貢献計算ルールのサービスを選べます <br> 単一ルールに縛られない前提で、サービス横断で好みの規制セットを持ちます
...
- メキシコの条件付き現金給付「プログレッサ」では、合意した行動と引き換えに現金を給付し、政府の望む行動を誘導しました
- 無料主義でも、妥当と見なす行動と貢献ポイントを突き合わせ、同型の誘導を行います
...
- ハードフォークでも同様に実験継続できる設計を許容します

- **他の評価軸の仕組みに迷惑を掛けない仕組み**

- 規制が手薄なコミュニティも許容しつつ、他コミュニティへの負荷を数値化します <br> 環境への垂れ流しなど外部不経済は、無ア貢献度をマイナスへ落とすトリガーにします <br>\* 悪影響の閾値は無料主義アプリの評価軸基準へ明文化します
- 単一アプリ群で統治しつつ、アプリ間協調プロトコルも別途定義します

- **データ取得の許可によって貢献度が高くなる**

- データ取得の許否はユーザーに委ねます <br> 拒否すれば推定精度と公平性が下がり、重み付け後の貢献度も薄くなります
  - 選択は自由ですが、保守的設定ほど対価も控えめになります

- **公平**

- 全てに同じ支援を与えるのではなく、家庭や遺伝子的に不遇の人たちに支援をすることで公平さを作る評価軸の仕組み
- 引用 <br> たとえば全員へ同量の支援と機会を与える公平と、不利な生い立ちの子どもへ個別支援を厚くする公平のどちらを尊ぶかという論点です <br> [https://honz.jp/articles/-/54112](https://honz.jp/articles/-/54112)
...
- 応募は容易で、欲しい品を宣言すると限定供給時の獲得ポイントが自動で重み付けされます
  - 幸福度を高めたと判定される条件に限ります（教育を全員一律で配るのが目的にならないよう、手段の目的化を避けます）
- 学びたいが資金のない人が宣言すれば、学べない人が多いほど指導側へ評価軸ポイントが厚く流れ、給与相当の貢献ポイントが自動で回ります

- 要望投稿型のプラットフォームを用意し、新サービス像や欲しい機能・デザインを募集します
- 投稿内容を改善したり実装した人へ貢献度を付与し、需要の可視化と称賛の分配を同時に進めます
- いいね数など簡易シグナルに比例して貢献ポイントも振ります
- 運営は需要に即した改善を素早く見つけ、アイデア提供者へ報酬を返しつつ利用者減に応じて投稿インセンティブを自動調整します

- **アイデアを投稿できる機能**

- 無料主義のプラットフォームではアイデア投稿欄を設け、自力では実現できない案を他者へ見せて共同実装できるようにします
- 上司任せで止まった題材も投稿しておけば、他者が実装した際に発案者へ評価軸ポイントが還流し、成果が報われるようにします

- **アイデアを出す人を最大化する仕組み**

- アイデアを出す人を最大化するうえで、出した側が実現側の評価軸ポイントの一定割合を得るかどうかを、無料主義アプリが自動調整します
- 出す人数が最大化するまで、割合を段階的に高めます
- 最大化に向けた手段は、次のとおりです
  - 出す側と実現する側の乖離が最大になるまで、出した側の受け取り割合を高めます
  - アイデアをオープンソースで公開する人が増えるようインセンティブを載せます
  - 実現側の人数を最大化するには、無料主義アプリが自動調整できるか検討します
  - 他人向けに公開しつつ自分でも実現できる前提にし、思いついたのに重み付けがなく投稿をためらう事態を減らします
  - 同趣旨のアイデアは先に投稿した側が有利になるよう設計し、投稿の速さへ動機を付けます
  - 匿名投稿を許し、批判コメントは自動非表示とします <br>\* 参考にされた匿名アイデアには、参照分に応じてシステムが評価軸ポイントを付与します
...
- 交渉を不要にしたい
  - 交渉なしで用地を得られるようにしたい
- 上司、親、妻、国、会社などの許可を不要
  - 子どもは親の許可なしで習い事へ通え、ほかの自由な行動も取りやすくします
  - 評価軸へ貢献するほど、子ども自身の自由の幅が広がるよう設計します
    - バカな親によって可能性を潰されることは少なくありません。一方、間違いが親によって正されることはほとんどないでしょう。親とは違う外の世界の方で間違いに気づく場面が多いです
...
思想が異なる人とは関わる必要はありませんし、干渉もされない社会の実現を目指します。
...
- 無料主義では、批判に応えるのではなく、批判する人が出ていかないといけない仕組みにする
  - それによって、的外れな批判で可能性が潰される事態を減らします

- **都合の良い表現の自由**

- 各評価軸コミュニティは、自分の思想に合わない表現を標準画面から外し、他コミュニティでは無言のままにします <br> 禁止対象はコミュニティ内だけで定め、本文データ自体はタグで保持します
  - 運用詳細は「タグ付けの仕組み」へ集中させます

- **タグ付けの仕組み**

- タグ付き投稿やファイルは原則削除せず、閲覧者はタグフィルタで非表示にできます
  - 運営が消さねばならない圧力と炎上リスクを下げ、非削除をデフォルトへ寄せます
- 嗜好が細分化すると交流コストが上がり、黙って情報を隠すほうが得になりがちです
- AI 友人などの補助もありますが、中核はタグによる視界制御です <br>\* 見たくないものだけ隔離し、少数派の発信はオンライン上に残します <br> 自分は見ない選択を尊重し、他者の閲覧権は奪いません
- 過激な政治や性的指向も消さず、タグ管理へ載せます
  - 全コンテンツへタグを付け、初期表示は穏当タグだけに限定し、詳細表示は自己責任で開きます
- 全面許可に見えると発信が萎縮するため、心理的安全性としての蓋を残しつつ内部閲覧導線は残します
  - 自分は見ないが他者の存在は容認する文化を維持します
- 実際は開放でも禁止に見える UI を設計し、評価軸ごとに「見せかけの禁止」を運用します
  - 企業が利益目的で非対称を作る典型と違い、ここではユーザが能動的に非対称を選びます <br> 画面から外すだけで実体は残し、詐欺のみ例外領域として遮断します
  - 基底データは共有し、差分ルールは見せ方のフォークで吸収します
- コミュニティ単位・個人単位で非表示タグを設定でき、コミュニティ側のデフォルトが個人より優先されます
- 目視レビューが要る非表示タグ付けには報酬を出し、アルゴリズムの隙間を人力で埋めます
...
- 子どもが逃げられる環境を整え、幸福度を検知して家庭内の様子を可視化します
- 毒親に似た振る舞いがあるかどうかを、短いサイクルで把握できるようにします
  - 子どもの幸福度が下がれば、親が獲得しうる評価軸ポイントも下がります
  - 一定水準を一定期間下回れば介入し、毒親からの脱出を支援します
...
- 精神面の安定を要件とする評価軸の仕組みが現れることもあります
- 第三者が問題行動の当事者へ注意した場面を想定します
- 幸福度の下落だけを根拠に、注意した側へペナルティが刺さる設計も現れ得ます
- 注意と結果の因果が逆転したように見え、評価軸上で逆風が吹く局面です
  - 加害側でありながら被害者意識へ寄せる人は参加を制限します
  - 同様の傾向を持つ層も対象外とする設計が現れ得ます

- **ノウハウを共有しようとせず、独占したい人がいた場合**

- 独占や差別、優位性の確保で幸福度を高めたい人もいます
- その行いが成立しているように世界を加工表示し、当事者と接点を作らなくて済む導線を用意します

- **自分と違う趣味嗜好の人とは関わろうとしないし、支え合おうとも、協力しようともしない問題**

- 宗教や民族、趣味嗜好を軸に協力を拒み、経済合理性より排除を選ぶ意思決定が散見されます
- 次の対応を組み合わせます
  - （1）所属している評価軸の仕組みは秘匿します
  - （2）協力に必要な情報以外は話さず、匿名での業務遂行を許容します
  - （3）無料主義のプロジェクトでは、相手プロフィールを合意しやすい形へ加工表示し、協業相手の表象を自動調整した状態で仕事を進めます
  - （4）関与を避けること自体は認めつつ、制裁や干渉へ踏み込んだ場合は無料主義アプリの評価軸へ逆行するとしてマイナス評価にします

- **「非干渉の権利」と「積極的な誤情報の受け入れ」**

- 誤情報を信じた生活でも社会が回る前提を形にします
- 情報を望む見え方へ加工する技術・社会・国家が拡散したとしても、制度が回る設計を目指します
- デマ由来の摩擦や、正しさへの修正苦から解放され、信じたい情報だけで生活するスタイルも許容します
  - その生活様式への需要は大きい一方、自身は現時点で控えめに留める選択もあります
- 「非干渉の権利」と「積極的な誤情報の受け入れ」の両立が進めば、メタバースなどで理想社会を別々に過ごしても全体は回り続けられます
  - 各人の現実認識は一致せず、同一テーマで会話しないままでも社会を維持できることが狙いです
...
- 仕事の場面まで含め、表示情報を加工して暮らせる社会を目指します
- 加工前提でも秩序が保てるルール設計を整えます
  - 顔や景色、ニュース、氏名、性別、音、匂い、味など、周囲の要素を好みへ合わせて過ごせます
- 見たくない対象は加工で避けます
- AR では見たくない対象を自動非表示にし、本人のフィルタへ委ねます
- デフォルトはルール最小とし、新たな取り決めが欲しければ本人の加工設定で試します
  - 他者への強要はしません
- 同じ行動を広げたい場合の誘導です
  - 同一評価軸の仕組みでは、新しい評価軸項目を足し、望ましい行動へ報酬を載せます
  - 望ましくない行動へは悪影響としてペナルティを載せます
  - 別の評価軸の仕組みへは、互換づけの際に評価対象外として扱い、志向の差は表面化させません
- 科学的妥当性を示しても他人の行動や考え方を強要しない前提を徹底します
  - 基本はルールなしへ寄せ、見たくない部分だけ本人が処理します
  - オプトアウト前提の社会です
...
- 啓発のみでは根絶は難しいため、人種が推測しづらい表示へ寄せる工夫も検討します
  - 心情としては重い一方、選択肢として残します
- 全面加工が進めば、過去の記録はすべて編集可能な資産へ近づきます
- 周囲が加工閲覧へ寄せるほど、加工せず実物を見せる態度は異質に映り得ます
  - マスク事情を抱える相手へ素顔提出を迫る同調圧力に似た摩擦も想定します
- 「見たいように加工する社会」という看板づくりです
- 嫌な内容は自動的に都合の良い情報へ滑らせ、価値観の違いごとに口論を減らします
  - 企業の提供物も加工で受け止めやすくし、なお嫌なら評価軸の仕組みで非表示へ回します
  - 供給側単独の善悪より、加工設定か非表示運用かが結果を分ける前提へ寄せます
- 他人の加工済み表示をなぞるのでなく、自分用の加工結果だけを見ます
- 無料主義では希少品でなくなるほど対価が乗る設計を厚くします
- 企業は見せ方より機能を薄く載せ、消費者側の加工や AI 最適化へ任せます
  - デザインに限らず、声のトーン、文体、匂いまで個人設定へ載せ替えます
- Instagram は投稿側の加工が中心でしたが、閲覧側が他人の顔写真を好みへ再加工する時代も想定します
  - 無料主義そのものと分離した話題でもあります
  - ブランド表現は最小限に留め、余白はユーザー加工へ渡し、提供コストを下げる方向です
- 事件の発言や動画は、被害者の心情へ触れ易い部類です
- その負担を下げるため AI 生成へ逃がす案も用意します
  - 閲覧が続く事実そのものが負担なら、誰も見ていないという表示へ加工して差し上げます
  - 条件は被害者が消えうる設計へ寄せることです
- 社会はよりリベラルへ傾き、合わなければ別の評価軸の仕組みへ移れます
  - これまで排していた嗜好も、徐々に共存へ寄せる圧が働きます
    - ヘイトスピーチ、LGBTQ、児童ポルノ、その他
- 誤情報を積極取得する社会です
  - 成立すれば認識差があっても摩擦が抑えられ、仮想居住でも回る国家モデルを試せます
- ヘイトなど見たくない主題は非表示へ回します
- 閲覧しても他害が無ければ履歴を残さず許容します
- 「誰かが見ている」事実が負担なら、誰も閲覧していない情報へ加工表示します
- 「自発的に誤情報を取りに行き受け入れる社会」です
  - 陰謀論やカルトへ傾いた層でも、他害が無ければ誤情報受容が広がる局面を許容します
  - 快楽志向の欲求と人生満足を両立させる設計を目指します
  - 望む情報だけを安定して得られる状態が目的です
  - 会話を極小化した分散運用が進めば、無料主義以外の領域でも実現へ近づきます
...
- 無料主義では各技術をオープンに保ち、特許型の独占を避けつつ報酬を用意します
- 参照や再利用のハードルを下げ、暗黙の囲い込みを弱めます
- 実装へ賛同しない層からの干渉を避けるため、新しい共同体の立ち上げを容易にし、反対側が手を伸ばしにくい枠組みを用意します

- **優秀な人材を科学技術の進歩に集める**

- いまはアクティブ運用ファンドの成績がパッシブより劣る一方で、金融業界の報酬は高止まりしています
- 優秀層が金融や長寿医療へ流れ、技術革新の現場が薄くなる構図も見えます
- 無料主義では技術革新へ人材が移るインセンティブを厚くする仕組みを目指します

- **オーファンテクノロジーを支援する**

- オーファンテクノロジー（市場規模が小さく利益が上がらないため開発が進まない技術）を、無料主義では支援したいです
- 科学技術特化の評価軸の仕組みを用意し、通常労働より厚い評価軸ポイントで基礎研究とオーファンテクノロジーを後押しします

- **途中公開**

- 論文は成果報告が中心になりがちですが、途中のアイデアも積極公開し、途中結果を評価へ載せたいです
  - それを無料主義で取り入れる
- 無料主義では途中結果を公開し、参照の追跡を自動化します
- 参照が検知できれば公開者へ評価軸ポイントと称賛を分配できます
- 従来は引用関係が曖昧で、評価も定量化しづらかった課題を埋めます
...
- 規制が薄い評価軸の仕組みで開発した薬を試し、所属を問わず結果を共有します
- 自社区画で試せない実験は、規制の薄い区画の知見として参照できるようにします
- 研究規制の壁を低くする狙いです
...
- 規制の薄い区画で試した結果を共有したい他区画は、その区画を評価する必要があります
- 評価を拒否すれば分配を受けられず、ゼロから研究を組み立てるしかありません
- 感情で否定的に切るより、理性で評価へ載せるインセンティブを厚くします

- **基礎研究を支援したい**

- 基礎研究を行っている人は問答無用で一定以上の地位とみなされるレベルの評価軸ポイントを提供する仕組みを作り、基礎研究を加速させる
- そのルール設定は科学技術促進へ寄せた評価軸の仕組みに載せます
- 規制が薄く、知性が集まるコミュニティーで運用すれば思想の整合も取りやすいです
...
- 一部の評価軸の仕組みでは、刑務所は不要になる
  - 現行区画から退去するだけでは足りません
  - 追放が他区画の評価軸へ外部性を浴びせるなら、互換ルールで不利が返る設計が要ります
  - 幸福度向上を掲げた区画が、幸福度の低い人を量産的に追い出す歪みも抑止します

- **国を跨いだ警察の機能**

- いまは警察が国内事件を主戦場とし、国境を跨いだ協力はほとんど確立されていません
- 無料主義では協力耐性そのものを設計し、相互評価へ載せやすくします
  - 協力しないと互換性を持たせる時に評価されない。無アの評価軸に貢献していないと見なす

- **人権侵害の定義**

- 国際人権法は無条件の人権侵害禁止へ強く傾きます
- 無料主義では評価軸への悪影響がある場合にだけ人権侵害と呼びたいです
- 悪影響が立証されたときに初めて人権侵害へ分類します
...
- 「周りに悪影響を与えない限りは自由であるべき」
  - ただし古典的人権観では、被害が見えにくいまま自由が削られる事例も残ります
- 人権侵害は無条件で悪いのではなく、人権侵害をされることによって、被害者の幸福度や、その被害者の人生の目的の達成を妨げるのが悪いだけ
  - 被害へ悪影響が無いなら、観念上の人権侵害ラベルは重く扱いません
...
- 身内優遇のルールメイクが露見したら、すぐ別評価軸の仕組みへ移れる導線を残し、捕まり続けないようにします

- **匿名の意思決定者**

- 意思決定者は匿名とし、人格票や経歴狩りの標的になりにくくします
- 投票は共有目標への貢献度で評価し、報酬を支払います
- プロジェクトごとに目標を定め、最も近づけられる能力を選考指標へ載せます
- 匿名のまま執行され、一定期間後は全体利得で報酬を確定します
- 賢い人を選ぶ選抜は、選挙だけに頼らずシステムと多層選抜へ載せます
- 人間による直接投票ではなく、自動選抜へ寄せます
...
- 事前に社会像を語らなくても、各評価軸の仕組みが目標を内蔵している前提で投票を始められます
- 採用基準は目標達成能力の測定へ寄せます
- 選抜の補足です
  - 貢献効率が高い人は評価軸ポイント合計も伸びやすく、意思決定権が自然に寄ります
  - リスク回避が進んでも意思決定枠へ高い報酬を載せ、非金銭インセンティブを厚くします
  - 金銭インセンティブだけでは動機が偏るため、名誉・守備範囲の設計も並べます
...
- 思想が政治に介入できないようにする
- 思想ごとに評価軸の仕組みを分け、思想は評価軸の中へ閉じ込めます
- 同思想が集まるため、反対思想の妨害が意思決定を止めにくくなります
- 設定した思想を最大化するだけで報酬が載る前提へ寄せます
...
- 非倫理的手法で得た技術を他区画が使いたい場合の会計です
- 参考側が得た評価軸ポイントから、非倫理度に応じた減点を先に引きます
  - 減点は悪影響の記録へ載せ、開発側の取り分も同時に削ります
- 倫理無視の開発コミュニティ B を利用する別区画では、倫理逸脱はマイナス評価のまま載せます
- ただし技術利用そのものは感情とは切り離し、貢献としてのプラスを積み上げます
  - 合算すると連続的な評価へ戻せます
- 現状は倫理違反が重いと成果をゼロにしがちですが、無料主義では技術価値と倫理減点を分けて扱えます
...
- 刑務所を故意に劣化させるのは誤りで、劣化が再犯低下に寄与すると判明した場合に限って許容されます
  - 被害者の負担を早く下げ、加害者は快適さと更生のバランスへ載せます
- 外部性は報酬関数へ内蔵し、更生が正の外部性として計上されるなら、運営は科学的に最適な復帰プランを選びます

- **加害者が特定できない場合でも。**

- 加害者特定が難しくても、被害は特定しやすいので、負の外部性を埋める補償から入ります
  - 騒音源が曖昧でも、集中を削る苦痛は観測できます
...
- 引用 <br>「他の参加者のうち何％が、その質問へ yes で答えると見積もるか？」という質問を追加する
- 引用（続き）<br> 正解を知る少数派が割合を低めに見積もる性質を利用して集合知を集約する
...
- 参考リンク: [Braintech ガイドブック PDF](https://brains.link/wp/wp-content/uploads/2022/10/Braintech-guidebook-ver1.1.pdf) ほか、[関連ニュースへのショートリンク](https://t.co/2IaBDNhQJX)

- **誰でもデータ分析できる方法の参考にしたい**

- AI と機械学習を身近にする DX プラットフォームです。ローコード・ノーコードで運用できる [Dataiku](https://www.dataiku.com/ja/)（米ニューヨーク州本社）は企業向けオールインワン製品です
- 「Everyday AI」を標榜し、専門家の知見を形式知へ落とし込みます。統計的背景が薄いユーザーでもグラフィカルな操作で分析できる点が強みです
...
- 例
  ...
  3. B はポイントを得る
  4. 換え先が乏しいと魅力は伝わりづらいです。そのため運用まで含めて A は段階的にルールを整えていきます
...
2. 格差による移行
   - 格差の拡大でサービスの購入層は薄くなります。そのなかでも利益を確保するには、無料主義で料金を払う余裕がない層へも届く仕組みが必要になります
3. 抜け駆け・評価経済
   - 多くの人は資本主義側にとどまろうとします。一方で、全面無料で顧客を奪う動きが出れば、移行は避けられません
     - SNS で話題化を狙って全面無料化する抜け駆けが起きれば、評価経済での収益モデルへ切り替えざるを得なくなります
     - 音楽のデータ化による海賊版音楽に対処するビジネスモデルとしてサブスク（Spotify など）に移行する流れ
       - 生活コストが下がると、フォロワーや評価を狙って無償提供する動きが増えます。有料でもよいサービスを、人気取りだけで無料にする人も現れます

### 資本主義と無料主義を併用

- **資本主義で価格をつけて販売しながら、その販売による評価軸を算出して評価軸ポイントを付与する**

- 資本主義で抽選をしているような場面でも、無料主義の評価軸スコアで順番を並べ替えれば倫理的な説明がつきやすいです
- 資本主義の抽選で配る場面では、評価軸ポイント順位が高い順に渡します
- この場面が増えるほど評価軸ポイントを稼ぐ動機も強まります
...
- 資本主義をメインとして無料主義アプリにも登録し、評価軸ポイントをためます
- 十分な所持者は資本主義側の値引きや、抽選枠での優先案内などを享受しやすくなります

- **心地よさや利便性の高さゆえに、無料主義へ対応したサービスを選ぶようになる**

- 「倫理的な運営」「満足度の向上」などの評価軸を設定した無料主義対応サービスは快適なので利用者は増えます
- そのうえで評価軸ポイントを稼ぐ利用動機も出てきます
- 経済圏内で需要が高いサービスを提供する人は、資本主義より高い報酬を得やすくなります

- **「無料主義を普及させる」という評価軸を設定**

- 「無料主義を普及させる」という評価軸を設定し、貢献に応じて評価軸ポイントを付与する方法もあります
  - 需要側は無料で得られるため無料主義を好みます。利用が広がると、欲しい品は評価軸ポイントでも得やすくなります
  - 手に入るコストが下がるほど、希少特典は無料主義へリソースを寄せたほうが合理的になりやすいです
  - その結果、無料主義へ寄る流れが強まります
- 無料主義に移行するときは、無理に国を新設せず、資本主義や現行の国家の枠内で抵触しない範囲のルールから始めます
  - 少しずつ資本主義の仕組みを形骸化させ、無料主義のルールが効く範囲を広げます
  - 皆が望み、徐々に移行し、現在の資本主義の仕組みを形骸化させる流れを作らないと進みません

- **市場の失敗を解決することで移行のインセンティブを作る**

- 資本主義では市場の失敗があり、報酬は伸びにくいことがあります
- 無料主義の報酬がそのすき間を埋めます
  - 歪み続きの市場ほど評価軸ポイントを前提へ組み込み、提供者と利用者が同じ基準へ寄っていく流れになります
  - 移行の動機になります
    - 保育士、児童相談所の人たちの給料、その他
- 資本主義で「市場の失敗」が起こっている部分に、無料主義を導入することから始める
  - 例えば保育士の給与は行政が決めて市場の原理が働きにくいので、無料主義側では無償提供の有無にかかわらず貢献度を市場原理で評価し、その評価を供給の指針にできます
- 生活必需品を受け取れるセーフティーネットと、資本主義では手に入りにくい限定品の提供に使う段階が最初のフェーズです
  - 「市場の失敗」分野では、対価が障壁で利用できていないが実需要がある品目を優先的に扱います

### 全員が損する事態を防ぐために移行

本当は有料で提供できるのに、ダンピングや無料提供の抜け駆けで全員が損する事態を防ぐため、無料主義へ移行しやすい流れになり得ます。
...
- オープンソースにするなどの抜け駆け
  - 抜け駆けを行う人や、AI モデルをオープンソースにする人は、学習費を自腹で負担しつつ利益を得にくいです
  - 有料でクローズド提供していた企業も、無償の抜け駆けで収益を奪われ、結果として誰も得しない状態になりやすいです
  - 資本主義のまま抜け駆けが進む社会は危険です
  - 人間の知能を超える汎用 AI が普及する局面では、資本主義を抱えたまま突き進むと全員が損しやすいです
- 賃金の抜け駆け
  - 賃金を安くてよいという合意が広がると、全体の賃金水準は下がりやすく、損失も膨らみやすいです
  - 無料主義では価格決定をシステムへ寄せ、人為的な値下げ競争で全体が損する事態を抑えたいです
- 値下げによる抜け駆け
  - 経済成長が続く限りインフレもあり、株価の平均は上がりやすいです
  - 一方、抜け駆けで無料提供が広がり GDP やインフレが止まれば株価は下がり、資本主義は苦境に陥りやすいです
  - その結果、無料主義へ移らざるを得なくなる局面もあります
- 評価経済学的な抜け駆け
  - フォロワー獲得のための無料提供が広がると、有料の企業は対応しにくくなります
  - 結果、実質的に無料提供を強いられる社会になり得ます
  - 生活コストが下がれば、フォロワー欲しさに無料提供する人や、純粋な貢献で動く人が増えます
  - 本来は有料でもよい商品やサービスを、対価なく出す人が出てきやすくなります
    - 抜け駆けの拡大で、利益を狙う側の取り分も削られ、景気は冷え込みやすい
    - 無料主義では無償提供でも評価軸ポイントとして報酬を設計できるため、この種の詰みを和らげられる
  - 土地を無料で貸したり原価で出すことを慈善とみなし、格安販売する飲食店はすでに存在します
  - その影響で周辺店は集客が難しくなります
    - そして、誰もが付加価値をつけられる戦略を取りにくくなりジリ貧化しつつ撤退が進まない
    - 価格競争が続き、利益志向の事業者まで無料や原価提供へ引きずられ、経済全体の活力が下がる
- 価格競争の激化で利益は枯れやすく、生活コストの低下は抜け駆けを増やしやすいです
- この組み合わせで資本主義は機能しづらくなり、無料主義の必要性が高まります
  - 評価に乏しいと動かない層まで動けなくなり、社会全体の活力低下が懸念されます
...
- 通貨安が止まらなくなったり、ハイパーインフレや借金の返済破綻などが重なった場合、無料主義へ移行せざるを得なくなる可能性がある
...
- 無料主義へ移行する方法の例として、次のステップを想定する
- 多額の資金で基金を設け、運用益を回す
- 共感する人へは、商品の原価譲渡や市場価格での購入を通じて支援する
- オープンソースエンジニア、介護士、研究者など、市場の失敗や制度のすきまで不当に低報酬となっている人を主な支援対象にする
- 基金を運用しつつ、誰がどれだけ貢献したかを測る算出方法のプロトタイプを作り、改善を重ねる
...
- 無料で良いことをしている人にベーシックサービスを提供するところから始める
  - OSS のエンジニア、NPO の職員、ボランティア参加者、看護師・研究者・保育士など、価値は高いのに報酬が抑えられがちな人へ、働きぶりに応じて評価軸ポイントを付与し、企業の協力で割引や無料提供を組み合わせる
