...
- 「公式パッケージの仕組み」の説明
  ...
  - 1 つの商材（無料主義アカウント）に、**複数の公式パッケージ**を登録してよい（用途や文脈に応じた配分レシピを複数持ち、自動分配や手動運用で使い分ける想定）
...
- 仕様
  - 別のアプリに対して、保有していたポイントを伝える API などがあれば良い？
...
1. OSSで提供したい
	- 商業サービスとして提供したら、どうせよりより競合がOSSで出てくるのだから、はじめからOSSで提供して社会全体に貢献したほうが良い
...
5. 競争激化の評価軸
    - 説明
	    - Fediverse的設計などで、別サービスへの移行の簡単さなど競走を激化する機能や設計を評価する
6. アーリーアダプターの評価軸
    - 説明
...
10. オープンにする評価軸
    - 説明
	    - 独占するよりもオープンソースにして共有した方が報酬を増やす評価軸
11. タスク提示型の評価軸
    - 流れ
        1. 評価軸の作成者が、このタスクを完遂した人に事前に配布する報酬を決めてJiraチケットを作成
        2. タスクは誰でも閲覧と成果物のアップが可能
        3. タスク作成者は、アウトプットを採用する提出者を選んで報酬を付与
12. 絵文字による貢献度算出の評価軸
    - 評価の流れ
        1. Discordやslackなどのチャットツールに接続
        2. チャットツール内の特定の絵文字を「評価」として設定
        3. 一定期間ごとに、特定の絵文字を集計
        4. 集計結果をもとに貢献度ポイントを付与
    - 貢献度の算出アルゴリズム
        1. Googleのページランクのように、コミュニティから評価されている人から与えられた評価は高くする
...
13. 作業の評価軸（作業ごとの重み付け）
    - 評価内容
        ...
        - チケット消化数
    - 評価の流れ
        1. ユーザーごとの各作業の数をAPIから取得
        2. JSONファイルで、各作業ごとの重み付け数を取得して掛け算する
        3. 計算結果を集計して出力
...
14. 育ちの評価軸
    ...
    - 例
        ...
        - つまり、1番評価が高くなるのは、貧乏な家に生まれて、その他でも全く恵まれないで育った環境の指標は３で仕事で結果を出す3ポイントで、評価が9ポイントになること
    - 指標
        1. 何月生まれか
        2. 虐待を受けたか
...
- **複数名義での参加**
	- 「個人用」と「製品／プロジェクト名義」など、同一人物が異なる名前で無料主義経済に参加する場面がある。サービス側は、ログイン主体をひとつにしたうえでも、複数のアカウントまたはプロフィールを切り替えて運用できる形を取りうる。
- **表示名とは独立した恒久的リンク**
	- 参加者に対するリンク（プロフィールURL等）は表示名変更に追従しない**内部ユーザーIDなどで安定化**させ、外部に貼った証明URLやポートフォリオのリンクが壊れないようにする狙いがある（証明データとの突き合わせは別途、サービス側プロフィールURLの正規化照合などで行う）。
...
- 幸福度などのあらゆる指標は絶対的な正確性より納得性が高ければ一旦はOKだし成立する
...
- 会話の加工
    - 説明
        - 自分が嫌な話題を相手が話した場合は、AIが加工して聞こえる内容を変える
    - 参考
...
- 参考
    1. アフリカ人向けのデジタル国を作ろうとしているAfropolitanが$2.1M調達を発表。
...
- 具体的な目標
    1. 労働あり
        - 1ヶ月(22日)の労働を最低賃金で労働した場合、2026年の日本で年間480万円を消費する生活ができるようにしたい
    2. 労働なし
        - 1日も働かずに、現在の年間240万円（月20万円）を消費した生活ができる
    3. セーフティーネット的サービスの提供
        - 高性能・高機能・高サポート体制・高コストの商材を買えないときに使えるサービスを充実させたい
        - 高い「Adobe」の代わりに最低限の機能がある「Affinity」
        - セーフティーネット的に最低限の無料サービスがある状態をすべてのサービスで作りたい
    4. 全ての商材の応募倍率が1を目指す
        - 全ての商材を無料で販売した場合の応募倍率が1以下になることを目指す
...
- 生産性の指標アイデア
    1. 「すべての商材の実質価格の平均値」の変動
        ...
        - 欲を満たせる商品
            - 無料主義では、供給者が値付けせず、需要者の間で優先的に得る為だけにポイントを使用する
            - 誰でも得られる商材は、これ以上優先的に得ようと獲得を争う人がいないので価格も0円に近づいていく。
            - この様に、実質価格は供給量と同じ人数だけ買いたい人が残るように価格が高くなる
            - なので、実質価格の平均値の下落を見れば、どれだけ人を絞っていないか・欲を満たせているかを示せる
            - 価格を高くしても売れると言う事は価格を高くすることによって本当は欲しいけど、高額な価格が払えない人たちを除外していると言う事。
            - 除外していると言う事は、欲を満たせていないと言う事。
    ...
    2. 昨年消費した商材を今年得る為に必要な実質価格の変動
        - 問題点
            1. 旬が過ぎた場合に比較できない
                - 流行っているタピオカ店の価格などは状況が変わっているので比較できない
        - 生産性を「昨年購入した商材を購入するために必要な実質価格」から「今年に同じ商材を購入するために必要な実質価格」の減少する割合で算出する
        - （今年の合計実質購入額÷今年12/31時点の発行ポイント総額） ÷（昨年の合計実質購入額÷昨年12/31時点の発行ポイント総額） を国民全員分だけ算出して、その平均値を取る
    ...
    14. GPI（世界平和度指数）
    15. GPI（Genuine Progress Indicator：真の進歩指標）
...
- 実現したいこと
    ...
    2. 別コミュニティーで生活
        ...
        - 現在は同じSNSなどで交流したり議論しているが干渉しない社会にしたい
    ...
    6. 評価されたい評価軸のサーバーに参加
        - 説明
            - Fediverse的設計のサービスに参加する場合は、各サーバーごとに、各評価軸の貢献に最適化したサーバーを用意したい
        - 流れ
            1. 各SNSなどのアプリごとに、評価されたい評価軸に適したサーバーに参加
...
- 具体的な仕組み
    ...
    3. マイナス評価の場合の罰則に利用
        - マイナス評価の場合は、「非干渉の仕組み」を利用して、一定期間OOのサービスの使用禁止を行う
...
- ハードウェアのオープンソース化
    - 例
        ...
        2. スマートフォン
            - フリーソフトウェア財団が「自由」に使えるスマホ・Librephoneプロジェクト
        3. 電子機器
            - オープンソース電子機器プラットフォームArduino
        4. 家電
            1. 冷蔵庫
        5. 家具
            1. スタンディングデスク
...
- 実現する方法
    - まずは、fediverse的設計を普及させるために必要なライブラリを作りたい
    - ２段階のライブラリを作る
        1. すべてのサービスに適応できるfediverse的設計の基盤になるライブラリ
        2. 各サービスの種類に特化したfediverse的設計になるライブラリ
            - 例）ActivityPubなど
...
- 収益化
    1. 広告
        - OSSとして競争＆協力してもらうために、サブスクのような自分だけ儲かる設計はNG
        - Fediverse的設計により、全員が協力して求人OSSを作る状態にしたい
...
- 広める方法
    1. HP求人の埋め込みによる求人数をはじめから増やす
    2. 想定質問をまとめたMarkdownを配布
...
- 求職者の課題
    ...
    2. 事前に記載してほしい情報がある
        ...
        - 例
            - 未経験エンジニアのSES求人で、教育後にアサインとあるが、カジュアル面談で1年間のコールセンター配属になることが判明。
    3. 求人を探すのが面倒
    4. 志望動機を考えるのが面倒
        - 説明
            - 各社ごとにビジョンや事業内容を見て志望動機を考えるのが面倒
        - 解決方法
            1. 埋め込みHPをMarkdown変換してコピペしてChatGPTに張り付けられる機能
...
- 求人掲載する企業の課題
    ...
    2. データ更新が面倒
        ...
        - 解決方法
            1. CSVアップロードで、複数の求人を一括更新
                - CSVアップロードの為のテンプレを用意して、ヘッダーに書くべき情報名を記載して起き、そこに追記してアップロードすることで更新可能にする
    3. ミスマッチ
        - 説明
            - new
        - 解決方法
...
- 求人サービスの機能
    - 求職者
        ...
        4. 「いいね」した求人の確認画面
        5. 自分の職務経歴＆職種選択の画面
            - markdownのテキスト入力欄と「職種・業界」選択のみを用意
        6. 求人の確認画面
        7. 応募ボタン
    ...
    - 求人掲載の企業
        ...
        2. 求人記載と異なる内容の場合の報告機能
            ...
            - 乖離度合いを5段階評価できるようにして、その乖離指数を求人に表示する
        3. 年収の記載分け
...
- 結論
    - 作業にかかる時間を削減できたら、一回当たりの供給量が増えるため効率化するインセンティブを作れる
...
- 「Fediverse」の説明
    - 独立した各サービスを一つのサービスとして使用できる仕組み
    - 移行のしやすさによる競争激化が社会をより良くする
    - 移動による投票
    
- 「Fediverse的社会」で実現したいこと
    1. 移行が簡単な社会
        - 不満足なら即移行
    ...
    7. 誰でも貢献する作業を実施可能
        - OSSのプルリクのように、誰でも貢献する作業ができるようにしたい
    8. 各評価軸別にサービス別を管理できるが、一つのサービスとして使用できる社会
        - 各評価軸に適した運営ルールの適用
    9. サービス自体の勝ち負け関係なく協力して1つのサービスを作り上げて行く社会
...
- 具体的な方法
    - 資本主義では難しいが、無料主義ではFediverseでも利益を出しやすい
    - fediverse的なビジネスモデルや設計が大前提になる社会
    - ソフトウェア、ハードウェア問わず
...
- サービスの例
    1. X(Twitter)・Mastodon
        - 別々のSNS投稿やユーザーやフォロワーをそれぞれのクライアントアプリで閲覧可能で全て共有して、1つのX(Twitter)アプリのルールに依存しない仕組み
    ...
    8. メモアプリ
        - メモの移行が大変
    9. EC・フリマ・オークション
        - メルカリ、フリマ楽天、Yahoo!オークション、Amazonでネットワーク効果を実装させない
...
- 参考技術
    1. IFTTT
        - IFTTTでRSS連携してBlueSkyの投稿をXでも投稿する仕組みを参考にしたい
...
- 問題
    - 給与の決まり方が「権力、慣性、模倣、公平性」の4要素で決まる。
...
- 報酬を「権力、慣性、模倣、公平性の4要素」ではなく「能力・成果」で決める方法
    1. 「競争激化の仕組み」を使用する
        - 給与の競争する仕組みが少ないので、上記4要素による給与決定になる。
        - それをより競争を激化したい
...
- 解決策
    1. 自由に自発的に許可なく新しいことを始められるような仕組み
        - 無料主義の「許可が不要な仕組み」
        - 無料主義の想定ではサービスは、BlueSky的(fediverse的)な設計でOSS化されている。
        - そのため、forkして運営しながら、fediverse的設計により相互運用性がある状態
...
- 解消したい問題
    ...
    2. 政治以外でも、SNSの「いいね」などにも適用
        - 毎月「いいね」の上限数 or 一投稿に10いいねまで。で強く賛同・少し賛同を使い分けたい
...
- **逆に、インフレ防止の為に評価対象外にしたり、ポイント発行しない場合**
    ...
    4. そのため貢献する人が少なくなり、その評価軸のポイントの交換比率が不利になり、使用場面は無くなる
        - その評価軸が、「React公式から認定されるReact発展の評価軸」など特定の対象の貢献を評価する評価軸だった場合は、Reactに貢献する人がいなくなり、他のライブラリが栄えるようになる。
...
- 説明
	- 全てのデータを共有・移行を簡単にしたい
	- あるサービス(A)とフォークしたサービス(B)で、Aの投稿データをBでも見ることができて、共有する社会にする
...
- 実現する方法
    1. 当選率を使用する方式
        ...
        - (1+(100-当選率))×入札額=得るために必要なポイント額
            ...
            - 他者の入札額が不明なオークションで
                - 他者の入札額がわかる場合は、予算と離れている時に入札自体行わず当選率が不明
    2. 平方根にする方式
...
- 参考
    1. GenomesDAO
        - ゲノム研究の民主化・分散化を進める
        - GenomesDAOはこれまでサードパーティが握っていたゲノム情報を個人が所有することができ、そこから得られる利益を個人が受け取ることができる仕組みを作っています
...
- 実現したいこと
    1. この世に存在するデータを独占せず、1つに集めて共有し、使いやすい状態に加工してあるデータベースの機能
    2. 分析する生産性を高める
    3. データの統一
        - 例）各アパレルごとのサイズの統一
...
1. 前もって消費 して、後から支払う
	...
	- 借りた額に達するまでは獲得したポイント× 0.8になるようにする。等の後払いみたいな仕組み
2. 「譲渡の仕組み」を使用する
...
1. ボルダルール
    - 現在の選挙は、1人に対してしか投票することができない。それぞれの議員が何位か投票できたほうが全体の利益になりやすい
    - 一人一票ではなく、「1位に3点、2位に2点、3位に1点」のように配点する形の投票システム
...
- 無料主義で実現する方法
    ...
    2. 価値を高めるインセンティブ
        - 使用している財の価値を高めるインセンティブを作るために、自分が獲得した時より放出した時に価値が高まっていれば、その差額の一部を貢献度ポイントとしてもらえるようにする
    3. 消費財
        - 消費財のCOSTは、 一定期間枠を占める仕組み
...
- 説明
	...
	- 裁判所が行っている判断を市場に任せて、どちらの方が社会全体に利益をもたらすかを計算して、その利益が大きい方が建設する権利を認める仕組み
...
1. 無料主義ポイントを信頼性の指標として使う
	...
	- なので、信頼してOK化、無料主義ポイントを信頼性の高さの指標として使用する
2. BAN連鎖
    - 無料主義アプリには、一人当たりに多くのSNSアカウント連携があることで、無料主義アカウントをブロックしたら、そこに紐づいている他のSNSアカウントもBANできる仕組みを作る
    - 一括ブロックが難しいSNSばかりなので、ブロックすべきアカウントのリストを共有する仕組みを作りたい
3. 取引
    - 資本主義と同様の取引
...
2. ポイントを多く保有する人物から優先的にアサインする方式
    - PoSの考え方
3. OpenPGPの「信頼の輪 (Web of Trust)」的な信頼できる評価者を選ぶ方式
    - AI生成のヨタ記事を垂れ流ししてるような人は基本的に全員信用できないので、今後はそういう人達を共有リストに入れて管理する仕組み作りも必要かもしれません。こういう時にどうしても必要になってくるのがPGP/OpenPGPが使っている「信頼の輪 (Web of Trust)」的な考え方
