...
- [無料主義 v3 beta](#無料主義-v3-beta)
  ...
  - [評価軸の仕組み](#評価軸の仕組み)
    - [概要](#概要-1)
    - [グループと評価軸は別物](#グループと評価軸は別物)
    - [「個人」、「一般的な評価軸」、「無料主義アプリ」の評価軸](#個人一般的な評価軸無料主義アプリの評価軸)
  - [証明の仕組み](#証明の仕組み)
    - [概要](#概要-2)
    - [手段](#手段)
    - [選び方・優先順位](#選び方優先順位)
    - [「証明する仕組み」の使用場面](#証明する仕組みの使用場面)
    - [OIDC認証](#oidc認証)
    - [SSI・DID・VC](#ssididvc)
    - [ワンタイム・チャレンジ](#ワンタイムチャレンジ)
    - [URLの相互リンク](#urlの相互リンク)
    - [PGP / Keyoxide など](#pgp--keyoxide-など)
    - [ドメイン所有での証明](#ドメイン所有での証明)
    - [「GrimoireLab」の「Sorting Hat」](#grimoirelabのsorting-hat)
    - [「GrimoireLab」の「Hat Stall」](#grimoirelabのhat-stall)
  - [互換性の仕組み](#互換性の仕組み)
    - [「互換性の仕組み」の説明](#互換性の仕組みの説明)
    - [「互換性の仕組み」の種類](#互換性の仕組みの種類)
    ...
    - [譲渡の仕組み](#譲渡の仕組み)
  - [購入方式の仕組み](#購入方式の仕組み)
    - [「購入の仕組み」の説明](#購入の仕組みの説明)
    - [ポイントの扱い方の種類](#ポイントの扱い方の種類)
    - [「一定期間預けて還元」とオークション](#一定期間預けて還元とオークション)
    - [商材履行（配送・対価）とプラットフォームの境界](#商材履行配送対価とプラットフォームの境界)
    - [購入方式](#購入方式)
    - [オークションの落札決定と支払額（マルチユニットを含む）](#オークションの落札決定と支払額マルチユニットを含む)
    - [逆オークション](#逆オークション)
    - [入札に使用できるポイントを決める方法](#入札に使用できるポイントを決める方法)
  - [貢献度の算出の仕組み](#貢献度の算出の仕組み)
    ...
    - [「貢献度の算出の仕組み」の原則](#貢献度の算出の仕組みの原則)
      - [評価結果の再利用](#評価結果の再利用)
      - [一つの評価軸ごとに一つの視点（KPI・目標）](#一つの評価軸ごとに一つの視点kpi目標)
      - [人と商材は評価方法を分けない](#人と商材は評価方法を分けない)
      - [同じ業界でも、適した評価方法は異なる](#同じ業界でも適した評価方法は異なる)
      - [分析結果の公開](#分析結果の公開)
    - [パッケージの仕組み](#パッケージの仕組み)
      - [パッケージの説明](#パッケージの説明)
      ...
      - [「幸福度の向上」パッケージ](#幸福度の向上パッケージ)
        ...
        - [「感情分析」サービス](#感情分析サービス)
        - [「感情分析」理論](#感情分析理論)
        - [**文章から感情を分析する方法**](#文章から感情を分析する方法)
        - [動画から感情を分析する方法](#動画から感情を分析する方法)
        - [音声から感情を分析する方法](#音声から感情を分析する方法)
        - [文章から幸福度を分析する方法](#文章から幸福度を分析する方法)
        - [動画から幸福度を分析する方法](#動画から幸福度を分析する方法)
        - [音声から幸福度を分析する方法](#音声から幸福度を分析する方法)
        - [その他](#その他)
      - [他のパッケージの例](#他のパッケージの例)
    - [データ取得サービス](#データ取得サービス)
...
9. 汎用的な法律の仕組み
10. 共同購入の仕組み
11. 限定招待のオークション形式の仕組み
12. オークション形式の仕組み
13. 他者購入＆指名販売の仕組み
14. 条件付き代理購入する仕組み（権利を提供する側の人に戻ってくる）
15. 条件付き代理購入する仕組み（権利を貰う側の人に戻ってくる）
16. 代理購入する権利の売買の仕組み
17. 評価軸ポイントを譲渡できる仕組み
18. 評価軸ポイント以外の、特定のサービス専用ポイントの付与の仕組み
19. 代理購入の仕組み
20. 報酬が決まる仕組み
21. 提供側の価格の決め方
22. 提供する相手を決める方法
...
43. 無料主義の販売方法
44. オークション形式の仕組み
45. 自分が共有しなければ他者の公開内容を使えない仕組み
46. 当選率向上による報酬 UP の仕組み
47. 無料主義アプリの仕組み
48. 枠の仕組み
49. 互換性の仕組み
50. オープン・公開＆共有する仕組み
...
- 説明
	- この世のすべての作業・取引を、一つの視点・観点から評価する仕組み。または、その評価を行う人達のグループのこと
	- その評価結果を基に貢献報酬を付与する
...
- 説明
    - 「グループ」と「評価軸」は別物
    - 「グループ」は活動単位で、「評価軸」は評価単位で、一つのグループを複数の評価軸が評価する場合があるし、一つの評価軸が複数のグループを評価する場合もある

- 例
    - 「あるSaaS（A）を開発するグループ」と「あるSaaSを発展への貢献を評価する評価軸（B）」は別物
	    - （A）が別の評価軸（C）に貢献した場合は、（A）が公式の評価軸として認定している（B）のポイントを持つ人たちで分配する

- 要件
    1. グループに特化した登録機能は不要
        - 個人ユーザーと同じで、SNSアカウント連携してアカウントを作成するだけでOK
...
- **「無料主義アプリの評価軸」**
    ...
    - 「無料主義アプリの評価軸」を基にアプリ内の善悪を判定したり、汎用的な法律が実現できる
...
2. 貢献データとプロフィール連携の突き合わせ
	- 評価軸が貢献者アップロード等でポイントを付与するとき、貢献データ側の識別子は**各サービス上で当該人物を表すプロフィールURL**を原則とする（OAuthのユーザーIDやアカウント名の生文字列同士のみの照合は、表記ゆれ・独自ドメインなどで一致し損ねやすい）。
	- アプリは、貢献データのURLとユーザーがプロフィールに登録した連携URLを**同一の正規化ルール**で揃えたうえで比較する。同一ユーザーが**複数の連携URL**を登録している場合は、正規化後の貢献側URLが**いずれか**と一致すれば同一人物として扱う。
	- **URLから人物を推測しない**
		- パスを`/`で区切り**末尾セグメントだけをユーザーIDとみなす**といった処理は行わない。記事パスや独自ドメインでは、ユーザー名がURL末尾に現れないことがある。**正規化したURL文字列同士で一致させる**のが基本である。
	- **相互リンク（HTML取得）側の読み取り優先順位の例**
		- 取得ページに`rel="me"`付きリンクが**一つでもあれば、そのリンク先のみ**を「自分への戻りリンク」候補として検証する。`rel="me"`が**一つもない**場合のみ、`rel`なしのリンクも対象に含めうる（Zennなど`rel="me"`非対応サービスと、誤クロール抑制のバランス）。
	- **同一外部URLとアカウントの一対一**
		- 報酬や連携の二重取得を防ぐため、検証済みとして紐づけた外部プロフィールURLは原則**一つのアカウントにだけ**許容し、ほかのアカウントへの再紐付けを許さない方針が取れる（運用・アプリ側でURL単位の占有を表現する）。
...
- **長所**
    1. ユーザーの作業が最も少ない
- **貢献データとの照合との整合**
	- アップロード側がプロフィールURLをキーにする前提と揃えるため、OAuth/OIDC で得られる識別子は、そのまま生の`sub`やスクリーンネームだけで突き合わせるのでなく、可能なら**連携サービス上のプロフィールURL形式**へ落とし込んで保持し、`貢献データとプロフィール連携の突き合わせ` と同様に**双方を正規化して一致判定**する（プロバイダによってはユーザーIDのみ返るため、URLへの組み立てルールが必要になる）。
            ...
            1. タスク実行時期で区切る
                ...
                - 仕様
	                ...
	                2. キーの粒度
					 ...
					 3. キーの粒度の問題点
						 1. 年月日をキーにした場合に、A評価軸が本当は1/24以前の複数タスクなのに、1/25を実行日としてポイント付与を集中することで、1/25はA評価軸を選ぶようなハックが生まれる。
...
- **消費との違い**
	- 「一定期間預けて還元」では、評価軸（またはプラットフォーム）が定める**預け期間のあいだだけ**、当該分のポイントは再利用できない状態に拘束され、満了後には**保有として再度流通に使える**形に戻る。経済圏から恒久的になくなる「消費」とは別物である。
- **価格決定ルールとの関係**
	- 拘束されるポイント**量は**、その取引で採る価格決定ルールで定まる。例としてオークションでは `オークションの落札決定と支払額` のシングルプライス等で支払単価が決まり、預ける／消費する／消費なしの別は、この確定後の**会計上の取り扱い**として乗る。
	- 「入札時はロックしない。落札確定時に初めて不足を検証し、足りなければ繰り上げ」という方針（`入札時と落札時の残高`）とも整合する。**預け**は確定後に拘束が始まる形で説明できる。
- **設定の粒度**
	- 預けの有無や期間の長さ・起算点（例：落札日以降）は、`ポイントを預ける必要性` で述べたとおり、アプリ単位／評価軸単位／取引単位のいずれかで規定される想定であり、評価軸の禁止がある場合は取引側の設定より優先する。
- **返還するポイント量**
	- 「一定期間預けて還元」とオークションを組み合わせる場合、拘束されるのは **`オークションの落札決定と支払額` で確定した支払額（シングルプライス等の単位価格×数量など、確定した会計上の負担額）** に相当する分であり、**利用者が画面上で入力した入札額そのもの**と必ずしも一致しない（シングルプライスでは確定支払が入札表示額より低いことがある）。
	- 預け期間終了後に保有へ戻すのは、この**確定支払・拘束分**を返すイメージである（実装ではグループや評価軸ごとの預け期間設定と組み合わせうる）。

### 商材履行（配送・対価）とプラットフォームの境界

- **範囲の切り分け**
	- 無料主義アプリおよび評価軸ポイントの仕組みの中心は、貢献の評価・保有・配分と、それを使った優先権等への**マッチング（出品・オークション等）まで**であり、現物の**配送・住所管理・本人確認のオペレーション**までをプラットフォームが肩代わりする前提ではない運用がありうる。
	- 履行は別のマーケットプレイス・決済サービスや当事者間の手続に任せ、無料主義側では結果の証跡だけを記録するという切り分けが取りうる（`商品の提供を強制的にする仕組み` と同様に、物理的強制より評価・信頼記録で不履行を扱う方針と相性がよい）。
- **記録として残すものの例**
	- レビュー、取引完了の申告、その補強となる任意の証明の添付など、**結果を第三者が追える形**が中心になる。
...
### オークションの落札決定と支払額（マルチユニットを含む）

- 位置づけ
	- 限られた商材を評価軸ポイントで競り合って配分する際の基本的な購入方式の一つがオークションである（即決価格は、時間をかけずに取得できる特別な出口としてオークションと併用されうる）。

- マルチユニット（複数個・複数人への販売枠）
	- 一つの出品で販売個数（枠）を複数にできる場合、入札が枠を超えるとマルチユニットオークションとして扱う。

- 落札者を選ぶ条件
	1. 同じ入札額どうしでは、入札日時が早い順（先着）を優先する。
	2. 異なる入札額では、高い入札額を優先する。
	3. 枠より入札が多い場合、優先度の低い側から落選させる（最も有利な入札の並びに入れない者から順に落とす）。

- 落札価格（シングルプライス方式の例）
	- 落札できなかった入札者のうちで、「最も入札額が高い人」の中で「入札時期が一番早い」者を「ギリギリ落選者」と呼ぶ。
	- ギリギリ落選者よりも入札日時が前の落札者は、ギリギリ落選者と同じ額を支払う。
	- ギリギリ落選者よりも入札日時が後の落札者は、ギリギリ落選者の入札額に評価軸が定める最小単位を1つだけ加算した額を支払う。
	- 落札者は上記に従い一律の単位価格で支払う。
	- 補足
		- 「落札できなかった者の最高入札＋1最小単位を、全落札者が一律で払う」方式だけにすると、0ポイント入札で先着した落札者まで最小単位の支払いが生じうるため、入札日時による払い分けを組み合わせる。
...
- 落札時の残高不足と信頼記録
	- 繰り上げ後も不履行が問題となる場合、評価軸コミュニティの合意のもと、ユーザーに「ブラックリスト行為」のような不足・不履行カウンタを持たせ、外部サービス連携や利用制限の判断材料としてよい（運用は評価軸側に委ねる）。
	- 一例として、**落札確定後に自分の番になったとき支払額を保有していない**状況を、カウントを増やす対象に含める運用・アプリ自動化がある（先に繰り上げで順位は解消し、その上で信頼記録につなげる）。
	- **マイナス帳簿**（貢献のマイナス付与などで保有が0未満）であっても、確定時点で支払額を満たせなければ上記の「不足」と同様に扱い、繰り上げと信頼記録の対象になりうる（手動入札・自動入札を問わない）。

- 同一入札者が複数個（希望購入数）を求める場合の配分
	- マルチユニットでは「落札優先順位」が決まったうえで、各入札ブロックごとに希望個数だけ枠を占有する。
	- **残枠が希望に満たない**ときは、その入札ブロックについて**得られる個数だけ**割り当て、優先順位が低い側は個数が減ったり割り当てゼロになりうる（先頭から順に枠と希望数を消費していくイメージ）。

- 自動入札（複数個・シングルプライスと整合する運用）
	- 参加者が事前に上限などを決め、状況に応じてシステムが入札・優先順位更新を代替する運用でありうる。
	- **考え方の一例**（枠があり、同順位〜は入札時刻で先着のとき）
  	- まず現在の並び順を決める単位での入札（例では 0 から）で枠が埋まり、順位末尾を押し出すには評価軸最小単位を 1 つ足した入札により優先キューを組み替える、といった規則で「価格と時刻」を両立する。
	- **順位の一例**（枠が 3、入札順 a→b→c がともに同額 0 で入札したときの優先 a→b→c のまま、d が 1 単位増の入札をしたときは末尾の c が枠から外れ、優先順は例として d→a→b となりうる）。
	- 自動入札の場合も、確定時点で保有が不足すれば繰り上げと同等の処理にし、そのケースでも上記と同様の信頼記録に載せられる。

- 入札方式の拡張（評価軸ごとの設定）
	- 複数当選者がいる場合の価格決定には、シングルプライス以外に Pay-as-bid や VCG 系などがある。無料主義上は評価軸が採用方式を選べる前提とし、実装・普及段階で差し替え可能にする。

- 単一販売枠（落札者が一人）への読み替え
	-  **落札者以外の入札のうち「「最も入札額が高い人」の中で「入札時期が一番早い」者** を「ギリギリ落選者」と呼ぶ
	- **落札者の支払額** は、 **その落札者とギリギリ落選者の入札日時の前後関係** に適用して決める。
  	- 具体的には、落札者の入札日時がギリギリ落選者より **前** ならギリギリ落選者と **同額**、**後** ならギリギリ落選者の入札額に **最小単位を1つ足した額** を支払う。
	- **差し引く（または預ける）のは、このように確定した支払額であり、自分が画面上で入力した入札額そのものではない**（二番手より高い入札をしていても、確定支払が二番手側の水準に抑えられることがある）。

- UI・運用上の補助表示（例）
	- 「いま優先順位を更新するために必要な最小の増分」を利用者に示す場合、多くは **現在の最高入札額に評価軸の最小単位を1つ加えた額** を下限として提示する（一覧や詳細で「取得に必要な入札額」として表示するパターン）。

- 終了間際の時間延長（スナイピング対策の一例）
	...
	- **強制終了**を出品者またはプラットフォーム運用に許す設計もありうる（不正対応・モデレーション・運営都合など。ルールは評価軸またはアプリで明示する）。

- 自動入札に関する製品ルールの例
	- **実行間隔の下限**：システムが代理で入札する処理に、最短間隔（例：**10分に1回まで**）を設け、短時間に集中的な更新やサーバー負荷を抑える。
	- **同じ上限額で自動入札が競合するとき**：どちらを優先するかを決める必要がある（例：**先に自動入札を設定した参加者を優先**）。
	- **上限と保有残高**：自動入札の上限が **現時点の保有を超えていても設定自体は禁止しない**（落札確定までにポイントが入る可能性を残す一方）、画面上では **警告のみ** を出す、という運用がありうる。

### 逆オークション

- 説明
	- 需要者が事前に支払う報酬（固定額）を決め、供給者側が受け取り額を下げる方向で競争するオークション形式。買い手がオークションを立て、供給側が応札する。

- 活用理由
	- 多くの既存プラットフォームは法定通貨中心であり、評価軸ポイントだけを使った逆オークションの場が乏しい。無料主義では受益者負担がない設計と相性がよく、事前固定報酬の水準を市場で決める用途に使える。
	...
	4. 合計する系
	    - 一覧
	        ...
	        13. ユニーク開発者数
				...
				- 注意
		            - 多くの貢献者は複数の電子メールを使用しているため、コミッターの合計数が人為的に増加する可能性がある
	        14. 未解決issueの残りの数
	        15. 貢献者の合計数
		        - 説明
		            - データ品質に関する既知の問題
	...
	17. セキュリティ関連
	    - 一覧
	        ...
	        9. LAGOON
				...
				- 説明
		            - コミュニティの複雑なエコシステムを理解するためのオープンソースプラットフォーム「LAGOON」
	        10. ARMS
	            - 説明
		            - オープンソースソフトウェアサプライチェーンにおけるエンジニアのセキュリティ資格を追跡するためのシステム「*ARMS*」
...
5. [https://qiita.com/Mizuiro__sakura/items/aa13593b239f91a51486](https://qiita.com/Mizuiro__sakura/items/aa13593b239f91a51486)
...
11. 日本語感情表現辞書
	- 説明
		...
		- [https://zenn.dev/mizuiro__sakura/articles/7e8373a759ef1b](https://zenn.dev/mizuiro__sakura/articles/7e8373a759ef1b)
...
13. 解説・ビジネス記事
	- [https://zenn.dev/mizuiro__sakura/articles/4226070bdd6309](https://zenn.dev/mizuiro__sakura/articles/4226070bdd6309) / [https://www.cloud-contactcenter.jp/blog/sentiment-analysis-in-business.html](https://www.cloud-contactcenter.jp/blog/sentiment-analysis-in-business.html)
...
2. **"オンライン・オフラインの社会的交流と幸福感：デジタル時代の人間関係の質の検討"**
    - オンラインおよびオフラインでの社会的交流の頻度と質が、個人の幸福感に与える影響を分析した研究です。オンラインでの質の高い交流も、幸福感の向上に寄与することが示されています。
...
| **GitLab REST API** | Merge Request・Issue・Job 等を GitLab*Merge-Request*／*Issues* Worker が取得 | GitLab MR Worker（API call → `pull_requests*`）([oss-augur.readthedocs.io](https://oss-augur.readthedocs.io/en/main/development-guide/workers/toc.html)) |
| **Discourse / Forum** | スレッド、投稿、リアクション | *Discourse-Analysis Worker* ([oss-augur.readthedocs.io](https://oss-augur.readthedocs.io/en/main/development-guide/workers/toc.html)) |
| **Messaging / Chat** | Slack・Matrix 等のメッセージログ（任意） | *Message-Insights Worker* ([oss-augur.readthedocs.io](https://oss-augur.readthedocs.io/en/main/development-guide/workers/toc.html)) |
...
| **セキュリティ** | OpenSSF Scorecard・GitHub Advisory 等で脆弱性指標を算出（2025 強化版） | *Security Worker*（v0.90.0 以降）([augurlabs.io](https://www.augurlabs.io/augurs-full-physical-data-model/)) |
...
| **Occasional Contributors**  | 年間貢献 ≤ *n* 回の開発者割合                | 初心者維持率を観測 ([Superuser](https://superuser.openinfra.org/articles/what-is-the-health-of-your-oss-project-find-out-with-chaoss-metrics-and-software-augur/)) |
...
*Augur は Git 系プラットフォームの全履歴＋コミュニケーションチャネルを機械収集し、CHAOSS メトリクス約 40 種類を SQL ベースで再現。*
...
18.  Libraries.io
	- 参考
	    - [https://pure.itu.dk/ws/portalfiles/portal/86173865/msr2021_pfeiffer_1_.pdf](https://pure.itu.dk/ws/portalfiles/portal/86173865/msr2021_pfeiffer_1_.pdf)
	- 説明
...
2. Chrome history API
	...
	- [https://developer.chrome.com/docs/extensions/reference/api/history?hl=ja](https://developer.chrome.com/docs/extensions/reference/api/history?hl=ja)
		- `chrome.history` API を使用して、アクセスしたページのブラウザの記録を操作します。ブラウザの履歴では、URL の追加、削除、照会を行うことができます。
            - Google Chromeの閲覧履歴を直接取得するための公式なAPIは、セキュリティとプライバシーの観点から一般には提供されていません。
            - ただし、Chrome拡張機能（Chrome Extensions）として、ユーザーの許可を得たうえで閲覧履歴を取得するためのAPIが存在します。それが Chrome History API です。
        - 拡張機能のマニフェストに権限を追加
...
3. `GitHub Octoverse`
	- 説明
		...
		- Octoverse で使われる別指標「Mona Rank」
	        - Octoverse 2023 以降で、人気プロジェクトなどの評価に使われる「Mona Rank」という指標があります
...
29. **git-fame 系**（`git-fame`／**git-fame-rb** ほか Node/Go 版）
	- 説明
...
31. **GitStats / gitstats**（HTML レポート系）
	- 説明
...
32. PyDriller（Python）／**RepoDriller（Java）**
	- 説明
...
35. gitdm（Git Data Miner）／**Git Development Metrics**（CNCF スクリプト群）
	- 説明
...
37. **Facade（“FOSS Anomaly Detector”）**
	- 説明
...
40. OSS Insight / OSS Insight Lite（PingCAP / TiDB上のGitHubイベント分析）
	- 説明
...
42. `mntnr/name-your-contributors`
	- 説明
...
57. `tenex/opensourcecontributors`
    - 説明
...
68. `amzn/oss-contribution-tracker`
	- 説明
...
70. `nokamoto/print-github-contrib`
	- 説明
