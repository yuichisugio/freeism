...
- **基本ルール**
  ...
  2. オークション関連の表示項目は、Server-Sent Events(SSE)を使用する
     - クライアントが能動的に再度リクエストしなくても、サーバーのデータ変更があった場合に反映されるようにしたいため。
  3. できる限りサーバーの負荷をかけず、サーバーのアクセス回数も減らす設計
     - 現状では、入札履歴と最大価格をServer-Sent Events(SSE)を使用して、最新情報を表示するそれ以外は、キャッシュやStateで管理して、更新が必要な部分のみで、更新
  4. 商品発送・住所の管理などは不要。
...
- 開発タスク
  ...
  6. **「画像の添付」機能の仕様を実装**
  7. **「出品商品一覧の仕様」の内容を実装**
  8. **「入札・落札した商品を管理する画面」を実装**
  9. **「オークション専用通知の仕様」の内容を実装**
  10. **「Sidebarの修正」の内容を実装**
  11. **「Task編集機能の修正」の内容を実装**
  12. **「検索欄の仕様」の内容を実装**
  13. **「テストの仕様」の内容を実装**
  14. **オークション機能の仕様書を作成**
...
- **パフォーマンス最適化**
  ...
  - 接続数制限: ユーザーごとの同時接続数を制限（5接続まで）
  - タイムアウト: 一定時間（30分）アクティビティがない接続は自動切断
- **フォールバックメカニズム**
  - SSEが利用できない環境向けに、ポーリングベースのフォールバックを提供
  - 3分間隔のポーリングによるデータ更新（SSE接続失敗時）
...
6. **オークションの入札画面のURLは、`dashboard/auction/<taskId>`にする**
7. **入札履歴**
...
- オークション専用の通知を作成
- 通知の仕様
  - すでに実装されている通知とは別機能として、オークション専用の通知を作成
  - ヘッダーには、既存のベルマークの通知の左横にオークションマークのIconを配置して、未読がある場合は未読通知の数を表示します。
  - ホバーしただけで通知内容が見れる。そのオークションのIconを押すとModalが表示
  - 通知のデザインは、既存の通知機能を参考
- 通知を行う条件
...
- SSEでリアルタイム通知を行う
  ...
  - これらの通知は、該当ユーザーがオンラインであれば即座に受け取り可能とする。
  - ユーザーがオフラインの場合は次回ログイン時に通知を確認できるようにデータベースに通知情報を保存しておく設計とする。
- 他の機能
  - 通知の削除機能
  - ページネーション機能
...
- 「落札商品の詳細画面」の仕様
  ...
  - 任意で「完了証明」を添付
    - その証明を画像で添付できたほうが良いが、一旦制限を掛けておく **できれば対応できるようにしておいて、DB負荷削減のために、本番使用までCL側で制限をかけておく方法で行きたい**
  - **「提供方法」**
    - 出品者が記載した方法が表示される
  - **「預けるポイント額」**
    - 落札するために使用したポイントの額
...
- **API設計**
  - 公開APIを作成する際に、GraphQLの使用を検討
    - アプリ内で利用する分では、REST APIとサーバーアクションで足りているが、公開APIでアクセスするときは必要なデータを選べた方が良いと思うので、ハイブリットで使用する検討を行う
- **機能追加**
...
- 実装内容
  - @task-input-form.tsx Task作成Formで、「報酬になる貢献」を選択した場合に、その報酬の画像をアップロードできる設問を追加する
    - アップロードする画像は、Windowに入ったらドラッグ&ドロップ可能になるようにする
  - @csv-upload-modal.tsx のドラッグ＆ドロップができる機能のコードを参考
  - 画像ストレージとして、Cloudflare R2を使用
  - 環境変数を記載するための、 @env.ts ファイルも修正
  - テストで機能を作成するだけなので、一旦は画像の添付はできないようにしておき、１箇所の変数を変更するだけで、画像添付が開始できるコードにする。
  - パフォーマンスと最適化
...
- 開発の流れ
  1. 「実装内容」を実現するために必要なデータベース（Prisma ORMのスキーマ）の修正 @schema.prisma
  2. 「実装内容」を実装
- 参考
  - @Amazon S3
