...
import type { ExecutorJsonItem } from "@/actions/auction/auction-utils";
...
export async function cachedGetAuctionListingsAndCount({
  ...
  userId,
  userGroupIds,
}: GetAuctionListingsParams): PromiseResult<{ listings: AuctionListingResult; count: number }> {
  ...
  if (userGroupIds.length === 0) {
    return { success: true, data: { listings: [], count: 0 }, message: "ユーザーが参加しているグループがありません" };
  }
  ...
  if (groupIds && groupIds.length > 0) {
    ...
    if (allowedGroupIds.length > 0) {
      ...
      return {
        ...
        message: "ユーザーは指定されたグループに参加していません",
      };
    }
  }
        ...
        if (Array.isArray(parsedExecutorsUnknown)) {
          ...
          taskExecutors = parsedExecutors
            .map((exec: unknown): ExecutorJsonItem | null => {
              ...
              const isExecutorObjectFromDBResult = isExecutorObjectFromDB(exec);
              if (isExecutorObjectFromDBResult.success && isExecutorObjectFromDBResult.data) {
                // ExecutorJsonItemFromDB型の場合は、ExecutorJsonItem型に変換
                return {
                  id: isExecutorObjectFromDBResult.data.id,
                  rating: isExecutorObjectFromDBResult.data.rating,
                  userId: isExecutorObjectFromDBResult.data.user_id,
                  userImage: isExecutorObjectFromDBResult.data.user_image,
                  userSettingsUsername: isExecutorObjectFromDBResult.data.username ?? "未設定",
                };
              }
   ...
   * オークション一覧と件数を返却
   */
  return { success: true, data: { listings: items, count }, message: "オークション一覧を取得しました" };
}
