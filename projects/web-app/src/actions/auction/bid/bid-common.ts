...
export async function executeBid(
   ...
   */
  const bidderUserId = autoBidUserId ?? validation.data?.userId;
  ...
  await prisma.$transaction(async (tx) => {
    ...
    if (validation.data?.auction?.isExtension) {
      const { success, message, data } = await processAuctionExtension({
        auctionId,
        auction: validation.data.auction,
        tx,
      });
      if (success && data) {
        toast.info(message);
    ...
    if (initialHighestBidderId !== bidderUserId && initialHighestBidderId !== null) {
      await sendAuctionNotification({
        text: {
          first: validation.data?.auction?.task?.task ?? "",
