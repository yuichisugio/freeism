...
- コードカバレッジ
  ...
  - E2Eテスト: 主要ユーザーフローの網羅
- `pnpm exec vitest --run --coverage`を実行して、現在のカバレッジを取得して、最もカバレッジが上がるコードを考察してからコード実装して、再度カバレッジを取得して数値が向上していることを確認
...
- ツール:
  - Prisma + テスト用DBによる結合テスト
    - Prisma 公式は「Docker などで本番と同じ RDBMS を立て、DATABASE_URL="postgres://…" pnpm
