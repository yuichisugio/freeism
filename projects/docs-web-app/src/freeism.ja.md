...
- 説明
  ...
  - 公式の評価軸との紐付けは、Points への初回ログイン後のアカウント設定、または Points の設定画面で行う
...
- **より細かい開示単位について**
  - 「評価軸ごとに保有ポイントを公開する／隠す」に加え、**履歴単位の非表示**（例：ポイント交換履歴・譲渡履歴・貢献度アップロード履歴）を行う
  - 連携済み外部アカウントの公開範囲は、[Accounts v0.1 仕様書](https://github.com/yuichisugio/freeism/blob/main/projects/accounts-web-app/docs/specification/v0.1/main.md)に従って設定する
  - 自分の評価履歴や行動様式まで含めて他人に見られるのを避け、必要なときだけ協力関係にある相手だけに見せるといった運用を可能にしたい

- **Points のプロフィール全体を秘匿する選択**
...
- 結論
  1. 無料主義のグループ（資本主義で言う会社）への貢献をしているか判断する評価軸（＝「グループの評価軸」）を作成しても、しなくても良い
     - ただし、「グループの評価軸」を用意すると管理しやすくなり、評価漏れも抑えられる利点がある
  2. 評価軸として「『経済圏の評価軸』のみ方式」にすると、分配は不要になる
...
- 評価軸の形
  ...
  - 「**経済圏の評価軸**」の説明
    - 会社や個人が参加し、互いに供給し合う一方で、経済圏グループへ貢献した人たちに報酬を回し続けるグループ
...
- 必要条件
  1. 「貨幣的に利用する評価軸ポイント」と「目標貢献に基づき得られる評価軸ポイント」は同一種類とする
...
- 現在のアイデア
  ...
  2. 既存の多くのアカウントを紐付けている評価軸ポイント管理サービスの OAuth クライアントの ID を使用する
...
- **「無料主義アプリの発展」評価軸**
  ...
  - 「無料主義アプリの評価軸」でマイナス評価になる場合は、評価軸ポイントの交換比率を不利な比率にする
...
- 「参考にされた側に付与する方法」についての、評価軸の設定
	- 説明
		- 参考にした側の人に付与するポイントの付与方法を、評価軸ごとに選べるようにしたい
		- 分配する or 分配せず、新しく発行する
	- 種類
		1. 参考にした側が獲得したポイントから分配
			- メリット
				1. ポイント発行量が少なく済むので、インフレになりにくい
		2. 分配せず、参考にした側の報酬は、新しく発行する
			- 説明
				- 参考にした側が獲得したポイントの 1 部を参考にされた側の人が得られるのは変わらないし、付与するポイント額も変わらない。
				- 変わるのは、その参考にされた人が獲得するポイントは、分配するのではなく、新たに新規発行して付与する方式
			- メリット
				1. 分配する必要がないため、参考にする側が、ポイント分配による報酬が減ることを避けるために他を使わず、自社で開発するような車輪の再開発が無くなる
					- 参考にされる側は、結局は下請けのようになってしまうので、参考にする側による搾取が生まれてしまう問題がない

### 証明の仕組み

この節では、アカウントの所有を証明する仕組みの構想と、関連技術・研究例を整理します。
現行サービスの要件は、[Accounts v0.1 仕様書](https://github.com/yuichisugio/freeism/blob/main/projects/accounts-web-app/docs/specification/v0.1/main.md)を正本とします。
...
外部アカウントの登録・所有権証明・紐付け先の変更・公開設定・貢献者の照合は、[Accounts v0.1 仕様書](https://github.com/yuichisugio/freeism/blob/main/projects/accounts-web-app/docs/specification/v0.1/main.md)にまとめています。
Points などの連携サービスは、この仕様に従って Accounts の情報を利用します。
...
- **仕組み**
  - アプリ A に「Sign in with B」や「B アカウントをリンク」ボタンを置き、B の認可画面で同意→B の安定 ID（例： subject / user ID）を取得して、A 側に「検証済み」と表示する
...
- 処理の流れ
  1. 「アプリ B のプロフィール欄」に「アプリ A のプロフィール URL」へ記載
      - 可能なら アプリ B のプロフィール欄に`rel="me"` を付与
  2. 「使用するアプリ A のプロフィール欄」に、「アカウント所有を証明したいアプリ B のプロフィール URL」を記載
  3. A に B の URL を保存時に、その URL にアクセスして HTML を解析し、A ページのリンクがあるか検証
...
- 設計
  ...
  2.  SortingHat は保守的
      - 誤マージを極力避ける
      - 基本は完全一致による自動マージ＋既知の対応表の取り込み＋人手での確認/修正という三段構えになっている
...
- 処理の流れ
  ...
  2.  正規化
      ...
      - UUID 生成の仕組み
        1.  正規化処理
            - 名前はアクセント記号を除去し（`unaccent_string`関数使用）、すべての文字列は小文字に変換されます。これにより、「Jöhn Smith」と「John Smith」が同一人物として認識される
        2.  複合キー方式
            - ソース、email、name、username の組み合わせから SHA1 ハッシュを生成することで、同一の情報セットは常に同じ UUID になる
        3.  柔軟性
            - email、name、username のうち少なくとも 1 つがあれば機能し、欠損データにも対応できる
  3.  自動マージ
...
- ポイント
  1.  必要なら自前で**より攻めたアルゴリズム**（類似度/機械学習）を SortingHat の API の上に実装する運用も可能
  2.  プロジェクト側で .mailmap 等の整備を進めると、自動統合の精度が上がる＆人手の負担が減る
  3.  所属推定（Affiliation）のロジック
      - `affiliate` は、メールのドメインと domains↔organizations の対応を使って、個体に組織在籍（enrollment）を自動付与します。上位/下位ドメインの関係もモデリングされ、在籍期間（start/end）を持つ形で紐づく
  4.  プロファイル補完・Bot・フィルタ
  ...
  5.  **ブラックリスト**
      - ブラックリストは新規挿入・更新時に毎回適用され、誤マージや不要エンティティ混入の抑止に使える
...
- 互換性が必要な理由
  1. 互換性がない場合は、現在持っている評価軸ポイントを失いたくないだけで、納得しにくい方針へ切り替えた評価軸に残り続ける囲い込みが起こってしまう
  2. 同じ観点で同じ分析を繰り返す車輪の再開発を防ぐ
...
- 交換比率の係数を算出する方法の例
  ...
  2. **ある評価軸による自分の評価軸への貢献度**
     ...
     - グループ A の評価軸で得たポイントを、グループ B の評価軸のポイントとして使いたい場合を想定する
     - 交換比率は、グループ A の目標に対するグループ B の貢献度から決める
  3. **同じタスクをしたときのAとBの評価軸の貢献度を算出する作業をタスクの種類分だけ行い、その平均値を交換比率とする**
     - 例として、評価軸 A を「健康度合いの向上」、評価軸 B を「OSS の発展」として、<br>A で 5 ポイント得ていた状況で、評価軸 B のポイントとして使いたい（互換性を持たせたい）場合を想定する。<br>A 側で 5 ポイント相当とみなせる作業を、評価軸 B 側で行った場合のポイントを対応付けて計算する

#### 貢献評価を代用する仕組み

- 説明
  - 評価軸が別の評価軸との類似度を算出し、その類似度に基づいてタスクの評価を代用する仕組みです
  - 「交換比率を指定して交換」などの他の「互換性の仕組み」は、`交換`であるため、どちらか一方のポイントにしかならないが、「貢献評価を代用する仕組み」では両方のポイントを得られる

- 使用場面・解決できる問題
  1. すべての評価軸で、すべてのタスクを分析するのは大変で評価が追い付かない問題
     - 評価軸 A はタスク B を評価済み。<br>一方で評価軸 C はタスクが詰まっており、タスク B をまだ評価できていない場合に、<br>評価軸 C は、評価軸 A との評価の方向性の類似度を算出し、タスク B を間接的に評価できる
...
- ハックを防ぐ方法・注意・懸念点
  1. 過小に公表するハック
     - 説明
       - 評価軸が人気を集めるために評価したタスクの数を過小に公表するハックを防止する方法
     - 例
       - 評価軸 A が A ポイントの価値を高める目的で、タスク B・C・D の評価分をタスク B だけの評価だったかのように偽り、ほかの評価軸へ伝えたとする
       - 評価軸 E では C と D は評価済みで、B だけを「貢献評価を代用する仕組み」で扱った場合、A の情報に従うと C と D が二重評価になる
       - A は不当に高い評価を付してユーザー人気を稼いでいるため、「無料主義の発展」の評価軸から評価軸 A へマイナス評価を与えて罰する
  2. 二重の付与
     - 説明
       - A 評価軸で評価済みのタスクを、「貢献評価を代用する仕組み」で B 評価軸ポイントとして付与できるのは、B が当該タスクの貢献度をまだ評価していない場合に限る<br>制限を置かないと二重評価になり、何度も B 評価軸ポイントを増やせてしまう
     - 例
       - A 評価軸でタスク C を「貢献評価を代用する仕組み」を使用してポイント付与したい場合に、B 評価軸との類似度を使用して付与する
       - A 評価軸ポイントを得たタスク C が、その後に正式な A 評価軸が評価を行うことで、「貢献評価を代用する仕組みによるポイント」と「正式な評価によるポイント」の二重付与になる
     - 二重付与を防ぐ方法
       1. 貢献度アップロードの「タスクの実行年月」に `年` と `月` の記載を必須とする。日付・時刻の記載は任意
       2. 任意でタスクごとの ID も記載する（例：GitHub の issue_number）
       3. 「貢献評価を代用する仕組み」の類似度で付与したポイント or 正規な評価しか選べない設計にする
     ...
     - 満たしたい条件
       ...
       3. 「類似度のポイント」と「正式評価のポイント」を両立したい
         - そのため、月ごとのポイント管理で両立しつつ、ハックを防げる設計にしたい
    ...
    - その他の要件
       1. 「貢献評価を代用する仕組み」の類似度は、どの評価軸の類似度で適用するか指定できるようにする
       2. 「付与し直す」ボタンを用意
       ...
       4. キーの粒度
         ...
         - 年月日をキーにすると、本来は 1/24 より前の複数タスクにまたがる A 評価軸の扱いを、実行日の 1/25 へ寄せてポイント付与を集中させる動きが起き得る
           - 結果として 1/25 ばかり A 評価軸を選ぶハックへつながる。そのハックで A 評価軸が人気を集めようとする
           - それを防ぐためにも、実行日のキーで分けず、類似度 or 正規評価のどちらかしか選択できないようにする
   3. タスクごとの貢献度の管理が複雑で面倒
     - 説明
...
- 「貢献評価を代用する仕組み」の類似度を算出する方法
  1. ベクトルのコサイン類似度
     - A 評価軸が設定している「『B 評価軸による評価の方向性』と『A 評価軸による評価の方向性』のコサイン類似度」を係数にして、評価の比率を決める
       - コサイン類似度は、-1 から 1 の範囲の値を取る
...
- 流れ
  1. 「貢献評価を代用する仕組み」を使用して自身の評価軸（A）のポイントが欲しい場合の、他の評価軸（B）との類似度を事前に、評価軸（A）が設定しておく
     - 毎回、類似度は評価先の評価軸が設定している類似度を使用する
  2. 「評価軸（B）の貢献度」×「評価軸（A）が設定した評価軸（A）と評価軸（B）の類似度」=「評価軸（A）のポイント」で算出する
     - これは、「評価軸（B）の貢献度」が算出済みで、評価軸（A）のポイントを得たいときの場面
     - タスクは実行月ごとに登録する。それにより二重の評価を防止する
  3. 算出した評価結果から、評価軸（A）ポイントをユーザーが受け取る

- 前提
  - 無料主義アプリ内に存在する評価軸の数だけ重複する評価になる前提
    - でも、「貢献評価を代用する仕組み」の類似度で、評価済みの A ポイントの評価で B ポイントを得られても、次に B を A に「ポイント交換の仕組み」で交換するには、A が指定した比率になるので完全な二重の付与にはならない
...
- マイナス残高と譲渡
  ...
  - ただしマイナス帳簿の状態では、他者へのポイント譲渡は行うことができない方針とする
...
- **「消費」について**
  - 落札・取得の確定時に、当該分のポイントを保有から差し引く
  - 差し引いた分は**無料主義の経済圏から恒久的に消失**する（burn 方式）
  - 他者へ渡す「譲渡」や、満了後に戻る「一定期間預けて還元」とは別もの

- **「消費なし」について**
  - 商材を得ても、当該分のポイントは保有から差し引かない
  - ポイントは**経済圏に残ったまま、再利用できる**状態を維持する
  - 一時的に拘束する「一定期間預けて還元」とも、恒久的になくなる「消費」とも別もの

- **「一定期間預けて還元」について**
  - 評価軸（またはプラットフォーム）が定める**預け期間のあいだだけ**、当該分のポイントは再利用できない状態へ拘束する
  - 満了後は**保有を維持したまま、再び流通へ回せる**状態に戻る
  - 経済圏から恒久的になくなる「消費」とは別もの
...
- **「即決価格」について**
  - 出品者が指定した額以上を提示した時点で、落札者として決定する買い切り価格
  - 競りを待たずに**すぐ取得できる**
  - 時間をかけて配分する「オークション」や、申告額を上回れば譲る「自己申告」とは別もの

- **「自己申告」について**
  - 預ける金額を自分で申告し、それを上回る申し出があれば譲る必要がある仕組み
  - 共同所有自己申告税（COST）に近い
  - 競り合って決める「オークション」や、指定額で即決する「即決価格」とは別もの

#### オークションの要件

- **説明**
  - 限られた商材を評価軸ポイントで競り合って配分する購入方式

- **マルチユニット**
  - 複数個・複数人への販売枠
  - 1 つの出品につき、販売個数を複数設定できるようにする
...
- **落札価格**
  ...
  - 「落札できた人」は、上記に従い決定した**価格**に**統一して**支払う
    - **支払うのは入札額ではない**
  - 補足
...
- **入札時と落札時の残高**
  1.  入札時点では保有ポイント額が入札額より大きいか検証せず、入札額のロックもしない（将来ポイントが入る可能性を残す）
  2.  落札確定時にのみ保有から処理する（消費、預けるなど）
...
- **落札時の残高不足と信頼記録**
  ...
  2. 入札額を保有していない場合は、「ブラックリスト行為」のような不足・不履行カウンタをカウントする
     - 落札確定後に自分の番になったとき支払額を保有していない状況を、カウントを増やす対象に含める運用・アプリ自動化がある場合のみ

- **同一入札者が複数個（希望購入数）を求める場合の配分**
  - マルチユニットでは「落札優先順位」が決まったうえで、各入札ブロックごとに希望個数だけ枠を占有する
  - **残枠が希望に満たない**ときは、その入札ブロックについて**得られる個数だけ**割り当て、優先順位が低い側は個数が減る場合がある
    - 先頭から順に枠と希望数を消費していくイメージ

- **自動入札の要件**
  - 参加者が事前に入札額の上限などを決め、落札枠から現在の入札額で出てしまう場合のみシステムが自動入札する機能
  - 落札枠から出た場合は、評価軸ポイントを事前に設定した落札枠に入る額から指定単位だけ加算した額で入札を行う
  - 自動入札の場合も、確定時点で保有が不足すれば繰り上げと同等の処理にし、そのケースでも上記と同様の信頼記録になる

- **入札方式の拡張**
  - 複数当選者がいる場合の価格決定には、シングルプライス以外に Pay-as-bid や VCG 系などがある
  - 無料主義では評価軸が採用方式を選べる前提とし、無料主義アプリ v0.3 以降で実装する予定

- **終了間際の時間延長**
  - 必要な理由
    - スナイピング対策
  - 終了直前にだけ入札が集中するのを抑えるため、**終了時刻の直前の時間帯に新規入札があったときだけ終了を延長する**、といったルールを製品・評価軸が採用しうる
  - 無料主義アプリの参照仕様の一例として、**オークション期間の0.1％にあたる残り時間**に入札があった場合に終了を延長する、延長時間は **オークション期間の0.1％〜0.5%**の範囲で乱数決定、**延長は最大2回まで**、などが挙げられる（数値は製品ごとに差し替え可能）
...
- **オークション形式の種類**
  ...
  6. **優先オークション**
     - 説明
       - 指定した条件が多い人から提供していく
     - 例
       - **指定した評価軸の累計獲得ポイント**が多い人から優先的に販売する
     - 必要な理由・使用場面
       - 評価軸ポイントが多い消費者から優先して提供すると、供給側は商品提供を通じて得られる分配ポイントが大きくなりやすい
       - その分配額は、その消費者が稼いだ貢献を経由したポイント分配の仕組みと結びつく
       - したがって、その人物が保有する評価軸ポイントの総額で優先度を見ると合理的
       - 分配ポイントが大きい消費者を探す手間を抑えるには、保有総額で並べ替えるのが簡便
  7. **それらの組み合わせ**
...
- **「貢献度合い別オークション形式」の説明**
  ...
  - 例
    - 顧客 A・B・C・D がいるときに評価軸を「幸福度の向上」とする前提での動き
    - A と B は、その商材提供により幸福度が上がりやすいグループ
    - C と D に提供しても幸福度はほとんど上がらない
    - このとき A と B に商品を提供すると供給側は評価軸ポイントを高めに獲得できる
    - A と B だけが参加できるオークションを第 1 とする
    - C と D が参加できるオークションを第 2 とし、この 2 つのマーケットを用意する
    - 第 1 のオークションでは A と B が入札する
    - 第 2 のオークションでも同様に C と D が入札する
    - 入札額は次のとおり
      - A は 20 ポイント
      - B は 5 ポイント
      - C は 100 ポイント
      - D は 40 ポイント
    - 入札額だけ並べると優先順は C→D→A→B になる
    - ところが貢献度を優先すると実際の提供順は A→B→D→C になる
    - まず貢献の見込みが高い第 1 のオークション参加者へ順番どおりに提供する
    - 続いて第 2 のオークション参加者へ提供する
  - 要件
    - 第 2 のオークション側の入札が第 1 の最低入札を上回っていても、先に提供されるのは第 1 のオークションの参加者
    - 第 1、第 2、第 3 などのグループ分けについては、上記 OO％とかで決めておく
      - その入札への参加者数が多くなるほど、グループ分けのグループ数も多くなるように設計するかも

- **その他**
  1. 他の人の入札金額は全て他の人がわかるようにする
  2. オークションで販売することが供給側にとってもっとも報酬が得られる方法とは限らない
     - 供給側にとってもっとも報酬が得られる経路は条件によって異なるため、判断は難しくなり得る
     1. 「貢献検知の仕組み」で評価軸ポイントを効率よく積む顧客へ優先供給するほうが有利な場合
     2. ノウハウを公開して他者の参考になれば報酬が大きくなる場合
  ...
  4. オークション形式で販売すると消費者が特定しやすくなり、評価軸への貢献度合いも捉えやすくなる
  5. 「共同入札」機能
...
1. **OSS で提供したい**
   - どうせ OSS で他から出るなら、初めから商用にするより　OSS で出したほうが良い
...
1. 評価軸を信頼性の指標として使う
   ...
   - なので、信頼して OK 化、評価軸ポイントを信頼性の高さの指標として使用する
...
- 無料主義を実現するための全体像としては、以下の内容
- `calc-contrib`で、貢献度の算出（貢献の発見も兼ねる場合がある）
- `find-contrib`で、貢献している行為の発見
- `points-web-app`で、付与する評価軸ポイントの管理
- `markets-web-app`で、評価軸ポイントの使い道を作る
- `accounts-web-app`で、あらゆるアカウントの ID を統合して管理する

### 分ける理由

- `calc-contrib`と`find-contrib`は、「貢献の発見のための依存関係の調査」と「評価ロジック」を分けたいので、分けた
- `points-web-app`を独立サービスにするのは、このサービス以外からでもポイント管理サービス(`points-web-app`)のポイントを使用できるようにする設計を意識するために別サービスにした
- `accounts-web-app`を独立サービスにするのは、`points-web-app`と責務の分離をしたいのと、アカウント統合するロジックだけを`points-web-app`以外のポイント管理サービスからでも、統合されたアカウントを取得できるようにしたいため
