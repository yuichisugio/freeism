    ...
    sections: [
      {
        ...
        items: [
          ...
          "ログイン後の画面のURLは、先頭にAccountsユーザーIDが付きます（例: /ausr_.../settings）。ほかのユーザーのURLを開くと、そのユーザーにログイン中なら切り替えます。ログインしていなければ、「アカウント連携」「設定」「開発者向け」ではログインを求め、ヘルプや利用規約などは現在のユーザーのまま表示します。",
    ...
    sections: [
      {
        ...
        items: [
          ...
          "After you sign in, page URLs start with your Accounts user ID (for example, /ausr_.../settings). If you open a URL for another user, Accounts switches to that user when they are signed in on this browser. Otherwise, Account links, Settings, and Developers ask you to sign in, while pages such as this help and the terms of use open as the current user.",
