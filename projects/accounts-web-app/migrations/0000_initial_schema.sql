...
CREATE INDEX `client_consents_client_user_consented_idx` ON `client_consents` (`client_id`,`user_id`,`consented`);--> statement-breakpoint
...
--> statement-breakpoint
CREATE INDEX `external_account_verifications_account_id_idx` ON `external_account_verifications` (`account_id`);--> statement-breakpoint
CREATE UNIQUE INDEX `external_account_verifications_evidence_unique` ON `external_account_verifications` (`account_id`,`method`,`evidence_key`);--> statement-breakpoint
CREATE TABLE `external_account_visibility` (
	`account_id` text NOT NULL,
	`client_id` text NOT NULL,
...
--> statement-breakpoint
CREATE INDEX `external_account_visibility_client_account_public_idx` ON `external_account_visibility` (`client_id`,`account_id`,`is_public`);--> statement-breakpoint
CREATE TABLE `external_accounts` (
	...
	`linked_at` integer,
	`is_public` integer DEFAULT false NOT NULL,
	`imported_verifications_json` text,
	FOREIGN KEY (`user_id`) REFERENCES `user`(`id`) ON UPDATE no action ON DELETE cascade
);
--> statement-breakpoint
CREATE INDEX `external_accounts_user_id_idx` ON `external_accounts` (`user_id`);--> statement-breakpoint
CREATE UNIQUE INDEX `external_accounts_id_user_id_unique` ON `external_accounts` (`id`,`user_id`);--> statement-breakpoint
CREATE TABLE `external_identifiers` (
	...
	FOREIGN KEY (`account_id`,`user_id`) REFERENCES `external_accounts`(`id`,`user_id`) ON UPDATE no action ON DELETE cascade
);
--> statement-breakpoint
CREATE UNIQUE INDEX `external_identifiers_active_key_unique` ON `external_identifiers` (`kind`,`provider`,`issuer`,`value`) WHERE "external_identifiers"."is_active" = 1;--> statement-breakpoint
CREATE INDEX `external_identifiers_user_host_kind_idx` ON `external_identifiers` (`user_id`,`host`,`kind`);--> statement-breakpoint
CREATE UNIQUE INDEX `external_identifiers_user_key_unique` ON `external_identifiers` (`user_id`,`kind`,`provider`,`issuer`,`value`);--> statement-breakpoint
CREATE TABLE `verification_identifiers` (
	`verification_id` text NOT NULL,
	`identifier_id` text NOT NULL,
	PRIMARY KEY(`verification_id`, `identifier_id`),
	FOREIGN KEY (`verification_id`) REFERENCES `external_account_verifications`(`id`) ON UPDATE no action ON DELETE cascade,
	FOREIGN KEY (`identifier_id`) REFERENCES `external_identifiers`(`id`) ON UPDATE no action ON DELETE cascade
);
--> statement-breakpoint
CREATE INDEX `verification_identifiers_identifier_id_idx` ON `verification_identifiers` (`identifier_id`);--> statement-breakpoint
