...
- 一覧取得では、クライアントの認証・権限を確認した後、指定ユーザーの存在と問い合わせ元への有効な情報提供同意を確認する。ユーザーが存在しない場合と有効な同意がない場合は、同じ内容のHTTP 404で取得対象がないことを返す。ユーザーと同意が有効な場合はHTTP 200で一覧を返し、提供対象が0件なら応答の`externalAccounts`を空配列とする
...
一覧取得では、対象ユーザーIDを必須のクエリパラメーター`accountsUserId`で指定する。
...
各結果には`accountsUserId`と`errors`も常に含める。`matched`では該当するAccountsユーザーIDと空の`errors`配列、`no_match`では`accountsUserId: null`と空の`errors`配列、`invalid_input`では`accountsUserId: null`と確認できた不備を含む`errors`配列を返す。

| `status` | 意味 |
| --- | --- |
| `matched` | 照合条件と提供許可を満たすAccountsユーザーが該当する |
...
- 本人のAccounts利用でログイン状態の保持期限がその時点から7日後へ更新され、7日間利用がなければ再ログインが必要になる。クライアントによる一覧取得・照合の利用中も、本人のログイン状態には本人の最終利用からの期限を適用する
...
- Web検証は、同じURLを繰り返し検証する場合も、複数URLを検証する場合も、1ユーザーあたりUTCの日付ごとに合計150回まで実行でき、151回目を拒否する。UTCの0時に翌日分の枠へ切り替わり、再び検証できる。保存済みの結果を返す冪等な再送には重ねて回数を消費しない
...
- `identities:read` scopeを持つクライアント用JWTで一覧取得・照合の両APIを利用でき、そのscopeを持たないJWTはDB照合前に拒否する
...
- 照合APIのJSONに仕様にない項目名が含まれる場合、最上位の項目は要求全体のエラーになり、個別の識別子内の項目はその入力のエラーになる。ほかの正しい識別子の照合は継続できる
- 一覧取得・照合APIの要求全体のエラーと、各入力の`errors`配列のエラーに、機械判定用の`code`と英語の`message`が含まれる
