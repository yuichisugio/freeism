...
- 本人が編集できる公開Webページをサービスを限定せず対象とし、Accountsの設定画面でURLを登録する。Web URLは1ユーザーにつき最大150件登録できる
...
| 対象 | 仕様と検証観点 |
| --- | --- |
| プロフィール・ログイン | [プロフィール](#accountsプロフィール)の入力条件、[登録とログイン](#登録とログイン)の登録確認・各Provider・期限更新・最後のログイン手段 |
| 所有権・連携 | [所有権と紐付け](#所有権と紐付け)の競合時を含む一意性・移動、[外部認証情報](#外部認証情報)の明示連携・表示名更新・暗号化・失効確認と再試行 |
| Webページ検証 | [URL正規化](#url正規化)の同値・非同値、[証明リンク](#証明に使用するリンク)の候補優先・本人一致・複数主体・紐付け更新、[取得](#外部ページの取得)の接続・redirect・容量・時間・MIME・cache制約 |
| 検証回数・Turnstile | [検証要求の制御](#検証要求の制御)の上限・日付切替・冪等再送・hostname/action・期限・再使用・応答cache制御 |
| 情報提供 | [公開設定](#公開設定)の一般公開と各クライアントの独立性・現在の許可、[管理画面](#管理画面と監査)の一括選択・同意ON時の保存条件・拒否時の状態維持、[提供項目](#表示提供する情報) |
| OAuthクライアント | [クライアント管理](#oauthクライアント管理)の入力・管理権限・リダイレクトURL・Secret表示と再発行・既存トークン失効 |
| 認可・API | [クライアント認証と権限](#クライアント認証と権限)の初回本人確認と継続利用、[照合手順](#入力ごとの照合手順)の現在の保存済み情報による判定、[API契約](#apiの具体案)の各状態・入出力・空配列・混在入力・要求全体のエラー・上限 |
...
| 画面共通 | [管理画面と監査](#管理画面と監査)の言語初期選択・各状態・キーボード・スクリーンリーダー |

## 未決事項

- 要件確定前に、管理画面・公開設定・OAuthクライアント管理・復元画面の構成と操作の流れを対話で確認する
