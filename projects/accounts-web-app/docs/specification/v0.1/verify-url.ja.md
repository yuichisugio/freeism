...
1. Accountsにログインした本人が、「アカウント連携」画面の一つの入力欄へURLを入力し、「保存して検証する」ボタンを押す。サービスの判定はAccountsが行う。「未検証で保存」を選んだ場合は、URLを未検証として登録する。
...
- 個別対応サービスの入力URLの証明に成功すると、URL規則で確定できる対象アカウントのユーザー名・プロフィールURLを保存できる。URLで作者・所有者を確定できない記事・投稿・成果物は、取得済みの同一HTMLで対応を確認できた範囲だけ紐付ける。Codeberg・Hugging Faceの成果物は現在の所有アカウントを対象とし、組織所有も含める。所有者を確認できなければ入力URL単独で検証する。読者コメントの投稿者から作者・所有者の識別子を推定しない。
