...
利用者がURLを一つ入力して「保存して検証する」ボタンを押すと、AccountsがAccountsプロフィールへの公開されたリンク・記述を先に確認する。リンクによる証明が成立しなければ、同じ検証要求でDNS TXTによるドメイン所有権証明を確認する。証明が成立すると、対象のWebページと、取得できる外部サービスの識別情報を登録する。
...
4. 入力URLが未登録でも対象として受け付け、公開ページから本人のAccounts公開プロフィールURLを示す証拠を先に確認する。リンクによる証明が成立した場合は、入力URLを登録してその結果を保存する。ページを取得できない場合やリンクが一致しない場合を含め、リンクによる証明が成立しなければ、同じバックエンド要求で入力URLのhostのDNS TXTを確認する。
5. DNS TXTが一致した場合は、入力URLを登録して`dns_txt`の成功証明を保存する。成功した証明と対象の識別子を本人の外部アカウントへ対応付ける。同じ外部アカウントにOAuthとWebの証明を追加できる。いずれも成立しない場合、未登録の入力URLは登録せず、方法別の結果を本人に示して「未検証で保存」を案内する。登録済みの入力URLの再検証では、方法ごとの直近の試行結果（`checked_at`・`result`・`failure_code`）だけを更新し、登録と有効な証明を維持する。
6. 本人が選択した公開設定に従い、Accountsの公開プロフィールや許可されたOAuthクライアントへ情報を提供する。提供対象は[公開設定](main.ja.md#公開設定)に従い、未検証の登録情報は本人向けに管理する。

Web URLは、未検証の登録を含めて1ユーザー150件までとする。数える対象は本人の`kind='url'`の全行であり、本人が登録したURL、証明で自動的に保存されたプロフィールURL、OAuth由来のURLを含む。登録・検証・OAuth連携・復元で`url`行を追加する前に、現在の行数が150未満であることを確認し、150に達していれば追加を拒否して上限到達を本人に示す。「保存して検証する」「未検証で保存」とも、未登録の入力URLは外部ページの取得・DNS TXTの照会・検証要求の頻度制限の前に確認し、上限に達していれば拒否する。登録済みURLの再検証は上限の影響を受けない。同一ユーザーの同じ正規化URLは一つの登録として扱う。この上限は登録URL数の上限であり、検証リクエストの頻度制限は[検証要求の制御](main.ja.md#検証要求の制御)に従う。
...
TXTレコードは、Workersの`node:dns/promises`の`resolveTxt`で取得する。この照会は`cloudflare-dns.com`（1.1.1.1）へのDNS over HTTPSで行われ、subrequestとして数える。各レコードのcharacter-stringを連結し、[URL正規化](#url正規化)を適用して、期待する公開プロフィールURLとの完全一致を判定する。CNAMEの行は無視する。DNS照会の期限は3秒とし、[外部ページの取得](#外部ページの取得)の5秒とは別に数える。[Workersのnode:dns](https://developers.cloudflare.com/workers/runtime-apis/nodejs/dns/)

一致するTXTが無い場合と、回答の無い応答（NXDOMAIN・NODATA・SERVFAIL）は`not_verified`、DNS over HTTPSのHTTP・通信の失敗とtimeoutは`indeterminate`とする。Workersの`node:dns`は回答の無い応答をすべて`ENOTFOUND`として返し、SERVFAILとNXDOMAINを区別できないため、SERVFAILも`not_verified`となる。TXTに期待するURLと同じAccountsサービスの異なるユーザーの公開プロフィールURLが併存する場合は、[リンクによる証明](#証明に使用するリンクと本文)と同じ規則で`indeterminate`とする。TXTを追加した直後は、DNSの反映やリゾルバのキャッシュにより不一致が続くことがあるため、時間をおいて再検証するよう結果の案内に含める。
...
サービス内ユーザー名・プロフィールURLへの証明の適用は、プロフィールページ（そのアカウントだけが編集できるページ）の証拠と、OAuth・OIDCの検証済み応答に限る。リンク証明が成功し、最終取得URLが個別対応サービスのプロフィールURLである場合は、そのアカウントのユーザー名・プロフィールURLを入力URLとともに同じ外部アカウントへ保存し、この証明を適用する。記事・投稿・Gist・Merge Request・Issue・リポジトリ・モデル・データセット・SpacesなどのコンテンツURLは汎用Webページとして登録・検証し、証明は入力URL単独に適用する。
...
- 候補のURLを正規化し、セッション本人に対応するAccounts公開プロフィールURLと完全一致した場合に証明を成立させる。同じ証拠候補の範囲に、期待するURLと、同じAccountsサービスの異なるユーザーを示す公開プロフィールURLが併存する場合は判断不能とし、新しい紐付けや紐付け先の更新を確定しない。期待するURLが無い場合は、ほかのユーザーの公開プロフィールURLの有無によらず`not_verified`とする。同一URLの繰り返しは一つとして数える。それ以外で条件を満たした検証は自動で確定する。
...
リンク証明（`bidirectional_link`）単独の成功では、`dns_txt`または`oauth`の有効な証明が支えている識別子を移動しない。今回確認した識別子のうち1つでも別のユーザーの`dns_txt`または`oauth`の有効な証明が支えている場合は、そのリンク証明全体を`indeterminate`（`HELD_BY_STRONGER_PROOF`）とし、どの識別子の所有者も変更せず、同じ要求でDNS TXTの確認へ進む。本人には、その識別子が別の方法で証明済みの利用者に紐付いていることを案内する。今回確認した識別子がすべてリンク証明だけに支えられている場合は、再証明で今回の本人へ移動する。移動の規則は[所有権と紐付け](main.ja.md#所有権と紐付け)に従う。
...
期限・容量・redirectの上限は、利用者の待ち時間と取得負荷を抑えるための共通条件とする。取得不能、上限超過、解釈できない応答はリンク検証の直近結果として本人へ伝え、DNS TXTの確認へ進む。成功以外のHTTPステータスは、404・410をページが無いことを確認できたものとして`not_verified`、401・403・429のアクセス制限と5xxなどのその他のステータスを`indeterminate`とする。入力エラーは、入力URL自体の構文や取得先の安全性の不備に限る。redirect先の安全性違反はリンク検証の失敗（`not_verified`）とし、同じ要求でDNS TXTの確認へ進む。
...
直近の試行の`result`は`verified`・`not_verified`・`indeterminate`のいずれかとする。「保存して検証する」の応答では、証拠を確認したページ（最終取得URL）をリンク証明が成立した場合だけ示し、成立しなかった試行では示さない。直近の試行結果は登録済みの外部アカウントに保存する。未登録の入力URLでいずれの方法も成立しなかった場合は、試行結果を保存せず応答で返し、外部アカウントのIDを`null`として未登録であることを示す。リンク設置、公開状態の変更、OAuthによる証明など、本人の次の操作の案内は、直近の試行の`failure_code`から導いて表示する。方法別の検証結果、検証日時、証拠URL、確認した識別子を記録し、既存の有効な証明と直近の検証試行の結果を区別する。API・JSONの項目名は、主仕様の`verificationStatus`と`verifications`へ対応付ける。成功した証明の`verifiedAt`と、直近の試行の`checkedAt`・`result`を分けて保持する。
...
- 一つのURLを入力して「保存して検証する」を押すとサービスとURLの種類を判定し、未登録URLも対象としてリンク検証からDNS TXTの確認へ進め、証明が成立すれば登録する。未登録URLで両方法が不成立なら登録せず、方法別の結果とともに「未検証で保存」を案内し、そのボタンを押すと未検証のまま登録できる。登録数150件の境界を、「保存して検証する」（外部通信の前の拒否と登録済みURLの再検証）と「未検証で保存」で検査できる。
...
- リンク証明単独の成功では、確認した識別子のうち1つでも別のユーザーの`dns_txt`または`oauth`の有効な証明が支えていればリンク検証全体を`indeterminate`とし、所有者を変更せずにDNS TXTの確認へ進む。確認した識別子がすべてリンク証明だけに支えられている場合は再証明で移動できる。
...
- リンク証明に成功し、最終取得URLが個別対応サービスのプロフィールURLであれば、そのURL規則で得たユーザー名・プロフィールURLへ証明を適用できる。最終取得URLがコンテンツURLの場合の証明は入力URL単独に適用する。
