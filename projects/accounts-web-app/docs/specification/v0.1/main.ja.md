...
- トップページ（`/`）は簡単な使い方を示す事前生成ページとし、ログインはどの画面からも開けるログイン用ダイアログで行う。未ログインではヘッダー右とトップページの「ログインする」から開き、OAuth Providerのログイン画面としての署名付きクエリ（`?sig=`）付きの表示と、ログイン失敗の戻り先（`?error=`）では自動で開く。ログイン済みではログインの操作を表示しない
- ログインの成功後は「アカウント連携」画面、失敗時はトップページのログイン用ダイアログへ戻る。URLのユーザーでログインするために開いた場合（[管理画面と監査](#管理画面と監査)の画面のURL）は、成功・失敗とも同じURLへ戻る
...
- 同じブラウザー内で複数のAccountsユーザーのセッションを保持し、ヘッダー右のアカウントのメニューで切り替える。メニューは現在のユーザーのアイコン（表示名の頭文字）から開き、このブラウザーでログイン中のAccountsユーザーの一覧（Multi Sessionの`listDeviceSessions`。現在のユーザーを示す）、その下に「アカウントを追加」、最下部に「ログアウト」を置く
- 一覧のユーザーを押すと、そのユーザーを現在のセッションにする（Multi Sessionの`setActive`）。AccountsユーザーID付きの画面では同じ画面のそのユーザーのURLへ移動し、移動先の画面が切り替える。未保存の変更がある場合は、[管理画面と監査](#管理画面と監査)の移動の確認を経る
- 「アカウントを追加」はログイン用ダイアログを開き、現在のユーザーへの外部アカウントの追加連携ではなく、別のAccountsユーザーとしてログインしてこのブラウザーのセッションに加える
- 「ログアウト」は、トップページへ移動してから現在のAccountsユーザーのセッションだけを終了する（Multi Sessionの`revoke`）。このブラウザーでほかのAccountsユーザーがログイン中なら、そのユーザーが現在のセッションになる
- 同じブラウザーで保持できるAccountsユーザーは、Multi Sessionの既定の上限の5人までとする。5人がログイン中のブラウザーで6人目としてログインすると、6人目は現在のユーザーになるが、メニューの一覧に加わらず、「ログアウト」もできない
- 同意画面でメニューからほかのユーザーを選ぶと、選んだユーザーで同じ認可要求を続ける。`prompt=consent`の要求では、選んだユーザーの同意画面が開き直される。`prompt=consent`の無い要求で、選んだユーザーが保存済みのOAuth同意を持つ場合は、同意画面を省いて利用側サービスへ戻る。同意画面の署名付きクエリが失効した後は切り替えられない
- 認可要求の`prompt=login`による再認証は保証しない。ログイン用ダイアログを閉じてメニューからログイン中のユーザーを選ぶと、再認証を経ずにそのユーザーで認可要求を続ける
...
| 設定 | 表示言語、[Accountsの表示名](#accountsプロフィール)、[JSON出力・復元](#jsonによるバックアップと移行)、[退会](#accountsユーザーの退会) |
| アカウント連携 | [外部アカウントの追加・解除](#所有権と紐付け)、[Web URLの登録・検証・結果確認](#webページの検証仕様)、[一般公開と連携先サービスごとの公開設定](#公開設定) |

- 公開プロフィール以外の画面（「アカウント連携」「設定」「開発者向け」、ヘルプ・プライバシーポリシー・利用規約・OSSライセンス）は、先頭にAccountsユーザーIDを付けたURL（例: `/{accountsUserId}/account-links`）で表示する。URLのIDは表示するユーザーの切替だけに使い、画面用API（BFF）と権限判定はCookieのセッションで本人を確認する
- IDの無いURL（`/account-links`など）は、ログイン済みなら現在のユーザーのID付きのURLへクエリを保って移り、未ログインならIDの無いまま表示してログイン後にID付きのURLへ移る。OAuth Providerの同意画面（`/account-links?sig=...`）も同じく現在のユーザーのID付きのURLへ移る
- 画面を開いたとき、URLのIDが現在の有効セッションのユーザーと異なる場合は、このブラウザーでそのユーザーがログイン中なら`setActive`で切り替えてから表示する
- ユーザーの情報を扱う3画面（「アカウント連携」「設定」「開発者向け」）では、URLのユーザーでこのブラウザーにログインしていなければログイン用ダイアログを開き、ログイン後に同じURLへ戻す。URLのユーザーと現在のユーザーが揃うまで画面の内容は表示しない
- ヘルプ・プライバシーポリシー・利用規約・OSSライセンスでは、URLのユーザーでこのブラウザーにログインしていなければログインを求めず、現在のユーザーのID付きのURL（未ログインならIDの無いURL）へ置き換えて表示する
- AccountsユーザーIDだけのURL（`/{accountsUserId}`）は、そのユーザーの「アカウント連携」画面のURLへ置き換える
- ヘッダー・フッターのリンクとアカウントのメニューの移動先、追加連携の戻り先も、ID付きのURLとする
- トップページとIDの無い画面（`/help`など）はビルド時に事前生成し、ID付きの画面はブラウザーで描画する
...
- 「設定」画面には表示言語の設定項目を置き、ログインしていなくても選べる。選んだ言語はすぐに表示へ反映し、ブラウザーの保存領域（localStorage）に保存する
