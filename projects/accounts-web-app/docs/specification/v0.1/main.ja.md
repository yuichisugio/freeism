...
- 画面を開いたとき、URLのIDが現在の有効セッションのユーザーと異なる場合は、このブラウザーでそのユーザーがログイン中なら`setActive`で切り替えてから表示し、ログインしていなければログイン用ダイアログを開いてログイン後に同じURLへ戻す。URLのユーザーと現在のユーザーが揃うまで画面の内容は表示しない
