...
describe("points Worker routing", () => {
  it("serves the health contract", async () => {
    const response = await SELF.fetch("https://points.test/api/health");

    expect(response.status).toBe(200);
    expect(response.headers.get("content-type")).toContain("application/json");
    await expect(response.json()).resolves.toEqual({ service: "points-worker", status: "ok" });
  });
  ...
  it("serves the SPA shell only for HTML navigation", async () => {
    const response = await SELF.fetch("https://points.test/dashboard", {
      headers: {
        Accept: "text/html",
  ...
  it("does not treat JSON or POST requests as navigation", async () => {
    const jsonResponse = await SELF.fetch("https://points.test/dashboard", {
      headers: { Accept: "application/json" },
    });
    const postResponse = await SELF.fetch("https://points.test/dashboard", {
      method: "POST",
