...
Marketsは、商材の出品、Auction、入札、配分、精算進行、落札証明を管理する。Taskを作成せず、評価軸、FIX、ポイント残高を所有しない。

### 主なaggregate

- `marketsUser`とMarkets自身のBetter Auth session/account
- `pointsConnection`と暗号化OAuth token metadata
- `listing`、`listingRevision`、`pointPackageSnapshot`
- `auction`、`auctionRevision`、`bid`、`autoBid`
...
- listingと最初のAuctionはCSVの同じ1行から同時作成する。一般利用者向けGUI一括入力formを作らない。
- UTF-8、最大5MiB、1,000非空行、全件validation、preview、confirm、全件原子確定を共通ruleとする。
- listingの作成者だけがAuction開始前に編集・取消できる。ただし受理済みbid、AutoBid、buy-now holdが1件でもあれば取消できない。
- 最初の有効bid以後、結果へ影響するeconomic fieldを変更できない。

### 3.2 開始前の編集と取消

- browser BFFは`PATCH /api/listings/{listingId}`と`POST /api/listings/{listingId}/cancellations`を提供する。request bodyのseller IDを信用せず、Markets Sessionの作成者ID、`expectedListingVersion`、`expectedAuctionVersion`を必須にする。
- 編集はserver時刻が`startsAt`より前で、Auctionが`DRAFT`または`SCHEDULED`である場合だけ許可する。入力をCSV作成時と同じruleで再検証し、既存rowを上書きせず新しい`listingRevision`、`auctionRevision`、必要なら`pointPackageSnapshot`を追加する。
- 取消は作成者、開始前、`DRAFT`または`SCHEDULED`、受理済みbid 0件、AutoBid 0件、buy-now hold 0件を同じD1 CAS guardで再確認する。成功時はlistingとAuctionを同じtransactionで`CANCELLED`へ進め、取消event、idempotency result、auditを追加する。物理削除しない。
...
> 本節のlisting文字／URL境界はDEC-254、Package文字／hash境界はDEC-257、現在lifecycleの30秒receiptはDEC-256の承認対象であり、各DECが`採用`へ変わるまで依存実装を開始しない。
...
- 外部参照URL: v0.2はlistingごとにちょうど1件。UTF-8で最大2,048 bytes、`https`だけを許可し、userinfo、fragment、control文字を拒否する。scheme／IDNA変換後hostは小文字、default portは除去し、pathのdot segmentとpercent encodingを標準URL parserで正規化して保存する
- seller Markets user IDと公開identity snapshot
- Points issuer/resource
- `pointPackageId`と不変`pointPackageRevisionId`
- 新規作成／開始前編集時点で、Public Revisionの履歴`status=ACTIVE`かつPointsの現在`lifecycleStatus=ACTIVE`であるPackage。確定直前のM2M listing eligibility receiptを必須とし、確定後にINACTIVEへ変わっても既存Auction snapshotは継続する
- package名、構成評価軸ID/名前、比率、各`minimumUnit`のsnapshot
- listing eligibility receipt ID、command ID／hash、Package eligibility version、検査時刻、有効期限、Markets D1 commit開始時刻のsnapshot
- 販売数量: 1〜1,000の安全整数
- Auction開始・終了時刻
...
v0.2は画像を保存・表示しない。listingの詳細はtextと安全な外部URLで表現する。
...
- package revision確定時にtickを計算し、listingへsnapshotする。
...
- commandは`commandId`、expected auction version、quantity、manual priceまたはAutoBid上限を持つ。
...
- listingのpackage revisionとbid対象が一致する。
...
> reviewの文字／URL境界はDEC-254の承認対象であり、`採用`へ変わるまで依存実装を開始しない。

- settle完了時に公開・永続的なproof IDとcanonical URLを作る。
- proofはAuction/listing/package revision、seller/buyer identity snapshot、allocation quantity、uniform price、component vector、settlement timestamp、plan hashを持つ。
...
外部からlisting作成、bid、購入、通知を行うpublic write APIはv0.2で提供しない。
