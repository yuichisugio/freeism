...
export { AuctionRoom } from "./backend/auction/auction-room";
export { AuctionSettlementWorkflow } from "./backend/settlement/auction-settlement-workflow";
...
export function fetchMarketsApi(request: Request, workerEnv: Env) {
...
export async function runScheduledMarkets(controller: ScheduledController, workerEnv: Env) {
  if (controller.cron !== "*/5 * * * *") return;
  const startedAt = Date.now();
  const results = await runMarketsCronJobs([
    () => runScheduledSettlementMaintenance(workerEnv),
    () =>
...
export default {
  ...
  scheduled(controller, workerEnv, context) {
    context.waitUntil(runScheduledMarkets(controller, workerEnv));
  },
