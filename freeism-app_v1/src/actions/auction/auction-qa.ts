...
export async function getAuctionMessagesAndSellerInfo(
  ...
  auctionEndDate: Date,
): PromiseResult<{ messages: AuctionMessage[]; sellerInfo: AuctionPersonInfo | null }> {
  ...
  if (!messages.success || !sellerInfo.success) {
    throw new Error(
      messages.message && sellerInfo.message
        ? messages.message + sellerInfo.message
        : messages.message || sellerInfo.message,
    );
  }
...
): PromiseResult<{ messageId: string; messageContent: string; createdAt: Date }> {
  // ーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーー
