...
export async function bulkCreateEvaluations(
  ...
  groupId: string,
  userId: string,
): PromiseResult<Array<{ count: number; message: string }>> {
  ...
  if (validationErrors.length > 0) {
    ...
    return { success: false, message: `データ検証に失敗しました: ${errorMessages.join("; ")}`, data: [] };
  }
  ...
  return {
    success: true,
    message: `${result.count}件のデータを登録しました`,
    data: [{ count: result.count, message: `${result.count}件のデータを登録しました` }],
  };
}
