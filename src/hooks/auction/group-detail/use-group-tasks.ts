...
export function useGroupTasks({ groupId, isGroupOwner, isAppOwner }: UseGroupTasksProps): UseGroupTasksReturn {
  ...
  const updateUrlParams = useCallback(
    (newTableConditions: GroupDetailTableConditions) => {
      ...
      if (newTableConditions.sort?.field) params.set("sort_field", newTableConditions.sort.field as string);
      if (newTableConditions.sort?.direction) params.set("sort_direction", newTableConditions.sort.direction);
      if (newTableConditions.searchQuery) params.set("q", newTableConditions.searchQuery);
      if (newTableConditions.contributionType) params.set("contribution_type", newTableConditions.contributionType);
      if (newTableConditions.status) params.set("status", newTableConditions.status);
      if (newTableConditions.itemPerPage) params.set("item_per_page", String(newTableConditions.itemPerPage));
  ...
  const { mutateAsync: deleteTaskMutateAsync, isPending: isDeletingTask } = useMutation({
    ...
    onSuccess: async () => {
      ...
      await queryClient.invalidateQueries({ queryKey: queryCacheKeys.tasks.byGroupIdWithConditions<GroupDetailTask>(groupId, tableConditions) });
      router.refresh();
    },
    onError: (error) => {
  ...
  const handleTaskEdited = useCallback(() => {
    void queryClient.invalidateQueries({ queryKey: queryCacheKeys.tasks.byGroupIdWithConditions<GroupDetailTask>(groupId, tableConditions) });
    toast.success("タスクデータを更新しました");
    router.refresh();
  }, [groupId, router, queryClient, tableConditions]);
