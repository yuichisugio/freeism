...
import type { AuctionEventData, AuctionWithDetails, BidHistoryWithUser } from "@/types/auction-types";
import { useCallback, useEffect, useMemo, useRef, useState } from "react";
import { AuctionEventType } from "@/types/auction-types";
...
export function useAuctionEvent(initialAuction: AuctionWithDetails): UseAuctionEventResult {
  ...
  const processEventDataByType = useCallback(
    (eventData: AuctionEventData): void => {
      ...
      const auctionData: AuctionWithDetails = {
        ...
        extensionTotalCount: receivedData?.extensionTotalCount ?? initialAuction.extensionTotalCount,
        extensionLimitCount: receivedData?.extensionLimitCount ?? initialAuction.extensionLimitCount,
        extensionTime: receivedData?.extensionTime ?? initialAuction.extensionTime,
        remainingTimeForExtension: receivedData?.remainingTimeForExtension ?? initialAuction.remainingTimeForExtension,
        options: {
          reconnectOnVisibility: true,
          clientId: clientId,
        },
  ...
  const editSSEdata = useCallback(
    (text: string): void => {
      ...
      console.log(
        `SSE_editSSEdata_SSEイベント解析結果: type=${event}, id=${id}, データ長=${data?.length || 0}, dataフィールド存在=${hasDataField}`,
      );
      ...
      try {
        ...
        while (isConnectedRef.current) {
          ...
          try {
            ...
            console.log(
              "SSE_handleSSEStream_SSEストリームからデータ受信:",
              value.length,
              "バイト, バッファ:",
              buffer.length,
            );
