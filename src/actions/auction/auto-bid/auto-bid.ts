...
import type { ValidateAuctionResult } from "@/actions/auction/bid-validation";
...
export async function executeAutoBid(params: ExecuteAutoBidParams): PromiseResult<ExecuteAutoBidReturn | null> {
  ...
  if (!validationDone && !validationResult) {
    const validateResult = await validateAuction(auctionId, {
      checkEndTime: true,
      requireActive: true,
  ...
  if (!validationResult?.auction) {
    console.error("自動入札処理: オークションのバリデーションに失敗しました");
    return {
      success: false,
      message: "検証エラー",
      data: null,
