 ...
 */
export async function getCachedGroupTaskAndCount(
  params: GetTasksByGroupIdProps,
): PromiseResult<GetGroupTaskAndCountReturn> {
  ...
  if (!tasks) {
    return {
      success: false,
      message: "タスクが見つかりません",
      data: { returnTasks: [], totalTaskCount: 0 },
    };
  }
  ...
  return {
    ...
    data: { returnTasks, totalTaskCount },
  };
}
