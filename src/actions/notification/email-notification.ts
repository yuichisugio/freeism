 ...
 */
export async function sendEmailNotification(params: NotificationParams): PromiseResult<null> {
  ...
  if (emailNotificationSettings.length === 0) {
    return { success: true, message: "メール通知設定が見つかりません", data: null };
  }
   ...
   * メール送信に成功した場合
   */
  return { success: true, message: "メール通知を送信しました", data: null };
}
