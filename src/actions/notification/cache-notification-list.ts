...
export const cachedGetNotificationsAndUnreadCount = cache(
  async (userId: string, page = 1, limit = 20): PromiseResult<NotificationAndUnreadCount> => {
     ...
     * パラメータチェック
     */
    if (!userId || !page || typeof page !== "number" || page < 1 || !limit || typeof limit !== "number" || limit < 1) {
