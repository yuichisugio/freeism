 ...
 */
export async function sendGeneralNotification(params: GeneralNotificationParams): PromiseResult<null> {
  ...
  if (targetUserIds.data.length === 0 || !targetUserIds.success) {
  ...
  const notificationParams: NotificationParams = {
    recipientUserIds: targetUserIds.data,
    title: params.title,
  ...
  if (
    ...
    return { success: true, message: "スケジュール通知の登録を完了しました", data: null };
  }
   ...
   */
  return { success: true, message: "通知の登録を完了しました", data: null };
}
