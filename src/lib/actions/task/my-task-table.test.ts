...
describe("getMyTaskData", () => {
  ...
  describe("正常系", () => {
    test("should return user tasks successfully with default conditions", async () => {
      ...
      // Act
      const result = await getMyTaskData(defaultTableConditions);
    ...
    test("should apply search query filter", async () => {
      ...
      // Act
      await getMyTaskData(tableConditions);
    ...
    test("should apply pagination correctly", async () => {
      ...
      // Assert
      expect(prismaMock.task.findMany).toHaveBeenCalledWith({
        where: expect.any(Object) as unknown as Prisma.TaskWhereInput,
        select: expect.any(Object) as unknown as Prisma.TaskSelect,
        orderBy: { createdAt: "desc" },
        skip: 10, // (3 - 1) * 5
        take: 5,
    ...
    test("should apply sort by taskFixedContributionPoint", async () => {
      ...
      const tableConditions: MyTaskTableConditions = {
        ...defaultTableConditions,
        sort: { field: "taskFixedContributionPoint", direction: "asc" },
      };
    ...
    test("should apply sort by taskCreatorName", async () => {
      ...
      const tableConditions: MyTaskTableConditions = {
        ...defaultTableConditions,
        sort: { field: "taskCreatorName", direction: "asc" },
      };
      ...
      expect(prismaMock.task.findMany).toHaveBeenCalledWith({
        where: expect.any(Object) as unknown as Prisma.TaskWhereInput,
        select: expect.any(Object) as unknown as Prisma.TaskSelect,
        orderBy: { creator: { settings: { username: "asc" } } },
        skip: 0,
        take: 10,
      });
    });
    ...
    test("should apply sort by taskStatus", async () => {
      ...
      const tableConditions: MyTaskTableConditions = {
        ...defaultTableConditions,
        sort: { field: "taskStatus", direction: "desc" },
      };
    ...
    test("should apply sort by taskFixedEvaluator", async () => {
      ...
      const tableConditions: MyTaskTableConditions = {
        ...defaultTableConditions,
        sort: { field: "taskFixedEvaluator", direction: "asc" },
      };
      ...
      expect(prismaMock.task.findMany).toHaveBeenCalledWith({
        where: expect.any(Object) as unknown as Prisma.TaskWhereInput,
        select: expect.any(Object) as unknown as Prisma.TaskSelect,
        orderBy: { fixedEvaluator: { settings: { username: "asc" } } },
        skip: 0,
        take: 10,
      });
    });
    ...
    test("should apply sort by taskFixedEvaluationLogic", async () => {
      ...
      const tableConditions: MyTaskTableConditions = {
        ...defaultTableConditions,
        sort: { field: "taskFixedEvaluationLogic", direction: "desc" },
      };
      ...
      expect(prismaMock.task.findMany).toHaveBeenCalledWith({
        where: expect.any(Object) as unknown as Prisma.TaskWhereInput,
        select: expect.any(Object) as unknown as Prisma.TaskSelect,
        orderBy: { fixedEvaluationLogic: "desc" },
        skip: 0,
        take: 10,
      });
    });
    ...
    test("should apply sort by auctionId", async () => {
      ...
      const tableConditions: MyTaskTableConditions = {
        ...defaultTableConditions,
        sort: { field: "auctionId", direction: "desc" },
      };
      ...
      expect(prismaMock.task.findMany).toHaveBeenCalledWith({
        where: expect.any(Object) as unknown as Prisma.TaskWhereInput,
        select: expect.any(Object) as unknown as Prisma.TaskSelect,
        orderBy: { createdAt: "desc" }, // auctionIdソートはcreatedAtソートにマップされる
        skip: 0,
        take: 10,
      });
    });
  ...
  describe("異常系", () => {
    test("should throw error when getAuthenticatedSessionUserId fails", async () => {
      ...
      // Act & Assert
      await expect(getMyTaskData(defaultTableConditions)).rejects.toThrow("タスク情報の取得中にエラーが発生しました");
    });
