 ...
 * オークション延長処理の結果型
 */
export type ProcessAuctionExtensionResult = {
  success: boolean;
...
export async function processAuctionExtension(params: ProcessAuctionExtensionParams): Promise<ProcessAuctionExtensionResult> {
  ...
  try {
    ...
    if (auction.extensionTotalCount >= auction.extensionLimitCount) {
      throw new Error("延長回数の上限に達しています");
    }
    ...
    if (remainingTime > triggerTime) {
      throw new Error("延長トリガーの条件を満たしていません");
    }
    ...
    return {
      success: false,
      newEndTime: null,
      message: `${error instanceof Error ? error.message : "オークション延長中に不明なエラーが発生しました"}`,
    };
  }
}
