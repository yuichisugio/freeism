 ...
 */
export async function processAutoBid(params: ProcessAutoBidParams) {
  try {
    // ーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーー

    const { auctionId, currentHighestBid, currentHighestBidderId } = params;

    // ーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーー

    // オークションの存在確認と基本的なバリデーション
    const validationResult = await validateAuction(auctionId, {
      checkEndTime: true,
      requireActive: true,
    });

    if (!validationResult.success || !validationResult.auction) {
    ...
    const autoBids = await prisma.autoBid.findMany({
      where: {
        ...
        ...(currentHighestBidderId ? { userId: { not: currentHighestBidderId } } : {}), // 現在の最高入札者の自動入札は除外
        isActive: true,
      },
      include: {
        user: {
    ...
    return {
      ...
      isLimitReached: newBidAmount >= highestAutoBid.maxBidAmount,
    };
